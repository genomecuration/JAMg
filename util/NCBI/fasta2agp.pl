...
### Alexie Papanicolaou a.papanicolaou@westernsydney.edu.au
...
my $fasta_outfile = "$sequence_file";
open (FILE, ">$fasta_outfile.contigs.fsa") ||   die "Failed to write to file '$fasta_outfile'\n";

open (OUT,">$fasta_outfile.agp") ||die $!;
...
while (my $seq_obj = $inseq->next_seq ) {
   ...
   my $filler9b;
     ...
     if ($component_type5 eq 'N') {
       ...
       print OUT "$object1\t$object_beg2\t$object_end3\t$part_number4\t$component_type5\t$gap_length6b\t$gap_type7b\t$linkage8b\t$filler9b\n";
   } else {
       ### print AGP line for contig
       print OUT "$object1\t$object_beg2\t$object_end3\t$part_number4\t$component_type5\t$component_id6a\t$component_beg7a\t$component_end8a\t$orientation9a\n";
         }
   }
} 
