static char rcsid[] = "$Id: splicetrie.c 218286 2019-01-23 16:46:55Z twu $";
#ifdef HAVE_CONFIG_H
#include <config.h>
#endif
  ...
  if (single_leaf_p(leaf = triestart[0])) {
    ...
    if (offseta > 0) {
      best_pairs = solve_end5_aux(&(*coordsptr),coords,best_pairs,&(triestart[-offseta]),
				  ...
				  cdna_direction,watsonp,genestrand,jump_late_p,pairpool,extraband_end,defect_rate);
    }
    if (offsetc > 0) {
...
dump_left_aux (int *best_nmismatches, Univcoordlist_T coords, Triecontent_T *triecontents,
  ...
  if (nmismatches > *best_nmismatches) {
	...
	nmismatches =
	  Genome_count_mismatches_substring(genomebits,genomebits_alt,query_compress,segment_left,pos5,pos3,plusp);
	...
	*best_nmismatches = nmismatches;
	Univcoordlist_free(&coords);
	return Univcoordlist_push(NULL,position);
	...
	debug3(printf("  nmismatches %d == best_nmismatches %d => pushing leaf %d (same best_nmismatches %d)\n",
		      nmismatches,*best_nmismatches,(int) leaf,nmismatches));
	return Univcoordlist_push(coords,position);
      } else {
	return coords;
      }
	...
	*best_nmismatches = nmismatches;
	...
	if (nmismatches < *best_nmismatches) {
	  ...
	  *best_nmismatches = nmismatches;
	  Univcoordlist_free(&coords);
	  coords = Univcoordlist_push(NULL,position);
	} else if (nmismatches == *best_nmismatches) {
	  debug3(printf("  nmismatches %d == best_nmismatches %d => pushing leaf %d (same best_nmismatches %d)\n",
			nmismatches,*best_nmismatches,(int) leaf,nmismatches));
	  coords = Univcoordlist_push(coords,position);
	}
...
Splicetrie_dump_coords_left (int *best_nmismatches, Triecontent_T *triestart, int pos5, int pos3,
  ...
  if ((n = Univcoordlist_length(coords)) > 1 && snpp == true) {
    array = (Univcoord_T *) MALLOCA(n * sizeof(Univcoord_T));
    Univcoordlist_fill_array_and_free(array,&coords);
    qsort(array,n,sizeof(Univcoord_T),Univcoord_compare);

    coords = (Univcoordlist_T) NULL;
    coords = Univcoordlist_push(coords,array[0]);
    lastvalue = array[0];
    for (k = 1; k < n; k++) {
      if (array[k] == lastvalue) {
	/* Skip */
      } else {
	coords = Univcoordlist_push(coords,array[k]);
	lastvalue = array[k];
      }
    }
    FREEA(array);
...
static Univcoordlist_T
dump_right_aux (int *best_nmismatches, Univcoordlist_T coords,
  ...
  if (nmismatches > *best_nmismatches) {
	...
	*best_nmismatches = nmismatches;
	...
	if (pos3 - pos5 <= 16) {
	  ...
	  segment_left = position - pos5;
	  nmismatches =
	    Genome_count_mismatches_substring(genomebits,genomebits_alt,query_compress,segment_left,
					      pos5,pos3,plusp);
	}
...
Splicetrie_find_left (int *best_nmismatches, Intlist_T *nmismatches_list, int i,
  ...
  nmismatches_int =
    Genome_count_mismatches_substring(genomebits,genomebits_alt,query_compress,origleft,
				      /*pos5*/pos3,/*pos3*/spliceregion,plusp,genestrand);
...
Splicetrie_find_right (int *best_nmismatches, Intlist_T *nmismatches_list, int i,
  ...
  nmismatches_int = 
    Genome_count_mismatches_substring(genomebits,genomebits_alt,query_compress,origleft,
				      /*pos5*/spliceregion,/*pos3*/pos5,plusp,genestrand);
  debug2(printf("  internal at %u => %d nmismatches/%d\n",
		origleft,nmismatches_int,SPLICE_REGION));
  if (nmismatches_int > SPLICE_REGION_MISMATCHES) {
    debug2(printf("  too many mismatches internal to splice site => returning NULL\n"));
