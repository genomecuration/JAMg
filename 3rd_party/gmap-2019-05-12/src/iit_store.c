static char rcsid[] = "$Id: iit_store.c 215546 2018-05-31 01:01:48Z twu $";
#ifdef HAVE_CONFIG_H
#include <config.h>
#endif
...
scan_header_spaces (int *labellength, bool *seenp, List_T *divlist, List_T *typelist, Tableint_T div_seenp, Tableint_T typetable, 
		    ...
		    int line_length) {
  char *divstring, *copy, *acc, *typestring, *p, *ptr, *divstart;
  char *valueptr;
  int divlength;
  int nscanned;
  ...
  *seenp = false;
  ...
  if (nscanned < 3) {
    ...
    *label = (char *) MALLOC((*labellength+1)*sizeof(char));
    ...
    if (*p == '\0') {
      divstring = (char *) MALLOC(1*sizeof(char));
      divstring[0] = '\0';
      *type = 0;		/* Empty type string */
      ...
      divstring = (char *) MALLOC((divlength+1)*sizeof(char));
      strncpy(divstring,divstart,divlength);
      divstring[divlength] = '\0';
