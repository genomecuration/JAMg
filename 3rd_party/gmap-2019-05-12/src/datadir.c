static char rcsid[] = "$Id: datadir.c 218159 2019-01-17 05:49:10Z twu $";
#ifdef HAVE_CONFIG_H
#include <config.h>
#endif
...
read_config_file (FILE *fp, char *tag) {
  char *directory, *seentag, *dirbuffer, *line;
  int line_length;

  while ((line = Getline_wlength(&line_length,fp)) != NULL) {
    seentag = (char *) MALLOC((line_length+1)*sizeof(char));
    dirbuffer = (char *) MALLOC((line_length+1)*sizeof(char));

    if (sscanf(line,"%s=%s",seentag,dirbuffer) > 0 && !strcmp(seentag,tag)) {
      directory = (char *) MALLOC((strlen(dirbuffer)+1)*sizeof(char));
...
find_fileroot (char *genomesubdir, char *genomedir, char *dbroot) {
  ...
  while ((entry = readdir(dp)) != NULL) {
    ...
    if ((p = rindex(filename,'.')) != NULL) {
      if (!strcmp(p,".version")) {
	FREE(fileroot);
	fileroot = (char *) MALLOC((p - &(filename[0]) + 1)*sizeof(char));
	strncpy(fileroot,filename,p-&(filename[0]));
	fileroot[p-&(filename[0])] = '\0';
