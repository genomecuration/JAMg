static char rcsid[] = "$Id: gmap.c 219229 2019-05-12 22:33:42Z twu $";
#ifdef HAVE_CONFIG_H
#include <config.h>
#endif
...
/* #include "localdb.h" */
/* #include "oligoindex_localdb.h" --  For Oligoindex_localdb_setup */
#endif
#include "stage2.h"
...
update_stage3list (List_T stage3list, Sequence_T queryseq,
  ...
  for (p = all_stage2results; p != NULL; p = List_next(p)) {
    ...
    pairarray = Stage3_compute_one(&cdna_direction,&sensedir,&pairs,&npairs,&goodness,
				   ...
				   watsonp,genestrand,/*jump_late_p*/watsonp ? false : true,maxpeelback,
				   oligoindices_minor,diagpool,cellpool,
				   pairpool,dynprogL,dynprogM,dynprogR,sense_try,sense_filter);
...
/* Not sure how to treat genestrand for usersegment */
static Stage3_T *
stage3_from_usersegment (int *npaths_primary, int *npaths_altloc, int *first_absmq, int *second_absmq,
  ...
  stage3middle_list = update_stage3middle_list(/*stage3middle_list*/NULL,queryseq,
					       ...
					       pairpool,diagpool,cellpool,chrnum,chroffset,chrhigh,chrlength,
  ...
  if (stage3middle_list == NULL) {
    *npaths_primary = *npaths_altloc = 0;
	...
	FREE_OUT(pairarray);
				      ...
				      matches,unknowns,mismatches,
				      qopens,qindels,topens,tindels,ncanonical,nsemicanonical,nnoncanonical,
				      chrnum,chroffset,chrhigh,chrlength,watsonp,/*genestrand for usersegment*/0,
				      /*querylength*/Sequence_fulllength(queryseq),
				      /*skiplength*/Sequence_skiplength(queryseq),
				      /*trimlength*/Sequence_trimlength(queryseq),
...
#endif
						       queryuc,stage2_alloc,oligoindices_major,oligoindices_minor,
						       pairpool,diagpool,cellpool,Gregion_chrnum(gregion),
...
merge_left_and_right_readthrough (bool *mergedp, List_T stage3list,
				  Stage3_T *stage3array_sub1, int bestfrom,
  ...
  if ((stage3 = Stage3_merge_local(best0,best1,/*minpos1*/0,/*maxpos1*/breakpoint,
				   ...
				   oligoindices_minor,diagpool,cellpool,
				   pairpool,dynprogL,dynprogM,dynprogR,maxpeelback)) == NULL) {
    *mergedp = false;
    ...
    stage3list = List_push(stage3list,(void *) stage3);
    debug2(Stage3_print_ends(stage3));
    *mergedp = true;
  }
...
find_breakpoint (int *cdna_direction, int *chimerapos, int *chimeraequivpos, int *exonexonpos,
  ...
  Stage3_extend_right(from,/*goal*/rightpos,
		      ...
		      /*queryuc_ptr*/Sequence_fullpointer(queryuc),
#endif
		      max_extend_p,pairpool,Stage3_genestrand(from),maxpeelback_from);
  ...
  Stage3_extend_left(to,/*goal*/leftpos,
		     ...
		     /*queryuc_ptr*/Sequence_fullpointer(queryuc),
#endif
		     max_extend_p,pairpool,Stage3_genestrand(to),maxpeelback_to);
...
check_middle_piece_chimera (bool *foundp, List_T stage3list, Sequence_T queryseq, Sequence_T queryuc,
  ...
  Sequence_T querysubseq = NULL, querysubuc = NULL;
  int npaths, i, j;
  Stage3_T bestfrom, bestto, from, to, middle;
  Stage3_T *by_queryend, *by_querystart;
  List_T r;
...
apply_stage3 (bool *mergedp, Chimera_T *chimera, List_T gregions, Sequence_T queryseq, Sequence_T queryuc,
  ...
  *chimera = NULL;
  ...
  /* Split on large introns (may need to check whether stage3 belongs
     to a chimera) */
  if (split_large_introns_p == true) {
    ...
    for (p = stage3list; p != NULL; p = List_next(p)) {
      ...
      if (Stage3_npairs(stage3) == 0) {
	Stage3_free(&stage3);
      } else if ((split_objects = Stage3_split(stage3,queryseq,pairpool)) == NULL) {
	debug(printf("Pushing %p onto newstage3list\n",stage3));
	newstage3list = List_push(newstage3list,(void *) stage3);
      } else {
  ...
  for (p = stage3list; p != NULL; p = List_next(p)) {
    ...
    if (circularp[chrnum] == false) {
      ...
      if ((newstage3 = Stage3_new_from_pairs(pairs_below,cdna_direction,watsonp,genestrand,sensedir,
					     pairpool,queryseq,/*query_subseq_offset*/0,
...
align_relative (FILE *input, char **files, int nfiles, int nextchar,
...
#endif
  if (mode == CMET_NONSTRANDED || mode == ATOI_NONSTRANDED || mode == TTOC_NONSTRANDED) {
    gregions = Stage1_compute_nonstranded(&lowidentityp,referenceuc,indexdb_fwd,indexdb_rev,
					  chromosome_iit,chrsubset_start,chrsubet_end,matchpool,
					  stutterhits,diagnostic,/*stopwatch*/NULL);
  } else {
    gregions = Stage1_compute(&lowidentityp,referenceuc,indexdb_fwd,indexdb_rev,/*genestrand*/0,
			      chromosome_iit,chrsubset_start,chrsubet_end,matchpool,
			      stutterhits,diagnostic,/*stopwatch*/NULL);
  }
  ...
  if (stage3list == NULL) {
    ...
    stage3array = stage3array_from_list(&npaths_primary,&npaths_altloc,stage3list,
  ...
  if (npaths_primary + npaths_altloc > 0) {
    ...
    while (jobid == 0 || (queryseq = Sequence_read_multifile(&nextchar,&input,read_files_command,&files,&nfiles)) != NULL) {
      Matchpool_reset(matchpool);
      Pairpool_reset(pairpool);
      Diagpool_reset(diagpool);
      Cellpool_reset(cellpool);
...
#ifndef PMAP
	    Stage3_translate_cdna_via_reference(stage3array[0],stage3ref);
#endif
...
open_input_stream (int *nread, Sequence_T *usersegment, int argc, char **argv) {
  ...
  if (user_cmdline != NULL) {
    ...
    *usersegment = global_usersegment = Sequence_read_unlimited(&nextchar,stdin);
    if ((min_matches = Sequence_fulllength(*usersegment)/2) > MIN_MATCHES) {
      min_matches = MIN_MATCHES;
    }
  ...
  if (user_cmdline != NULL) {
    ...
    *nread = 1;
      ...
      inbuffer = Inbuffer_new(nextchar,input,read_files_command,files,nfiles,inbuffer_nspaces);
...
main (int argc, char *argv[]) {
  ...
  if (dbroot != NULL) {
    ...
    genomelength = Univ_IIT_genomelength(chromosome_iit,/*with_circular_alias*/true);
  }
  ...
  if (user_pairalign_p == true) {
    ...
    /* Create genomecomp_blocks and genomebits_blocks for each usersegment */
  ...
  Stage3_setup(/*splicingp*/novelsplicingp == true || knownsplicingp == true,novelsplicingp,
	       ...
	       /*genome_totallength*/genomelength);

  Splicetrie_setup(splicesites,splicefrags_ref,splicefrags_alt,
		   trieoffsets_obs,triecontents_obs,trieoffsets_max,triecontents_max,
		   genomebits,genomebits_alt,/*snpp*/false,amb_closest_p,
		   /*amb_clip_p*/true,/*min_shortend*/2);
