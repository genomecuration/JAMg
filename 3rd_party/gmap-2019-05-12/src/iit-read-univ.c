...
/* chrhigh is one past the highest position in the chromosome */
void
Univ_IIT_interval_bounds_linear (Univcoord_T *low, Univcoord_T *high, Chrpos_T *length, T this,
				 int index) {
  Univinterval_T interval;
...
/* divint_crosstable maps from chromosome_iit chrnums to iit divints */
int *
...
#if 0
  for (chrnum = 0; chrnum < nchromosomes; chrnum++) {
    printf("Genomic chrnum %d => IIT chrnum %d\n",chrnum+1,crosstable[chrnum+1]);
  }
#endif
...
/* chrnum_crosstable maps from iit divints to chromosome_iit chrnums */
int *
Univ_IIT_chrnum_crosstable (T chromosome_iit, IIT_T iit) {
  ...
  ndivs = IIT_ndivs(iit);
  crosstable = (int *) CALLOC(ndivs+1,sizeof(int));

  for (divint = 1; divint < ndivs; divint++) {
    chr = IIT_divstring(iit,divint);

    /* upon lookup, chrnum is 1-based, so we need to store in chrnum+1 */
    crosstable[divint] = Univ_IIT_find_one(chromosome_iit,chr);
  }

#if 0
  for (divint = 0; divint < ndivs; divint++) {
    printf("IIT divint %d => IIT chrnum %d\n",divint+1,crosstable[divint+1]);
  }
#endif
...
Univ_IIT_annotation (char **restofheader, T this, int index, bool *alloc_header_p) {
  ...
  if (annotation[0] == '\0') {
    ...
    *restofheader = (char *) MALLOC((1+len+1)*sizeof(char));
    *restofheader[0] = ' ';
    strncpy(&((*restofheader)[1]),annotation,len);
    (*restofheader)[1+len] = '\0';
...
/* Assumes we have a running value for *chrhigh and that successive calls have increasing values for left */
Univcoord_T
Univ_IIT_update_chrnum (int *chrnum, Univcoord_T *chroffset, Univcoord_T chrhigh, Chrpos_T *chrlength,
		     T this, Univcoord_T left, int querylength, int circular_typeint) {
  int start_chrnum, end_chrnum, test_chrnum;
  Univcoord_T test_chroffset, test_chrhigh;
  Chrpos_T most_inbounds, inbounds, test_chrlength;

  if (left >= chrhigh) {
    *chrnum = Univ_IIT_get_one(this,left,left);
    Univ_IIT_interval_bounds(&(*chroffset),&chrhigh,&(*chrlength),this,*chrnum,circular_typeint);
    debug4(printf("Updating chrnum to be %d\n",*chrnum));
  }

  if ((Univcoord_T) (left + querylength) >= chrhigh) {
    /* Straddles two or more chromosomes */
    /* Test first chromosome */
    start_chrnum = *chrnum;
    most_inbounds = chrhigh - left;
    debug4(printf("Straddle.  First chromosome inbounds: %u.  ",most_inbounds));

    
    /* Test middle chromosomes */
    end_chrnum = Univ_IIT_get_one(this,left + querylength,left + querylength);
    for (test_chrnum = start_chrnum + 1; test_chrnum < end_chrnum; test_chrnum++) {
      Univ_IIT_interval_bounds(&test_chroffset,&test_chrhigh,&test_chrlength,this,test_chrnum,circular_typeint);
      debug4(printf("Next inbounds: %u.  ",test_chrhigh - test_chroffset));
      if ((inbounds = test_chrhigh - test_chroffset) > most_inbounds) {
	*chrnum = test_chrnum; *chroffset = test_chroffset; chrhigh = test_chrhigh, *chrlength = test_chrlength;
	most_inbounds = inbounds;
      }
    }

    /* Test last chromosome */
    Univ_IIT_interval_bounds(&test_chroffset,&test_chrhigh,&test_chrlength,this,end_chrnum,circular_typeint);
    debug4(printf("Last inbounds: %u\n",(left + querylength) - test_chroffset));
    if ((/*inbounds = */(left + querylength) - test_chroffset) > most_inbounds) {
      *chrnum = end_chrnum; *chroffset = test_chroffset; chrhigh = test_chrhigh, *chrlength = test_chrlength;
      /* most_inbounds = inbounds; */
    }
  }

  debug4(printf("Returning chrnum %d\n",*chrnum));
  return chrhigh;
}
...
Univ_IIT_get_chrnum (Univcoord_T *chroffset, Univcoord_T *chrhigh, Chrpos_T *chrlength,
		     T this, Univcoord_T left, int querylength, int circular_typeint) {
  int chrnum, start_chrnum, end_chrnum, test_chrnum;
  Univcoord_T test_chroffset, test_chrhigh;
  Chrpos_T most_inbounds, inbounds, test_chrlength;

  chrnum = Univ_IIT_get_one(this,left,left);
  Univ_IIT_interval_bounds(&(*chroffset),&(*chrhigh),&(*chrlength),this,chrnum,circular_typeint);
  debug4(printf("Setting chrnum to be %d\n",chrnum));
