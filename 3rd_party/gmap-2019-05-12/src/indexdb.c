static char rcsid[] = "$Id: indexdb.c 218168 2019-01-17 06:29:38Z twu $";
#ifdef HAVE_CONFIG_H
#include <config.h>
#endif
#ifndef HAVE_MEMCPY
...
#endif
      if ((filesize = Access_filesize(filenames->positions_filename)) != end0 * sizeof(UINT4)) {
	fprintf(stderr,"Something is wrong with the genomic index: expected file size for %s is %zu, but observed %zu.\n",
		filenames->positions_filename,(size_t) (end0*sizeof(UINT4)),filesize);
	abort();
      }
#endif	/* PMAP */
#endif	/* Sanity check */
...
#ifdef HAVE_MMAP
      if (multiple_sequences_p == false && preload_shared_memory_p == false && unload_shared_memory_p == false) {
...
Indexdb_read_inplace (int *nentries,
  ...
  *nentries = end0 - ptr0;
  ...
  if (*nentries == 0) {
    return NULL;
#ifdef LARGE_GENOMES
  } else if (this->positions_high_access == FILEIO || this->positions_access == FILEIO) {
    abort();
#else
    ...
    debug0(
	   ...
	   for (ptr = ptr0; ptr < end0; ptr++) {
	     printf(" %d %u",this->positions_high[ptr],this->positions[ptr]);
	   }
	   printf("\n");
	   );
...
Indexdb_count_with_diagterm (Positionsptr_T *ptr0, T this, Oligospace_T oligo, int diagterm) {
  ...
  *ptr0 = Bitpack64_read_two(&end0,oligo,this->offsetsmeta,this->offsetsstrm);
  ...
  debug0(printf("Indexdb_count_with_diagterm: oligo = %06llX, offset pointers are %u and %u, diagterm %d\n",
  ...
  if ((nentries = end0 - *ptr0) == 0) {
    ...
    /* Skip any positions that extend past genomicpos 0 (for GSNAP, we have negative diagterms) */
#ifdef LARGE_GENOMES
...
#elif !defined(GSNAP)
/* GMAP reads instead of using pointers */

#elif defined(LARGE_GENOMES)
int
Indexdb_largeptr_with_diagterm (unsigned char **positions_high, UINT4 **positions,
  ...
  debug0(printf("Indexdb_largeptr_with_diagterm: oligo = %06llX, offset pointers are %llu and %llu, diagterm %d\n",
		oligo,ptr0,end0,diagterm));
  ...
  if ((nentries = end0 - ptr0) == 0) {
    ...
    *positions = (UINT4 *) NULL;
    ...
    p = &(this->positions_high[ptr0]);
    q = &(this->positions[ptr0]);
    
    r = &(this->positions_high[end0]);
    while (p < r && ((Univcoord_T) *p << 32) + *q < (Univcoord_T) -diagterm) {
      p++; q++;
    }
...
Indexdb_ptr_with_diagterm (UINT4 **positions,
  ...
  debug0(printf("Indexdb_ptr_with_diagterm: oligo = %06llX, offset pointers are %u and %u, diagterm %d\n",
		oligo,ptr0,end0,diagterm));
  ...
  if ((nentries = end0 - ptr0) == 0) {
    *positions = (UINT4 *) NULL;
    ...
    p = &(this->positions[ptr0]);

    /* Skip any positions that extend past genomicpos 0 */
    r = &(this->positions[end0]);
    while (p < r && *p < (UINT4) -diagterm) {
      debug0(printf("Skipping %u, which is less than -diagterm %u\n",*p,(UINT4) -diagterm));
      p++;
    }
...
#elif defined(GSNAP)
/* GSNAP uses pointers instead of reading */
...
Indexdb_read_with_diagterm (int *nentries, T this, Oligospace_T oligo, int diagterm) {
  Univcoord_T *positions;
  Positionsptr_T ptr0, end0;
#if defined(LARGE_GENOMES) || defined(DEBUG0)
  int i;
#endif
...
#elif 0
/* GMAP no longer calls this.  Calls Indexdb_read_with_diagterm instead */
/* Analogous to Indexdb_read_with_diagterm */
Univcoord_T *
Indexdb_read_with_diagterm_sizelimit (int *nentries, T this, Oligospace_T oligo, int diagterm,
  ...
  debug0(printf("Indexdb_read_with_diagterm_sizelimit: oligo = %06llX, offset pointers are %u and %u, diagterm %d\n",
		oligo,ptr0,end0,diagterm));
  ...
  if (*nentries > size_threshold) {
    debug0(printf("nentries %d > size_threshold %d\n",*nentries,size_threshold));
    *nentries = 0;
    return (Univcoord_T *) NULL;
