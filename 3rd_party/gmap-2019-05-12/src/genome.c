static char rcsid[] = "$Id: genome.c 218286 2019-01-23 16:46:55Z twu $";
#ifdef HAVE_CONFIG_H
#include <config.h>
#endif
#ifndef HAVE_MEMCPY
  ...
  *nunknowns = 0;
  ...
  if (chromosome_iit == NULL) {
    ...
    /* None provided, perhaps because aligning to user segment */
	     ...
	     (chrnum_right = Univ_IIT_get_one(chromosome_iit,left+length-1,left+length-1))) {
...
Genome_get_segment_blocks_right (char *segment, char *segmentalt, Univcoord_T left, Chrpos_T length, Univcoord_T chrhigh,
  ...
  if (length == 0) {
    ...
    /* All out of bounds */
    for (i = 0; i < length; i++) {
      segment[i] = segmentalt[i] = '*';
    }
    segment[length] = segmentalt[length] = '\0';
    return;
