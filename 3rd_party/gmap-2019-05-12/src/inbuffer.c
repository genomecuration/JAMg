static char rcsid[] = "$Id: inbuffer.c 214797 2018-04-21 01:00:48Z twu $";
#ifdef HAVE_CONFIG_H
#include <config.h>
#endif
...
Inbuffer_read (Sequence_T *pairalign_segment, T this, bool skipp) {
  ...
  debug(printf("Calling Inbuffer_read\n"));

  queryseq = Sequence_read_multifile(&this->nextchar,&this->input,this->read_files_command,&this->files,&this->nfiles);
  if (skipp == true) {
...
fill_buffer_master (T this) {
  ...
  while (nread < this->nspaces &&
	 (queryseq1 = Shortread_read_filecontents(&this->nextchar,&queryseq2,
						  ...
						  workers_comm,
#endif
						  this->read_files_command,&this->files,&this->nfiles,
...
#endif
				     this->read_files_command,&this->files,&this->nfiles,skipp = (this->inputid % part_interval != part_modulus))) != NULL) {
    if (skipp) {
#if 0
      /* Shortread procedures won't allocate in this situation */
      Shortread_free(&queryseq1);
...
Inbuffer_first_request (T this) {
  ...
  debug(printf("Calling Inbuffer_first_request\n"));
