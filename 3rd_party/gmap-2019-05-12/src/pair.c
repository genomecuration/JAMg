static char rcsid[] = "$Id: pair.c 218286 2019-01-23 16:46:55Z twu $";
#ifdef HAVE_CONFIG_H
#include <config.h>
#endif
#ifndef HAVE_MEMCPY
...
donor_score (Univcoord_T genomicpos, Univcoord_T chroffset, bool revcomp, Genome_T genome,
  ...
  if (revcomp == false) {
    if ((genome_blocks = Genome_blocks(genome)) != NULL) {
      ...
      return Maxent_hr_donor_prob(genomicpos + 1,chroffset);
    } else {
...
acceptor_score (Univcoord_T genomicpos, Univcoord_T chroffset, bool revcomp, Genome_T genome,
  ...
  if (revcomp == false) {
    ...
    if ((genome_blocks = Genome_blocks(genome)) != NULL) {
      /* Add 1 to get from exon end to intron start */
      return Maxent_hr_antiacceptor_prob(genomicpos + 1,chroffset);
    } else {
...
/* Used by Pair_print_compressed_old */
static void
...
splice_site_probs (double *sense_prob, double *antisense_prob,
  ...
  if (prev_splicesitep == true) {
    if (watsonp == true) {
      ...
      *sense_prob += Maxent_hr_acceptor_prob(chroffset+exon_genomestart-1,chroffset);
      *antisense_prob += Maxent_hr_antidonor_prob(chroffset+exon_genomestart-1,chroffset);
    } else {
  ...
  if (splicesitep == true) {
    if (watsonp == true) {
      ...
      *sense_prob += Maxent_hr_antidonor_prob(chroffset+exon_genomeend-1,chroffset);
      *antisense_prob += Maxent_hr_acceptor_prob(chroffset+exon_genomeend-1,chroffset);
    }
  }
