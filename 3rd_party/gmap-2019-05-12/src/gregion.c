static char rcsid[] = "$Id: gregion.c 218147 2019-01-16 21:28:41Z twu $";
#ifdef HAVE_CONFIG_H
#include <config.h>
#endif
...
static int
support_cmp (const void *x, const void *y) {
  T a = * (T *) x;
  T b = * (T *) y;

  if (a->support > b->support) {
    /* Larger support should beat smaller support */
    return -1;
  } else if (a->support < b->support) {
    return +1;

  } else if (a->genomiclength < b->genomiclength) {
    /* For the same support, prefer more compact regions */
    return -1;
  } else if (a->genomiclength > b->genomiclength) {
    return +1;
  } else {
...
gregion_overlap_p (T x, T y) {
  ...
  if (x->plusp != y->plusp) {
    debug(printf("Case 0 => different strands\n"));
    ...
    if (y_genomicstart > x_genomicend || x_genomicstart > y_genomicend) {
      debug(printf("Case 0: x %u..%u, y %u..%u => no overlap\n",
	...
	debug(printf("Case 1: x %u..%u, y %u..%u",x_genomicstart,x_genomicend,y_genomicstart,y_genomicend));
	...
	*/
	...
	if (plusp == true && (y->querystart >= x->querystart || y->queryend >= x->queryend)) {
	  debug(printf(" => query coords not consistent: x %d..%d, y %d..%d\n",
		       x->querystart,x->queryend,y->querystart,y->queryend));
	  return false;

	} else if (plusp == false && (y->querystart <= x->querystart || y->queryend <= x->queryend)) {
	  debug(printf(" => query coords not consistent: x %d..%d, y %d..%d\n",
		       x->querystart,x->queryend,y->querystart,y->queryend));
	  return false;
      ...
      } else if (y_genomicend > x_genomicend) {
	debug(printf("Case 2: x %u..%u, y %u..%u",x_genomicstart,x_genomicend,y_genomicstart,y_genomicend));
	...
	*/
	...
	/* Note: Like case 4, both branches are true for plus and minus */
	if (plusp == true && (y->querystart >= x->querystart || y->queryend <= x->queryend)) {
	  debug(printf(" => query coords not consistent: x %d..%d, y %d..%d\n",
		       x->querystart,x->queryend,y->querystart,y->queryend));
	  return false;
	...
	/* y_genomicend == x_genomicend */
	debug(printf("Case 3: x %u..%u, y %u..%u",x_genomicstart,x_genomicend,y_genomicstart,y_genomicend));
	...
	*/
	...
	if (plusp == true && (y->querystart >= x->querystart || y->queryend != x->queryend)) {
      ...
      if (y_genomicend < x_genomicend) {
	debug(printf("Case 4: x %u..%u, y %u..%u\n",x_genomicstart,x_genomicend,y_genomicstart,y_genomicend));
	...
	*/
	...
	/* Note: Like case 2, both branches are true for plus and minus */
	if (plusp == true && (y->querystart <= x->querystart || y->queryend >= x->queryend)) {
	  debug(printf(" => query coords not consistent: x %d..%d, y %d..%d\n",
		       x->querystart,x->queryend,y->querystart,y->queryend));
	  return false;
	...
	debug(printf("Case 5: x %u..%u, y %u..%u\n",x_genomicstart,x_genomicend,y_genomicstart,y_genomicend));
	...
	*/
	...
	/* y_genomicend == x_genomicend */
	debug(printf("Case 6: x %u..%u, y %u..%u\n",x_genomicstart,x_genomicend,y_genomicstart,y_genomicend));
	...
	*/
	...
	if (plusp == true && (y->querystart <= x->querystart || y->queryend != x->queryend)) {
      ...
      /* y_genomicstart == x_genomicstart */

      if (y_genomicend < x_genomicend) {
	debug(printf("Case 7: x %u..%u, y %u..%u",x_genomicstart,x_genomicend,y_genomicstart,y_genomicend));
	...
	*/
	...
	debug(printf("Case 8: x %u..%u, y %u..%u",x_genomicstart,x_genomicend,y_genomicstart,y_genomicend));
	...
	*/
	...
	/* y_genomicend == x_genomicend */
	debug(printf("Case 9: x %u..%u, y %u..%u",x_genomicstart,x_genomicend,y_genomicstart,y_genomicend));
...
Gregion_filter_unique (List_T gregionlist) {
  ...
  qsort(array,n,sizeof(T),support_cmp);

  debug(
	for (i = 0; i < n; i++) {
	  gregion = array[i];
	  printf("  After sorting by support %d: %d..%d #%d:%u-%u (plusp = %d)\n",
		 i,gregion->querystart,gregion->queryend,gregion->chrnum,gregion->chrstart,gregion->chrend,gregion->plusp);
	}
	);
  ...
  for (i = 0; i < n; i++) {
    ...
    for (j = i+1; j < n; j++) {
      y = array[j];
#ifdef DEBUG
      printf("Comparing these regions %d and %d:\n",i,j);
      printf("   ");
      Gregion_print(x);
      printf("   ");
      Gregion_print(y);
	...
	debug(printf(" => Found overlap.  Eliminating %d\n\n",j));
	eliminate[j] = true;
      }
    }
  }
...
Gregion_filter_clean (List_T gregionlist, int nchrs) {
  ...
  for (p = gregionlist; p != NULL; p = List_next(p)) {
    ...
    if (low_basetables[gregion->chrnum] == NULL) {
      low_basetables[gregion->chrnum] = Uinttable_new(n,/*save_contents_p*/false);
      high_basetables[gregion->chrnum] = Uinttable_new(n,/*save_contents_p*/false);
    }
  ...
  for (p = gregionlist; p != NULL; p = List_next(p)) {
    ...
    if (low_basetables[gregion->chrnum] == NULL) {
      Uinttable_free(&(low_basetables[gregion->chrnum]));
      Uinttable_free(&(high_basetables[gregion->chrnum]));
    }
  }
