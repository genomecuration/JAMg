static char rcsid[] = "$Id: pairpool.c 218147 2019-01-16 21:28:41Z twu $";
#ifdef HAVE_CONFIG_H
#include <config.h>
#endif
...
#if 0
/* Problem: Removes gaps.  Better to use clean_path_end3_chromosomal_bounds instead */
/* Note: Do not provide actual chrlength, but rather chrhigh - chroffset, so circular chromosomes work */
List_T
Pairpool_copy_bounded (List_T source, T this, Chrpos_T chrlength) {
  List_T dest = NULL;
  Pair_T pair;

  while (source != NULL) {
    pair = (Pair_T) source->first;
    if (pair->genomepos >= chrlength) {
      /* Skip */
    } else {
      dest = Pairpool_push_copy(dest,this,pair);
    }
    source = source->rest;
  }
  return List_reverse(dest);
...
Pairpool_add_genomeskip (bool *add_dashes_p, List_T pairs, int r, int c, int dist,
  ...
  if (*add_dashes_p == true) {
    ...
    debug(printf("Large gap %c%c..%c%c\n",left1,left2,right2,right1));
#ifndef NOGAPHOLDER
