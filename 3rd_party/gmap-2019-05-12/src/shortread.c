static char rcsid[] = "$Id: shortread.c 218147 2019-01-16 21:28:41Z twu $";
#ifdef HAVE_CONFIG_H
#include <config.h>
#endif
#ifndef HAVE_MEMCPY
...
#if 0
unsigned long long **
Shortread_input_divide (bool *fastq_format_p, char *read_files_command, char **files, int nfiles, int naliquots) {
  ...
  while (filei < nfiles) {
    ...
    if ((input1 = Fopen_read_text(read_files_command,files[filei])) == NULL) {
      exit(9);
    } else {
...
Shortread_chop_primers (T queryseq1, T queryseq2) {
  ...
  if (chop1 > 0) {
    ...
    if (queryseq1->quality != NULL) {
      ...
      queryseq1->chop_quality[chop1] = '\0';
...
Shortread_new (char *acc, char *restofheader, bool filterp,
	       ...
	       int barcode_length, int endtrim_length, bool invertp,
	...
	if (*nextchar == '+') {
	  ...
	  *nchars1 += 1;
	  ...
	  queryseq1 = Shortread_new(acc,restofheader,filterp,Read1,long_read_1,fulllength1,
				    ...
				    barcode_length,endtrim_length,invert_first_p,/*copy_acc_p*/false,skipp);
	  (*queryseq2) = Shortread_new(/*acc*/NULL,/*restofheader*/NULL,filterp,Read2,long_read_2,fulllength2,
				       /*quality*/NULL,/*long_quality*/NULL,/*quality_length*/0,
				       barcode_length,endtrim_length,invert_second_p,/*copy_acc_p*/false,skipp);
	}
...
#endif
			 char *read_files_command, char ***files, int *nfiles, bool skipp) {
  T queryseq1;
  int nextchar2 = '\0';		/* Can be anything but EOF */
  char *acc, *restofheader, *acc2, *restofheader2;
  char *long_read_1, *long_read_2, *long_quality;
...
#ifdef USE_MPI_FILE_INPUT
	if (read_files_command != NULL) {
	  fprintf(stderr,"--read-files-command not implemented yet for MPI\n");
	  exit(9);
	} else if (read_files_command == NULL && (*input1 = MPI_fopen((*files)[0],workers_comm)) == NULL) {
...
Shortread_read (int *nextchar, int *nchars1, int *nchars2, T *queryseq2,
  ...
  if (fastq_format_p) {
    ...
    queryseq1 = Shortread_read_fastq_text(&(*nextchar),&(*nchars1),&(*nchars2),&(*queryseq2),
					  &(*input1),&(*input2),read_files_command,&(*files),&(*nfiles),skipp);
...
Shortread_read_filecontents (int *nextchar, T *queryseq2,
  ...
  if (fastq_format_p) {
    return read_fastq_filecontents(&(*nextchar),&(*queryseq2),&(*filecontents1),&(*filecontents2),
				   ...
				   workers_comm,
#endif
				   read_files_command,&(*files),&(*nfiles),skipp);
