static char rcsid[] = "$Id: parserange.c 216944 2018-10-10 07:14:00Z twu $";
#ifdef HAVE_CONFIG_H
#include <config.h>
#endif
...
convert_to_chrpos (Chrpos_T *chrpos, char *genomesubdir, char *fileroot, Univcoord_T position) {
  ...
  chromosome = Univ_IIT_string_from_position(&(*chrpos),position-1,chromosome_iit);
...
Parserange_query (char **divstring, Univcoord_T *coordstart, Univcoord_T *coordend, bool *revcomp,
  ...
  *revcomp = false;
  ...
  if ((coords = rindex(query,':')) != NULL) {
    *divstring = (char *) MALLOC((coords-query+1)*sizeof(char));
    strncpy(*divstring,query,(coords-query)*sizeof(char));
    (*divstring)[coords-query] = '\0';
    coords = &(coords[1]);	/* Skip ":" */
...
bool
Parserange_universal (char **div, bool *revcomp, bool *whole_chromosome_p,
  ...
  *whole_chromosome_p = false;
  ...
  if ((coords = rindex(query,':')) != NULL) {
    ...
    *div = (char *) MALLOC((coords-query+1)*sizeof(char));
    strncpy(*div,query,(coords-query)*sizeof(char));
    (*div)[coords-query] = '\0';
    coords = &(coords[1]);	/* Skip ":" */
