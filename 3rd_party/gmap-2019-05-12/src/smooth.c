static char rcsid[] = "$Id: smooth.c 218183 2019-01-17 13:03:20Z twu $";
#ifdef HAVE_CONFIG_H
#include <config.h>
#endif
...
static void
get_exonmatches (int *total_matches, int *total_denominator, int **exonmatches,
...
static void
find_internal_bads_by_prob (bool *deletep, int *exonstatus, int *exonmatches, int nexons,
...
Smooth_pairs_by_intronprobs (bool *badp, List_T pairs, Pairpool_T pairpool) {
  int *exonstatus;
  int *exonmatches, nexons, nintrons;
  ...
  *badp = false;
  ...
  if (pairs != NULL) {
    ...
    if (nintrons > 0) {
      get_exonmatches(&total_matches,&total_denominator,&exonmatches,&nexons,pairs);
      ...
      debug(printf("\nFind internal bads\n"));
      exonstatus = (int *) MALLOCA(nexons * sizeof(int));
      find_internal_bads_by_prob(&(*badp),exonstatus,exonmatches,nexons,
