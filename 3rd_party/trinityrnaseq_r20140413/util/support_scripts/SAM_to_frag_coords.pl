...
my $sort_exec = "$FindBin::Bin/../../trinity-plugins/coreutils/bin/sort";
my $sam_file;
my $full_flag = 0;
...
sub extract_frag_coords {
    ...
    unless (-s "$read_coords_file.sort_by_readname" ){
        ...
        my $cmd = "$sort_exec --parallel=$CPU -S$sort_buffer -T . -k1,1 -k2,2 -k4,4n $read_coords_file > $read_coords_file.sort_by_readname";
        &process_cmd($cmd);
    ...
    my $cmd = "$sort_exec --parallel=$CPU -S$sort_buffer -T . -k1,1 -k3,3n $pair_frag_coords_file > $pair_frag_coords_file.coord_sorted";
