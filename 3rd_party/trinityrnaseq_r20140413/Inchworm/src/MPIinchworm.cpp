...
unsigned int get_node_for_central_kmer (kmer_int_type_t central_kmer);
...
void run_MPI_master_all_completion_check();
    ...
    */
    ...
    #pragma omp parallel sections num_threads(2)
    {
        ...
        #pragma omp section 
        {
            ...
            long this_mpi_section_start = THIS_MPI_NODE_ID * mpi_section_length;
            long this_mpi_section_end = this_mpi_section_start + mpi_section_length;
		    ...
		    if (NUM_MPI_NODES > 1) {
		      node_for_central_kmer = get_node_for_central_kmer(central_kmer);
            ...
            if (THIS_MPI_NODE_ID == 0) {
                ...
                cerr << "Master node running MPI_completion check.\n";
                
                run_MPI_master_all_completion_check();
...
vector<kmer_int_type_t> inchworm (KmerCounter& kcounter, char direction, kmer_int_type_t kmer, Kmer_visitor& visitor, float min_connectivity) {
    ...
    while (true) {
        ...
        unsigned int node_for_central_kmer = get_node_for_central_kmer(next_kmer_core);
...
unsigned int get_node_for_central_kmer (kmer_int_type_t central_kmer) {
    ...
    return(central_kmer % NUM_MPI_NODES);
...
void zap_kmers(KmerCounter& kcounter, vector<kmer_int_type_t>& kmer_path) {
    ...
    for (unsigned int i = 0; i < kmer_path.size(); i++) {
        ...
        unsigned int MPI_node_id = get_node_for_central_kmer(central_kmer);
...
void run_MPI_master_all_completion_check() {
    ...
    int first_node = 0;
    if (MASTER_SLAVE_MODE)
        first_node = 1; // master is not running a kmer server, so no need to send a msg to it.  probably doesnt matter though.
         ...
         #pragma omp section 
        {
            if (is_assembly_node) {
                ...
                for (unsigned int i = 0; i < kmers.size(); i++) {
                    ...
                    if (THIS_MPI_NODE_ID == 0)
                        cerr << "Kmer is: " << decode_kmer_from_intval(kmer, kmer_length) << endl;
...
void run_MPI_kmer_server(KmerCounter& kcounter) {
    ...
    cerr << "** Launching run_MPI_kmer_server() for node: " << THIS_MPI_NODE_ID << endl;
  
    stringstream filenamebuilder;
    filenamebuilder << "mpi_kmer_server.mpi_" << THIS_MPI_NODE_ID << ".log";
