...
main: {
    ...
    if ($method eq "GLM") {
        ...
        ## some fiddling to meet A vs. B requirements below, given the looping scheme.
        if ($reference_sample) {
            

            @A_samples = ($reference_sample, "NULL"); # need one more for loop requirements below, last one not used.
            @B_samples = grep { $_ ne $reference_sample } @B_samples;
                        
            print "Comparing reference: $reference_sample to others:\n" . Dumper(\@B_samples); 
            
            unshift (@B_samples, "NULL"); # first one not used in loops below
