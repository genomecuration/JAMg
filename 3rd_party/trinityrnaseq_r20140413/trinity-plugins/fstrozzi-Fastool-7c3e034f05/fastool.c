...
int main(int argc, char *argv[])
{
	...
	for(int i = 1; i < argc; ++i)
	{
		if(strcmp(argv[i],"--rev") == 0) reverse_complement = 1;
			...
			if (i+1 == argc) {
				fprintf(stderr, "String to append is missing!\n");
				exit(1);
			}
			else {
			...
			if (illumina_trinity && string_to_append) {
				fprintf(stderr, "You are using both --append and --illumina-trinity options. You can only provide one or the other.\n");
				exit(1);
			}
			read_from_file = 1;
			gzFile fp;
