...
sub _parse_file{
  ...
  foreach my $tx (@all_txs){
	...
	if($cds->length < 3){
	  ...
	  for(my $i = 0; $i <= $#$starts;$i++){
	    if($i == $#$starts){
	      unless($i <= $#$cds && $$cds[$i]->stop >= $$starts[$i]->stop){
		$error = 1;
	      }
	    }
	    else{
	  ...
	  if($error){
	    if($errors[47] < $max_error_count){
	      print $warnings "Start codon annotated in intron or outside of cds ".
          "region on transcript_id = \"".$tx->id."\".\n";
	    }
	    $errors[47]++;
	    if (_check_errors_against_badlist($gtf,47)) {
	      push @bad_list, $tx->id;
	    ...
	    while($pos < 3){
	      my $end;
          unless($#$cds >= $cds_pos) {
        print $warnings "Not enough CDS for a full start codon ".
  ...
  if($seqname){
    ...
    foreach my $tx (@all_txs){
      ...
      if($seq =~ /^$/){
        print $warnings "No sequence for $tx_id\n";
        next;
      }
      if($tx->strand eq "-"){
        $seq = _rev_comp($seq);
    ...
    $tx->remove_cds($terminal_exon->start)
      if($terminal_exon->stop < $terminal_exon->start);
...
sub _load_no_check{
  ...
  while(my $in_line = <GTF>){
    ...
    #create objects
    my $feature =
      GTF::Feature::new($data[2], $data[3], $data[4], $data[5], $data[7],\%attributes);
    if($feature->type eq "CDS"){
      push @all_cds, $feature;
    }
    elsif($feature->type eq "inter") {
...
sub create_utr_objects_from_exons{
  my ($tx) = @_;
  my $exons = $tx->exons;
  my $start = $tx->coding_start-1;
  my $stop = $tx->coding_stop+1;
  if($start != -1){
    my $type = "5UTR";
    if($tx->strand eq "-"){
      $type = "3UTR";
...
# GTF::Transcript::fix_frame
#   This will fix the frame of a transcript, ignoring previous frame information
#   and recalculating the frame values.

sub fix_frame {
  my($tx) = @_;

  $tx->_update;
  return unless(@{$tx->cds} > 0);
  my $frame = 0;
  if($tx->strand eq '+') {
    for (my $i = 0; $i < @{$tx->cds}; $i++) {
      $tx->cds->[$i]->set_frame($frame);
      # see mblab.wustl.edu/GFF22 for information on this calculation.
      $frame = (3-(($tx->cds->[$i]->length-$frame)%3))%3;
    }
  }
  else {
    for (my $i = @{$tx->cds}-1; $i >= 0; $i--) {
      $tx->cds->[$i]->set_frame($frame);
      # see mblab.wustl.edu/GFF22 for information on this calculation.
      $frame = (3-(($tx->cds->[$i]->length-$frame)%3))%3;
    }
...
# GTF::Transcript::fix_start_codon
#   This will add a start codon to a transcript with a missing or incorrect
#   start codon.  This only goes by the position of the 5'-most CDS, and
#   does not check for a correct ATG sequence.  Takes 1 optional argument.
#
#     replace - (optional) pass 1 to replace the existing start codon 
#                     default: 0 (don't replace)

sub fix_start_codon {
  my ($tx,$replace) = @_;
  $replace ||= 0;
  if(@{$tx->start_codons} > 0) {
    return unless($replace);
    $tx->{Starts} = [];
  }
  $tx->_update;
  if($tx->strand eq '+') {
    for(my ($sc_len, $cur_cds) = (0,0);
        $sc_len < 3 && $cur_cds < @{$tx->cds}; 
        $cur_cds++) {
      my ($start,$stop);
      if($tx->cds->[$cur_cds]->length < 3) {
        $start = $tx->cds->[$cur_cds]->start;
        $stop  = $tx->cds->[$cur_cds]->stop;
      }
      else {
        $start = $tx->cds->[$cur_cds]->start;
        $stop  = $tx->cds->[$cur_cds]->start+2;
      }
      my $feat = GTF::Feature::new("start_codon",$start,$stop,".",".");
      $tx->add_feature($feat);
      $sc_len += $stop-$start+1;
    }
  }
  else {
    for(my ($sc_len, $cur_cds) = (0,-1);
        $sc_len < 3 && $cur_cds > -1*@{$tx->cds}; 
        $cur_cds--) {
      my ($start,$stop);
      if($tx->cds->[$cur_cds]->length < 3) {
        $start = $tx->cds->[$cur_cds]->start;
...
# GTF::Transcript::fix_stop_codon
#   This will add a stop codon to a transcript with a missing or incorrect
#   stop codon.  This only goes by the position of the 5'-most 3' UTR exon, and
#   does not check for a correct ATG sequence.  Takes 1 optional argument.
#
#     replace - (optional) pass 1 to replace the existing start codon 
#                     default: 0 (don't replace)
...
sub fix_stop_codon {
  ...
  if($tx->strand eq '+') {
    if(@{$tx->utr3} == 0) {
      return unless(@{$tx->cds} > 0);
      my $trans_stop = $tx->cds->[-1]->stop;
      my $feat = GTF::Feature::new("stop_codon",
        $trans_stop+1,$trans_stop+3,".",".");
      $tx->add_feature($feat);
    }
    else {
      for(my ($sc_len, $cur_utr) = (0,0);
          $sc_len < 3 && $cur_utr > -1*@{$tx->utr3}; 
          $cur_utr++) {
        my ($start,$stop);
        if($tx->utr3->[$cur_utr]->length < 3) {
          $start = $tx->utr3->[$cur_utr]->start;
          $stop  = $tx->utr3->[$cur_utr]->stop;
        }
        else {
          $start = $tx->utr3->[$cur_utr]->start;
          $stop  = $tx->utr3->[$cur_utr]->start+2;
        }
        my $feat = GTF::Feature::new("stop_codon",$start,$stop,".",".");
        $tx->add_feature($feat);
        $sc_len += $stop-$start+1;
      }
    }
    ...
    if(@{$tx->utr3} == 0) {
      return unless(@{$tx->cds} > 0);
      my $trans_stop = $tx->cds->[0]->start;
      my $feat = GTF::Feature::new("stop_codon",
        $trans_stop-3,$trans_stop-1,".",".");
      $tx->add_feature($feat);
    }
    else {
      for(my ($sc_len, $cur_utr) = (0,-1);
          $sc_len < 3 && $cur_utr < @{$tx->utr3}; 
          $cur_utr--) {
        my ($start,$stop);
        if($tx->utr3->[$cur_utr]->length < 3) {
          $start = $tx->utr3->[$cur_utr]->start;
...
# GTF::Transcript::fix_utr3_stop_codon_overlap
#   This function fixes a 3UTR feature that overlaps the stop codon.
#   This will not fix any other overlaps it may have.

sub fix_utr3_stop_codon_overlap {
  my ($tx) = @_;

  return unless(@{$tx->stop_codons} > 0 && @{$tx->utr3} > 0);
  $tx->_update;
  my $stop_stop = $tx->stop_codons->[-1]->stop;
  my $utr3s = $tx->utr3;
  for (my $i = 0; $i < @$utr3s; $i++) {
    my $utr3 = $utr3s->[$i];
    next unless($utr3->start <= $stop_stop);
    if($utr3->stop <= $stop_stop) { #if completely contained, remove utr3
      my $last_utr3 = pop @$utr3s;
      last if($i == @$utr3s);
      $utr3s->[$i] = $last_utr3;
      $i--;
    }
    else {                          # just need to change the start position
      $utr3->set_start($stop_stop+1);
    }
  }
}

# GTF::Transcript::correct_gff_stop_codon
#   GFF has a different standard to include the stop codon in the CDS regions.
#   This function corrects the last exon and (if present) the position of the
#   stop codon ASSUMING that it is incorrect (i.e. without checking the
#   sequence.  Takes no arguments.
sub correct_gff_stop_codon {
  my ($tx) = @_;
  $tx->_update;
  if($tx->strand eq "+") {
    if(@{$tx->cds} > 0) {
      $tx->cds->[-1]->set_stop($tx->cds->[-1]->stop-3);
    }
    if(@{$tx->stop_codons} > 0) {
      $tx->stop_codons->[0]->set_start($tx->stop_codons->[0]->start-3);
      $tx->stop_codons->[-1]->set_stop($tx->stop_codons->[0]->stop-3);
    }
  }
  else{
    if(@{$tx->cds} > 0) {
      $tx->cds->[0]->set_start($tx->cds->[0]->start+3);
    }
    if(@{$tx->stop_codons} > 0) {
      $tx->stop_codons->[-1]->set_start($tx->stop_codons->[0]->start+3);
      $tx->stop_codons->[0]->set_stop  ($tx->stop_codons->[0]->stop+3);
    }
  }
}
...
# line of a gtf file.
package GTF::Feature;
# GTF::Feature::new(feature, start, end, score, frame, attributes)
...
#                   '0', '1', '2', or '.'.
#      attributes - (Optional) The additional attributes specific to this 
#                   feature besides gene_id and transcript_id, in the form of 
#                   an href
sub new {
  my $feature = bless {};
  ($feature->{Type}, $feature->{Start}, $feature->{End}, $feature->{Score},
   $feature->{Frame}, $feature->{Attributes}) = @_;
  my $start = $feature->{Start};
  my $stop = $feature->{Stop};
  $feature->{Attributes} = {} unless(defined($feature->{Attributes}));
...
sub copy{
  ...
  my $copy = GTF::Feature::new($feature->type,$feature->start,$feature->stop,
			       $feature->score,$feature->frame,$feature->attributes);
...
# GTF::Feature::attributes()
#   This function returns all attributes in a hash reference, excluding the
...
sub output_gtf{
  ...
  for my $key (keys %{$feature->attributes}) {
    ...
    print $out_handle " $key \"".$feature->attributes->{$key}."\";";
  }
  print $out_handle "\n";
}
