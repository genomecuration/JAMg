...
sub parse_evidence_chains {
    ...
    while (<FILE>) {
        ...
        $attributes =~ /ID=([^; ]+)/;
        my $chainID = $1 or confess "error, no chainID in attributes $attributes of line $_\n";
        
        $attributes =~ /Target=([^; ]+)/;
        my $accession = $1 or confess "Error, no accession (target) in attributes $attributes of line $_\n";
        ...
        my $key = $ev_type . "/" . $accession . "/" . $chainID;
