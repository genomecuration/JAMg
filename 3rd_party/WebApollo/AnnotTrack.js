...
//AP changes start here
    alignSequence: function(family) {
        var selected = this.selectionManager.getSelection();
       this.genericWsCall(selected , family , '/ws/align.ws.php', 'Performing Alignment... ','Alignment');
    },
    blastSequence: function() {
        var selected = this.selectionManager.getSelection();
        this.genericWsCall( selected , null , '/ws/blast.ws.php','Performing BLASTP @ NCBI... ', 'Blast')
    },
    blastLocallySequence: function() {
        var selected = this.selectionManager.getSelection();
        this.genericWsCall( selected , null , '/ws/blast.ws.local.php','Performing BLASTP locally... ', 'Blast')
    },
    signalpSequence: function() {
        var selected = this.selectionManager.getSelection();
        this.genericWsCall( selected , null , '/ws/signalp.ws.php','Checking for signal peptide... ', 'SignalP')
    },
//AP changes end here
...
// AP/TV changes starts here
/**
* variable that helps remember previous input of user.
*/
customAlignmentValue : '',
errorMsg : '',
validateSequence:function( sequence ){
  if (sequence.length == 0) {
    this.errorMsg = 'No sequence entered';
    return false;
  }
  return this.isFasta( sequence );
},
isFasta:function( sequence ){
  // split on >
  var records = sequence.split('>'), record, lines, valid = true, numRecords = sequence.match(/[\>]/g),i;
//check if the first character is >
  if( !sequence.match(/^[\>]/) ){
    this.errorMsg = 'Fasta header is not present';
    return false;
  }

  numRecords = numRecords && numRecords.length || 0;
  // this is due to split fn of js. split fn returns an array with the string when there is no match. this condition should tackle such cases.
  if ( !numRecords || ( ( records.length - 1 ) != numRecords ) ) {
    this.errorMsg = 'Fasta header is not present';
    return false;
  }
  // split on line feed
  for( i in records){
    ...
    if( record ) {
      lines = record.split('\n');
      if ( (lines.length < 2)){
       this.errorMsg = 'Sequence is not in fasta format';
        return false;
      }
      lines.shift();
      // check if it has the right characters.
      if( lines.join('').match(/[^ACDEFGHIKLMNPQRSTVWXY\n-]/gi) ) {
       this.errorMsg = 'The sequence should only contain non redundant IUPAC nucleotide or amino acid codes (except for N/X/-)';
        return false;
      }
    }
  }
...
numberOfSequences:function( sequences ){
  var matches = sequences.match(/[\>]/g);
  return matches && matches.length;
},
isSameLength:function( sequence ){
   // split on >
  var records = sequence.split('>'), record, lines, valid = true, newLength , prevLength, i;
  // split on line feed
  for( i in records){
    record = records [i];
    if( record ) {
      ...
      lines.shift();
      // check if it has the right characters.
      newLength = lines.join('').length;
      prevLength = prevLength || newLength ;
      if ( newLength != prevLength ){
       this.errorMsg = 'Prealigned sequences should have the same length';
        return false;
      }
...
formatFasta:function( sequence ){
  // split on >
  var records = sequence.split('>'), record, lines, newSequence = '', newRecords = [], i;
  // split on line feed
  for( i in records){
    record = records [i];
    if( record ) {
      ...
      newSequence += '>'+lines[0] +'\n';
      lines.shift();
      newSequence += this.chunk_split( lines.join('') , 80 , '\n' );
      newRecords.push( newSequence );
    }
  }
...
chunk_split:function (body, chunklen, end) {
  ...
  return body.match(new RegExp(".{1," + chunklen + "}", "g")).join(end);

},
customAlign:function(){

               var content = dojo.create('div' , { className: "search_sequence" }),
                 sequenceLabel = dojo.create( "div", { className: "search_sequence_label", innerHTML: "FASTA formatted sequence or alignment" }, content ),
                 textArea = dojo.create("textarea", { className: "search_sequence_input", value: this.customAlignmentValue, style: "height:300px;width=400px" }, content),
                 divRadio = dojo.create("div", { className: "search_all_ref_seqs_area" }, content),
                 radio_unaligned = dojo.create('input' , { type:'radio',name:'alignment', value:'unaligned',defaultChecked:true }, divRadio ),
                 label_radio_unaligned = dojo.create('label' , { className: "search_sequence_label" , innerHTML:"Raw unaligned sequences to de-novo align with gene model" + "<br/>"}, divRadio),
                 radio_prealigned = dojo.create('input' , { type:'radio',name:'alignment',value:'prealigned'}, divRadio ),
                 label_radio_prealigned = dojo.create('label' , { className: "search_sequence_label" , innerHTML:"Existing alignment to profile align the gene model"}, divRadio),
                 sequenceButton = dojo.create("button", { innerHTML: "Search" }, content),
                 track = this,dialog;
               var submit = function(){
                 var custom_profile = dojo.attr( textArea , "value").toUpperCase(), unaligned , prealigned;
                 var ok = true;
                 unaligned = dojo.attr( radio_unaligned , "checked") ? 1 : 0;
                 prealigned = dojo.attr( radio_prealigned , "checked") ? 1 : 0;
                 ok = track.validateSequence ( custom_profile );
                 custom_profile = track.formatFasta( custom_profile );
                 if( prealigned ){
                   if ( track.numberOfSequences( custom_profile ) < 2 ) {
                     track.errorMsg = 'If providing an alignment (prealigned) then we need more than one sequence!';
                     ok = false;
                   }
                   if( !track.isSameLength( custom_profile ) ){
                     track.errorMsg = 'If providing an alignment (prealigned) then sequences must be the same length!';
                     ok = false;
                   }
                 }
                 if( ok ) {
                   var alignment_result = new dijit.Dialog({
                     id: "alignment_custom",
                     title: "Alignment",
                     class: "nonModal",
                     content: "Performing alignment... " + '<img src="img/spinner.gif">',
                   });
                   // format fasta
                   alignment_result.connect(alignment_result, "hide", function(e){
                     dijit.byId("alignment_custom").destroy();
                           });
                   var selected = track.selectionManager.getSelection()
                   var features = track.getFeatures( selected );
                   track.customAlignmentValue = custom_profile;
                   dojo.xhrPost( {
                      content: { custom_profile: custom_profile,unaligned:unaligned,prealigned: prealigned , sequence:features },
                      url: "/ws/customalign.ws.php",
                      handleAs: "text",
                      timeout: 8000 * 1000, // Time in milliseconds
                      load: function(response, ioArgs) {
                       alignment_result.set("content",'<div style="width: 1000px; height: 500px; overflow: auto;">' + response + '</div>');
                       return response;
                     },
                     error: function( response , ioArgs){
...
//                     track.handleError(response);
                   ...
                   alignment_result.show();
                   track.popupDialog.hide();
                 } else {
                   alert( 'Sequence validation failed: ' + track.errorMsg );
                 }
               };
               dojo.connect( sequenceButton , 'onclick', submit );
               dialog = this.openDialog( 'Provide unaligned sequences or an existing alignment in FASTA format', content );
},
    getFeatures:function( records ){
      var type = "peptide";
      var features = '"features": [', track = this;
      for (var i = 0; i < records.length; ++i)  {
        var record = records[i];
        var annot = record.feature;
        var seltrack = record.track;
      ...
      features += ']';
            ...
            var trackName = track.getUniqueTrackName();
                ...
                postData += ', "type": "' + type + '" }';
                var deferred_sequence = dojo.xhrPost( {
                    postData: postData,
                    url: context_path + "/AnnotationEditorService",
                    handleAs: "json",
                    sync: true,
                    ...
                    load: function(response, ioArgs) {
                       var sequence = '';
                        for (var i = 0; i < response.features.length; ++i) {
                                ...
                                var cvterm = feature.type;
                                var residues = feature.residues;
                                sequence += ">" + feature.uniquename + " (" + cvterm.cv.name + ":" + cvterm.name + ") " + residues.length + " residues [" + type + (flank > 0 ? " +/- " + flank + " bases" : "") + "]\n";
                                var lineLength = 70;
                                for (var j = 0; j < residues.length; j += lineLength) {
                                        sequence += residues.substr(j, lineLength) + "\n";
                                }
                        }
                       return sequence;
                ...
                var sequence = deferred_sequence.results[0];
      return sequence;
    },
/**
* generic function that calls web service with the sequences of the selected track
* parameters
* @records (mandatory) - selected track
* @family (optional)- AP?
* @ws (mandatory)- webservice that should be called
* @loadingMsg (optional)- custom message that should appear on calling webservice
* @title (optional) - title of the dialog box
*/
    genericWsCall: function(records,family, ws , loadingMsg , title) {
        if ( !ws ) {
          this.errorMsg ( 'Webservice not set' );
          return false;
        }
        ...
        var alignment_result = new dijit.Dialog({
            ...
            title: title,
            class: "nonModal",
            content: loadingMsg + '<img src="img/spinner.gif">',
        });
        alignment_result.connect(alignment_result, "hide", function(e){
            dijit.byId("alignment_window").destroy();
        });
        var alignSequence = function(type) {
            var features = '"features": [';
            for (var i = 0; i < records.length; ++i)  {
                var record = records[i];
                var annot = record.feature;
            ...
            var trackName = track.getUniqueTrackName();
               ...
               // send to ws
                var deferred_aln = dojo.xhrPost( {
                   content:  { sequence : sequence,family:family },
                   handleAs: "text",
                    url: ws,
                   timeout: 8000 * 1000,
                   load: function(response, ioArgs) {
                       alignment_result.set("content",'<div style="width: 800px; height: 500px; overflow: auto;">' + response + '</div>');
                       return response;
                   },
                   // The ERROR function will be called in an error case.
                   error: function(response, ioArgs) {
                       console.log("error with align");
                       console.error("HTTP status code: ", ioArgs.xhr.status);
                       return response;
                   }
                });
        ...
        alignSequence("peptide");
       alignment_result.show();
    },
// AP/TV changes ends here
...
//AP changes start here
       ...
       annot_context_menu.addChild(new dijit.MenuSeparator());
       index++;
        var annot_context_menu_webservices = new dijit.Menu();
       index++;

        contextMenuItems["blastp_ncbi"] = index++;
        annot_context_menu_webservices.addChild(new dijit.MenuItem( {
           label: "BLASTP vs NCBI RefSeq",
           onClick: function(event) {
               thisObj.blastSequence();
           }
        } ));

        contextMenuItems["blastp_local"] = index++;
        annot_context_menu_webservices.addChild(new dijit.MenuItem( {
           label: "BLASTP (local) vs UniProt UniRef90",
           onClick: function(event) {
               thisObj.blastLocallySequence();
           }
        } ));

        contextMenuItems["signalp"] = index++;
        annot_context_menu_webservices.addChild(new dijit.MenuItem( {
           label: "Check for signal peptide",
           onClick: function(event) {
               thisObj.signalpSequence();
           }
        } ));

       annot_context_menu_webservices.addChild(new dijit.MenuSeparator());
       index++;

        contextMenuItems["align_custom"] = index++;
        annot_context_menu_webservices.addChild(new dijit.MenuItem( {
           label: "Align with user sequences",
           onClick: function(event) {
               thisObj.customAlign();
           }
        } ));
        ...
        contextMenuItems["align_Bm_CSP"] = index++;
        annot_context_menu_webservices.addChild(new dijit.MenuItem( {
           label: "vs Bombyx CSP",
           onClick: function(event) {
              thisObj.alignSequence('Bmor_CSP');
           }
        } ));

        contextMenuItems["align_Bm_GR"] = index++;
        annot_context_menu_webservices.addChild(new dijit.MenuItem( {
           label: "vs Bombyx GR",
           onClick: function(event) {
              thisObj.alignSequence('Bmor_GR');
           }
        } ));

        contextMenuItems["align_Bm_IR"] = index++;
        annot_context_menu_webservices.addChild(new dijit.MenuItem( {
           label: "vs Bombyx IR",
           onClick: function(event) {
              thisObj.alignSequence('Bmor_IR');
           }
        } ));

        contextMenuItems["align_Bm_OBP"] = index++;
        annot_context_menu_webservices.addChild(new dijit.MenuItem( {
           label: "vs Bombyx OBP",
           onClick: function(event) {
              thisObj.alignSequence('Bmor_OBP');
           }
        } ));

        contextMenuItems["align_Bm_OR"] = index++;
        annot_context_menu_webservices.addChild(new dijit.MenuItem( {
           label: "vs Bombyx OR",
           onClick: function(event) {
              thisObj.alignSequence('Bmor_OR');
           }
        } ));

        contextMenuItems["align_Bm_SNMP"] = index++;
        annot_context_menu_webservices.addChild(new dijit.MenuItem( {
           label: "vs Bombyx SNMP",
           onClick: function(event) {
              thisObj.alignSequence('Bmor_SNMP');
           }
        } ));


        annot_context_menu.addChild(new dijit.PopupMenuItem({
          label:"Webservices (cf Alexie)",
          popup:annot_context_menu_webservices
        } ));
