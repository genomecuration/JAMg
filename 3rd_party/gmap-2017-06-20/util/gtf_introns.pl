...
if (defined($opt_d)) {
	...
	if (0) {
	    ...
	    $FP = new IO::File("| /data_storage/sysadmin/packages/gmap/JUN17/bin/get-genome -D $opt_D -d $opt_d > get-genome.out");
	} else {
	    $FP = new IO::File("| /data_storage/sysadmin/packages/gmap/JUN17/bin/get-genome -d $opt_d > get-genome.out");
	}
