static char rcsid[] = "$Id: indexdb.c 115433 2013-11-18 18:24:33Z twu $";
#ifdef HAVE_CONFIG_H
#include <config.h>
#endif
#ifndef HAVE_MEMCPY
...
Indexdb_new_genome (Width_T *basesize, Width_T *index1part, Width_T *index1interval,
  ...
  if ((filenames = Indexdb_get_filenames_no_compression(&new->index1part,&new->index1interval,
    ...
    *index1interval = new->index1interval;
	...
	bitpackpages = (UINT4 *) Access_allocated(&bitpackpages_len,&seconds,filenames->pages_filename,sizeof(UINT4));
      } else {
	bitpackpages = (UINT4 *) MALLOC(1*sizeof(UINT4));
	bitpackpages[0] = -1U;
      }
      Bitpack64_read_setup(bitpackpages,new->gammaptrs,new->offsetscomp,new->offsetscomp_blocksize);
#else
      Bitpack64_read_setup(/*bitpackpages*/NULL,new->gammaptrs,new->offsetscomp,new->offsetscomp_blocksize);
#endif
      poly_T = ~(~0UL << 2*new->index1part);
...
Indexdb_offsets_from_bitpack (char *bitpackptrsfile, char *offsetscompfile, Width_T offsetscomp_basesize
  ...
  if (blocksize == 1) {
    ...
    Bitpack64_read_setup(/*bitpackpages*/NULL,bitpackptrs,offsetscomp,blocksize);
...
Indexdb_offsets_from_bitpack_huge (char *bitpackpagesfile, char *bitpackptrsfile, char *offsetscompfile, Width_T offsetscomp_basesize
  ...
  if (blocksize == 1) {
    ...
    fprintf(stderr,"Expanding bitpackcomp into offsets...");
    Bitpack64_read_setup(bitpackpages,bitpackptrs,offsetscomp,blocksize);
    for (oligoi = 0UL; oligoi < oligospace; oligoi += blocksize) {
      Bitpack64_block_offsets_huge(&(offsets[oligoi]),bitpackptrs,offsetscomp,blocksize,oligoi);
    }
