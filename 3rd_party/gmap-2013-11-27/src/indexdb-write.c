static char rcsid[] = "$Id: indexdb-write.c 116708 2013-11-27 19:43:54Z twu $";
#ifdef HAVE_CONFIG_H
#include <config.h>
#endif
...
static int
compute_q4_diffs_bidir (UINT4 *diffs, Positionsptr_T *offsets) {
  UINT4 packsize;
  int i;
  Positionsptr_T maxdiff = 0, top;
  int firstbit, msb;
...
#ifdef HAVE_64_BIT
static int
compute_q4_diffs_bidir_huge (UINT4 *diffs, Hugepositionsptr_T *offsets) {
  UINT4 packsize;
  int i;
  UINT4 maxdiff = 0, top;
  int firstbit, msb;

  maxdiff |= (diffs[32] = (UINT4) (offsets[64] - offsets[63]));
  maxdiff |= (diffs[33] = (UINT4) (offsets[64] - offsets[62]));
  maxdiff |= (diffs[34] = (UINT4) (offsets[64] - offsets[61]));
  maxdiff |= (diffs[35] = (UINT4) (offsets[64] - offsets[60]));
  for (i = 36; i < 64; i++) {
    maxdiff |= (diffs[i] = (UINT4) (offsets[64+32-(i+1-4)] - offsets[64+32-(i+1)]));
  }
  for (i = 31; i >= 4; i--) {
    maxdiff |= (diffs[i] = (UINT4) (offsets[i+1] - offsets[i+1-4]));
  }
  maxdiff |= (diffs[3] = (UINT4) (offsets[4] - offsets[0]));
  maxdiff |= (diffs[2] = (UINT4) (offsets[3] - offsets[0]));
  maxdiff |= (diffs[1] = (UINT4) (offsets[2] - offsets[0]));
  maxdiff |= (diffs[0] = (UINT4) (offsets[1] - offsets[0]));
...
check_offsets_from_bitpack (char *bitpackptrsfile, char *bitpackcompfile, Positionsptr_T *offsets,
  ...
  Bitpack64_read_setup(/*bitpackpages*/NULL,bitpackptrs,bitpackcomp,/*blocksize*/64);
...
check_offsets_from_bitpack_huge (char *bitpackpagesfile, char *bitpackptrsfile, char *bitpackcompfile,
  ...
  Bitpack64_read_setup(bitpackpages,bitpackptrs,bitpackcomp,/*blocksize*/64);

  for (oligoi = 0UL; oligoi < oligospace; oligoi += blocksize) {
    Bitpack64_block_offsets_huge(offsets64,bitpackptrs,bitpackcomp,/*blocksize*/64,oligoi);
    for (i = 0; i <= 64; i++) {
      if (offsets64[i] != offsets[oligoi+i]) {
#ifdef OLIGOSPACE_NOT_LONG
	fprintf(stderr,"Problem with bitpack at oligo %u+%u = %u: %lu != %lu.  Please inform twu@gene.com\n",
		oligoi,i,oligoi+i,offsets64[i],offsets[oligoi+i]);
#else
	fprintf(stderr,"Problem with bitpack at oligo %lu+%lu = %lu: %lu != %lu.  Please inform twu@gene.com\n",
		oligoi,i,oligoi+i,offsets64[i],offsets[oligoi+i]);
#endif
	exit(9);
      }
...
Indexdb_write_bitpackptrs (char *bitpackptrsfile, char *offsetsfile, Positionsptr_T *offsets,
  ...
  if (blocksize == 1) {
    ...
    bitpackptrs = (UINT4 *) CALLOC((oligospace/blocksize + 1) * METAINFO_SIZE,sizeof(UINT4));
    bitpackptri = 0;
...
#ifdef HAVE_64_BIT
void
Indexdb_write_bitpackptrs_huge (char *bitpackpagesfile, char *bitpackptrsfile, char *offsetsfile,
				Hugepositionsptr_T *offsets, Oligospace_T oligospace, Blocksize_T blocksize) {
  Hugepositionsptr_T currpage, nextpage;
  FILE *offsets_fp;
  FILE *bitpackpages_fp, *bitpackptrs_fp, *offsetscomp_fp;
  UINT4 bitpackpages[25];	/* Allows us to handle up to 100 billion positions */
  UINT4 *bitpackptrs;
  int bitpackptri;
  Oligospace_T oligoi;
  ...
  if (blocksize == 1) {
    ...
    for (oligoi = 0; oligoi < oligospace; oligoi += blocksize) {
      ...
      /* Offset */
      if (offsets[oligoi] >= nextpage) {
	fprintf(stderr,"\nAt oligo %u (block %u), offsets %lu >= nextpage %lu",
		oligoi,oligoi/blocksize,offsets[oligoi],nextpage);
	bitpackpages[pagei++] = oligoi/blocksize;
	currpage = nextpage;
	nextpage += POSITIONS_PAGE;
      }
      bitpackptrs[bitpackptri++] = offsets[oligoi] - currpage;

	
      /* Pack block of 64 diffs */
      packsize = compute_q4_diffs_bidir_huge(diffs,&(offsets[oligoi]));
      offsets_buffer_i = Bitpack64_write_vert(offsetscomp_fp,offsets_buffer,offsets_buffer_size,
					      offsets_buffer_i,diffs,packsize);
    ...
    /* Need to write the final pointers */
    bitpackptrs[bitpackptri++] = nwritten;
    if (offsets[oligospace] >= nextpage) {
      fprintf(stderr,"\nAt final oligo %u (block %u), offsets %lu >= nextpage %lu",
	      oligospace,oligospace/blocksize,offsets[oligospace],nextpage);
      bitpackpages[pagei++] = oligospace/blocksize;
      currpage = nextpage;
      /* nextpage += POSITIONS_PAGE; */
    }
    bitpackptrs[bitpackptri++] = offsets[oligospace] - currpage;


    /* Write bitpackpages */
    bitpackpages[pagei++] = -1U; /* Final value */
    if ((bitpackpages_fp = FOPEN_WRITE_BINARY(bitpackpagesfile)) == NULL) {
      fprintf(stderr,"Can't write to file %s\n",bitpackpagesfile);
      exit(9);
    } else {
      fprintf(stderr,"\nHave %d pages:",pagei);
      for (i = 0; i < pagei; i++) {
	fprintf(stderr," %u",bitpackpages[i]);
      }
      fprintf(stderr,"\n");
      FWRITE_UINTS(bitpackpages,pagei,bitpackpages_fp);
      /* FREE(bitpackpages); */
      fclose(bitpackpages_fp);
    }
  ...
  if (compression_types == NO_COMPRESSION) {
	...
	fprintf(stderr,"For bitpack compression, basesize must be 3 less than blocksize.  Changing requested basesize to %d\n",
      ...
      sprintf(pagesfile,"%s/%s.%s%02d%02d%c%s",
	      destdir,fileroot,IDX_FILESUFFIX,bitpack_basesize,index1part,interval_char,"bitpackpages");
      ...
      Indexdb_write_bitpackptrs_huge(pagesfile,pointersfile,offsetsfile,offsets,oligospace,offsetscomp_blocksize);
      if (offsets[oligospace] == 0) {
	/* Don't check bitpack, which will crash on an empty file */
      } else if (offsetscomp_blocksize > 1) {
	fprintf(stderr,"Checking bitpack...");
