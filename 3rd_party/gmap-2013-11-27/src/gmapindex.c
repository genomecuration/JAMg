static char rcsid[] = "$Id: gmapindex.c 115433 2013-11-18 18:24:33Z twu $";
#ifdef HAVE_CONFIG_H
#include <config.h>
#endif
...
/* Program variables */
typedef enum {NONE, AUXFILES, GENOME, UNSHUFFLE, COUNT, OFFSETS, POSITIONS, SUFFIX_ARRAY, LCP, LCP_UNCOMPRESS} Action_T;
...
main (int argc, char *argv[]) {
  ...
  char *lcpptrsfile, *lcpcompfile, *saindexfile, *sarrayfile;
  ...
  while ((c = getopt(argc,argv,"F:D:d:z:b:k:q:ArlGUCHOPSLXWw:e:Ss:m")) != -1) {
    switch (c) {
    ...
    case 'X': action = LCP_UNCOMPRESS; break;
  ...
  if (action == AUXFILES) {
    ...
    if (Univ_IIT_coord_values_8p(chromosome_iit) == true) {
      ...
      Sarray_write_lcp(lcpptrsfile,lcpcompfile,sarrayfile,genomecomp,genomelength);
      Genome_free(&genomebits);

      Sarray_write_index(saindexfile,sarrayfile,lcpptrsfile,lcpcompfile,genomecomp,genomelength);
    ...
    sarrayfile = (char *) CALLOC(strlen(destdir)+strlen("/")+strlen(fileroot)+strlen(".sarray")+1,sizeof(char));
    sprintf(sarrayfile,"%s/%s.sarray",destdir,fileroot);
    lcpptrsfile = (char *) CALLOC(strlen(sourcedir)+strlen("/")+strlen(fileroot)+strlen(".salcpptrs")+1,sizeof(char));
    sprintf(lcpptrsfile,"%s/%s.salcpptrs",sourcedir,fileroot);
    lcpcompfile = (char *) CALLOC(strlen(sourcedir)+strlen("/")+strlen(fileroot)+strlen(".salcpcomp")+1,sizeof(char));
    sprintf(lcpcompfile,"%s/%s.salcpcomp",sourcedir,fileroot);

    genomelength = Univ_IIT_genomelength(chromosome_iit,/*with_circular_alias_p*/true);
    Sarray_lcp_uncompress(lcpptrsfile,lcpcompfile,sarrayfile,genomelength);
