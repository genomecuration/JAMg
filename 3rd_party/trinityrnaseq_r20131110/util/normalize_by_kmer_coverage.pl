...
my $output_directory = "tmp_normalized_reads";
my $help_flag;
my $seqType;
my $left_file;
my $right_file;
...
#                           (default: '2G' for small test data sets - set larger for production runs) 
#
...
#
#  --JELLY_CPU <int>                     :number of threads for Jellyfish to use (default: = $JELLY_CPU)
...
sub generate_stats_files {
    ...
    if ($PE_reads_unordered) {
        ...
        foreach my $info_aref (@$files_need_stats_aref) {
            ...
            my $cmd = "sort -k5,5 -T . -S $JM_string $stats_file > $sorted_stats_file";
