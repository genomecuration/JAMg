...
#                                   # (only --JM is absolutely required, since defaults exist for the other settings)
#                                   --CPU=6
#                                   --JM=10G
#                                   --seqType=fq
#                                   --SS_lib_type=RF
...
#
#  --genes_too                     Run the DE analysis at the component (gene) level in addition to the transcript (isoform) level.
#
###########################################################################################################
...

{
    ...
    my @needed_tools = qw (R bowtie); # Trinity.pl, RSEM, and samtools are set by relative paths.
    my $missing_flag = 0;
...
unless (-s "trinity_out_dir/Trinity.fasta") {
    ...
    if (-s $reads_ALL_right_fq) {
        ...
        ## Run Trinity:
        $cmd = "$BASEDIR/Trinity.pl --left $reads_ALL_left_fq --right $reads_ALL_right_fq ";
        
    }
    else {
        # run left as single
        $cmd = "$BASEDIR/Trinity.pl --single $reads_ALL_left_fq ";
        
    }
    
    my @trinity_params = qw ( --seqType --JM --CPU --SS_lib_type --monitoring --no_reduce );
...
foreach my $condition (@conditions) {
    ...
    foreach my $replicate (@replicates) {
        ...
        if ( $GENES_TOO && ! -s "Trinity.components_lengths.txt") {
        ...
        push (@genes_rsem_files, "$replicate.genes.results") if $GENES_TOO;
    }
...
foreach my $target_type ("trans", "components") {
    ...
    if ($target_type eq "components" && ! $GENES_TOO) { next; }
