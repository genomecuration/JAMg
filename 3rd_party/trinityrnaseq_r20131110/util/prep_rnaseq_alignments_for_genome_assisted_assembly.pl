...
sub prep_read_partitions {
    ...
    ## define fragments
    my $cmd = "$UTIL_DIR/SAM_to_frag_coords.pl --sam $sam --min_insert_size 1 --max_insert_size $max_intron_length "; ## writes file: $sam_file.frag_coords
    &process_cmd($cmd) unless (-s "$sam.frag_coords");
    
    ## define coverage
    $cmd = "$UTIL_DIR/fragment_coverage_writer.pl $sam.frag_coords > $sam.frag_coverage.wig";
    &process_cmd($cmd) unless (-s "$sam.frag_coverage.wig");
    ...
    &process_cmd($cmd) unless (-s $partitions_file);
    ...
    &process_cmd($cmd) unless (-d "Dir_${partitions_file}");
