...
data = read.table("$matrix_file", header=T, row.names=1, com='')
...
for (i in 1:nsamples) {
    ...
    #print(cat("sample_type:", sample_type))
    #print(rep_names);
    ...
    if (sum(col_indices) != length(rep_names)) {
        stop(cat("Error, not all replicates accounted for. Found only", rep_names, "for sample:", sample_type))
    }
