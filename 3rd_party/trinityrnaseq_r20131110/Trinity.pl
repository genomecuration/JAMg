...
my $VERSION = "Trinityrnaseq_r20131110"; 
...
sub run_chrysalis {
    ...
    my $cmd = "$UTILDIR/partition_chrysalis_graphs_n_reads.pl --deBruijns $chrysalis_output_dir/bundled_iworm_contigs.fasta.deBruijn --componentReads $chrysalis_output_dir/readsToComponents.out.sort -N 1000 -L $min_contig_length --compdir $component_directory ";
...
sub run_inchworm {
    ...
    if (! $FORCE_INCHWORM_KMER_METHOD) {
        ...
        unless (-e $jelly_finished_checkpoint_file) {
            ...
            ## merge results
            my @kmer_db_files = <mer_counts_*>;
            my $jelly_db = "";
            if (scalar(@kmer_db_files) > 1) {
                $cmd = "$JELLYFISH_DIR/bin/jellyfish merge -o jellyfish.kmer.db @kmer_db_files";
                &process_cmd($cmd);
                $jelly_db = "jellyfish.kmer.db";
                
                # remove these kmer files after the merge
                foreach my $kmer_db_file (@kmer_db_files) {
                    unlink($kmer_db_file);
                }
                
            }
            else {
                $jelly_db = shift @kmer_db_files;
            ...
            $cmd = "$JELLYFISH_DIR/bin/jellyfish histo -o $jelly_kmer_fa_file.histo $jelly_db";
            &process_cmd($cmd);
    ...
    return if -e "$file_prefix.fa";
   	for (my $i=0;$i<scalar(@initial_files);$i++){
   	 ...
   	 if ($f=~/\.gz$/){
   	  ...
   	  $new=~s/\.bz2$//;
   	  unlink($new);
	  my $bunzip2_exec = `which pbzip2` || `which bunzip2`;chomp($bunzip2_exec);
   	  &process_cmd("$bunzip2_exec -dkc $f > $new");
   	  $initial_files[$i] = $new;
   	 }
   	}
