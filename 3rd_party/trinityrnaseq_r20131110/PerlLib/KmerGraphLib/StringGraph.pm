...
sub new {
	...
	my $self = { 
        ...
        _edge_color_labels => {},  # keyed on (prev_node, next_node, color)

        _indirect_edge_color_labels => {}, # keyed on color only
...
sub add_sequence_to_graph {
	my $self = shift;
	my ($acc, $sequence_node_names_aref, $weight, $color) = @_;  ## Sequence type can be undef, ReferenceTranscript, or GreedyPath
	...
	my $prev_node_name = $sequence_node_names_aref->[0];
	my $prev_node = $self->get_or_create_node($prev_node_name);
	$prev_node->{_count}++;
	$prev_node->track_reads($acc);
   
	if ($color) {
		$prev_node->add_color($color);
	}
	
	for (my $i = 1; $i <= $#$sequence_node_names_aref; $i++) {
		my $next_node_name = $sequence_node_names_aref->[$i];
		my $next_node = $self->get_or_create_node($next_node_name);
		$next_node->track_reads($acc);
		
		if ($color) {
			$next_node->add_color($color);
		}
        
		$next_node->{_count}++;

		my $prev_edge_weight = $self->get_edge_count($prev_node, $next_node);

        $self->_set_edge_color_label($prev_node, $next_node, $color, $acc);
        
		## only link together kmers that contain only recognizable nucleotides
		$self->link_adjacent_nodes($prev_node, $next_node, $weight);
		
		my $edge_weight = $self->get_edge_count($prev_node, $next_node);
		if ($edge_weight < $weight) {
			confess "Error, added weight $weight to edge $prev_node -> $next_node and didn't stick";
		}
...
sub create_node {
        ...
        my $node = new StringNode($node_name);
...
sub _get_edge_color_label_key {
    my $self = shift;
    my ($prev_node, $next_node, $color) = @_;

    my $edge_color_label = join("$;", $prev_node, $next_node, $color);

    return($edge_color_label);
    
}

sub _set_edge_color_label {
    my $self = shift;
    my ($prev_node, $next_node, $color, $label) = @_;
    
    my $key = $self->_get_edge_color_label_key($prev_node, $next_node, $color);
    
    $self->{_edge_color_labels}->{$key} = $label;
    
    $self->{_indirect_edge_color_labels}->{$color} = $label;
    
    return;
}

sub _get_edge_color_label {
    my $self = shift;
    ...
    my $label = $self->{_edge_color_labels}->{$key};
    
    return($label);
    
}

sub _get_indirect_edge_color_label {
    my $self = shift;
    my $color = shift;

    return($self->{_indirect_edge_color_labels}->{$color});
		...
		foreach my $next_node (@next_nodes) {
			...
			if (@colors_in_common) {
				foreach my $color (@colors_in_common) {
                    my $edge_label = $self->_get_edge_color_label($node, $next_node, $color);					
                    
                    if ($edge_label) {
                        $text .= "\t$id->$next_id [label=\"$edge_label\", color=\"$color\"];\n";
                    }
                    else {
                        ### look for indirect edge
                        #my $indirect_edge_label = $self->_get_indirect_edge_color_label($color);
                        #$text .= "\t$id->$next_id [label=\"$indirect_edge_label\", color=\"$color\", style=\"dashed\"];\n";
                    }
                }
			}
