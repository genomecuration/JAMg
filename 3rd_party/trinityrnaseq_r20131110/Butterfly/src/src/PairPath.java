...
public static  boolean individual_paths_are_compatible (List<Integer> pathA_in, List<Integer> pathB_in) {
	...
	public boolean isCompatibleAndContainedBy (
			PairPath other_pp, 
			DirectedSparseGraph<SeqVertex, SimpleEdge> graph,
			DijkstraDistance<SeqVertex, SimpleEdge> dijkstraDis) {
		
		Integer other_first_id = other_pp.getFirstID();
		Integer other_last_id = other_pp.getLastID();
		
		if (! this.isCompatible(other_pp) ) {
			return false;
		}
		...
		// see if the edge node identifiers are the same (cheap and easy)
		Integer this_first_id = this.getFirstID();
		Integer this_last_id = this.getLastID();
		
		if (other_first_id == this_first_id && other_last_id == this_last_id) {
		...
		SeqVertex this_v1 = SeqVertex.retrieveSeqVertexByID(this_first_id);
		SeqVertex this_v2 = SeqVertex.retrieveSeqVertexByID(this_last_id);
		
		SeqVertex other_v1 = SeqVertex.retrieveSeqVertexByID(other_first_id);
		SeqVertex other_v2 = SeqVertex.retrieveSeqVertexByID(other_last_id);

		if ( (this_v1.equals(other_v1) || dijkstraDis.getDistance(other_v1, this_v1) != null)
				&&
			(this_v2.equals(other_v2) || dijkstraDis.getDistance(this_v2, other_v2) != null) ) {
			return(true);
		}
		else {
			return(false);
	...
	public boolean isCompatibleAndContainedBy (List<Integer> path) { // better name
		
		return(isCompatibleAndContained(path));
