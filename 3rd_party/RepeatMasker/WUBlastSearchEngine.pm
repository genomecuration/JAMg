...
#     Revision 1.129  2013/06/20 17:49:19  rhubley
...
sub search {
       ...
       && $searchResultsCollection->size() > 0 )
  {
    ...
    if ( $this->getScoreMode() == SearchEngineI::complexityAdjustedScoreMode ) {
      ...
      # TODO Break this out into insert open/extn and del
      #      open/ext in rescore routine
      $gapExtn = $this->getInsGapExt();
      if ( !$matrixObj ) {
    ...
    if ( $minScore || $complexityAdjust ) {
      ...
      for ( my $i = $searchResultsCollection->size() - 1 ; $i >= 0 ; $i-- ) {
        ...
        if ( $complexityAdjust ) {
          my ( $newScore ) = $result->rescoreAlignment(
                                                scoreMatrix    => $matrixObj,
                                                gapOpenPenalty => $gapOpen,
                                                gapExtensionPenalty => $gapExtn,
                                                complexityAdjust    => 1
          );
          $result->setScore( $newScore );

          # No longer need alignment data....so per user request purge it
          if ( !$this->getGenerateAlignments() ) {
