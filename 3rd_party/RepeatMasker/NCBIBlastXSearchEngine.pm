...
#
# bless(
#      'NCBIBlastXSearchEngine' );
#
###############################################################################
# ChangeLog
#
#     $Log: NCBIBlastXSearchEngine.pm,v $
#     Revision 1.14  2013/06/20 17:49:18  rhubley
...
Usage: 
  ...
  my $rmEngine = NCBIBlastXSearchEngine->new( 
                    pathToEngine=>"/usr/local/rmblast/bin/blastx" );

  $rmEngine->setMatrix( "/users/bob/simple.matrix" );
  $rmEngine->setQuery( "/users/bob/query.fasta" );
  $rmEngine->setSubject( "/users/bob/subject.fasta" );
  my $searchResults = $rmEngine->search();

=head1 DESCRIPTION

  A concrete implementation of the abstract class / interface SearchEngineI
  which use the NCBIBlastX sequence search engine.

=head1 INSTANCE METHODS

=cut 

package NCBIBlastXSearchEngine;
use strict;
use SearchEngineI;
use SearchResultCollection;
use Data::Dumper;
...
#
my $VERSION = 0.1;
my $CLASS   = "NCBIBlastXSearchEngine";
my $DEBUG   = 0;
...
sub setPathToEngine {
  ...
  croak $CLASS
      ...
      . "a path to the RMBlastX program.\n"
      if ( $value =~ /^\s*$/ );
  ...
  if ( $result =~ /^(blastx):\s*(\S+)/ ) {
    ...
    croak $CLASS
        ...
        . "engine variant and version! Program\n"
        . "returned: [ $result ]\n";
  }
...
sub search {
  ...
  if ( ( $value = $this->getSubject() ) ) {
    ...
    if ( -f "$value.psq" || -f "$value.pin" || -f "$value.phr" ) {
      $parameters .= " -db $value";
    }
    else {
      croak $CLASS
  ...
  if ( defined( $value = $this->getMatrix() ) ) {
    ...
    if ( -f $value ) {
      ...
      # NCBIBLAST expects the path to be above "aa" or "nt" sub
      # directories
  ...
  my $errFile;
  do {
    $errFile = $outputDirName . "/ncbiBXResults-" . time() . ".err";
  } while ( -f $errFile );
  my $pid;
  my $runParameters;
  ...
  if ( $DEBUG ) {
    ...
    my $outFile;
    do {
      $outFile = $outputDirName . "/ncbiBXResults-" . time() . ".out";
    } while ( -f $outFile );
    open OUT, ">$outFile";
    while ( <$POUTPUT> ) {
      print OUT $_;
...
=head2 parseOutput()
  ...
  Use: my \@results = &parseOutput( searchOutput => "",
                                            pValueCutoff => #,
                                            scoreMode => "",
                                            scoreCutoff => #
                                          );

        searchOutput   : NCBI blastX results file or FH
...
sub parseOutput {
  ...
  if ( ref( $parameters{'searchOutput'} ) !~ /GLOB|FileHandle/ ) {
    print $CLASS
        ...
        if ( $DEBUG );
    open $NCBIFILE, $parameters{'searchOutput'}
        or die $CLASS
        . "::parseOutput: Unable to open "
        . "results file: $parameters{'searchOutput'} : $!";
  }
  else {
    $NCBIFILE = $parameters{'searchOutput'};
  }
...
/^\s+Score\s+=\s+[\d\.]+ bits \((\d+)\),\s*Expect(?:\(\d+\))? = ([\d\.e-]+)\s*$/
        )
    {
      $score = $1;
      my $eValue = $2;
      $pValue = sprintf( "%2.2e", 1 - exp( -$eValue ) );

#print "Captured score = $score, eValue = $eValue and calcd pvalue = $pValue\n";
    }
...
##
##      $matFileName : Name of the file containing the NCBIBlast Matrix
##
## Returns
##      This is a very specific matrix reader for NCBIBlast matrices
