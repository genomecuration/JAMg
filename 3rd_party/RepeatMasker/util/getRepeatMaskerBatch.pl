...
#    Revision 1.10  2013/06/20 17:49:19  rhubley
...
#  $Name: open-4-0-3 $ macro will be blank and thus we use
#  this as the development version.
#
my $CVSTag = '$Name: open-4-0-3 $';
my $version;
if ( $CVSTag =~ /\$\s*Name:\s*open-(\S+)\s*\$/ ) {
  ...
  $version =
'development-$Id: getRepeatMaskerBatch.pl,v 1.10 2013/06/20 17:49:19 rhubley Exp $';
}
