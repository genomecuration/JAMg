...
Site **graph ( char *PData,long int PData_Len,char *TRAIN_DIR, long int *splicesiteno, char *ProtDomFile,long int offset,int ese,char *Name,int PREDNO,int force)
{
  ...
  for(i=0;i<MODEL_NO;i++) {
	...
	continue;
      ...
      if(strcmp (Param,"use_protein")==0) {
        fprintf(stderr,"0 Using protein file\n");
  ...
  for(m=0;m<MODEL_NO;m++) {
    ...
    for(i=0;i<6;i++) {
      ...
      if (TCod[m][i] == NULL) {
	fprintf(stderr,"Memory allocation for coding %li failure.\n",i);
	abort();
      }
      TCod[m][i][0]=0;
  ...
  for(i=0;i<2;i++) {
    ...
    for(m=0;m<MODEL_NO;m++) {
      if(Use_Protein[m]) {
        fprintf(stderr,"Using protein file\n");
	Protein[m][i]=NULL;
...
//      sscanf(Line,"%s %d %d %f %e",File_Name,&begdom,&enddom,&bitscore,&eval);
      sscanf(Line,"%s %li %li %f %lf",File_Name,&begdom,&enddom,&bitscore,&eval);
//      fprintf (stderr, "Should be: %s", Line);
//      fprintf (stderr, "Is: %s %li %li %f %e\n\n", File_Name,begdom,enddom,bitscore,eval);
...
//	    fprintf(stderr,"i=%li baseprob=%f j=%li enddom=%li Data_len=%li\n",i,baseprob,j,enddom,Data_Len);
  ...
  for(i=1;i<Data_Len+1;i++) {
    ...
    for(m=0;m<MODEL_NO;m++) {
      if(Use_Protein[m]) {
        fprintf(stderr,"2 Using protein file\n");
  ...
  for(i=1;i<Data_Len+1;i++) {
    ...
    for(m=0;m<MODEL_NO;m++) {
      if(Use_Protein[m]) {
        fprintf(stderr,"3 Using protein file\n");
  ...
  */
  ...
  for(m=0;m<MODEL_NO;m++) {
    if(Use_Protein[m]) {
        fprintf(stderr,"4 Using protein file\n");
      for(i=0;i<2;i++) 
