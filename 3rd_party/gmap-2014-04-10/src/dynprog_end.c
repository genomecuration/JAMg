static char rcsid[] = "$Id: dynprog_end.c 132731 2014-04-08 21:19:57Z twu $";
#ifdef HAVE_CONFIG_H
#include <config.h>
#endif
...
find_best_endpoint_8 (int *finalscore, int *bestr, int *bestc, Score8_T **matrix_upper,
		      ...
		      int extraband_end_or_paired, bool jump_late_p) {
  ...
  if (endalign == QUERYEND_GAP || endalign == BEST_LOCAL) {
    ...
    if (rlength <= SIMD_MAXLENGTH_EPI8 || glength <= SIMD_MAXLENGTH_EPI8) {
      ...
      find_best_endpoint_8(&(*finalscore),&bestr,&bestc,matrix8_upper,matrix8_lower,
			   rlength,glength,extraband_end,!jump_late_p);
    ...
    find_best_endpoint_std(&(*finalscore),&bestr,&bestc,matrix,rlength,glength,extraband_end,
			   !jump_late_p);
#endif
...
Dynprog_end5_splicejunction (int *dynprogindex, int *finalscore, int *missscore,
  ...
  find_best_endpoint_to_queryend_indels_std(&(*finalscore),&bestr,&bestc,matrix,rlength,glength,extraband_end,
					    !jump_late_p);
#endif
