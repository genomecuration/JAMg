static char rcsid[] = "$Id: sarray-write.c 132688 2014-04-08 17:34:25Z twu $";
#ifdef HAVE_CONFIG_H
#include <config.h>
#endif
...
Sarray_write_index_separate (char *indexiptrsfile, char *indexicompfile, char *indexjptrsfile,char *indexjcompfile,
  ...
  noccupied = make_index_separate(saindexi_old,saindexj_old,
				  oligospace,/*querylength*/indexsize,genomecomp,SA,n);
...
Sarray_discriminating_chars (UINT4 *nbytes, UINT4 *SA, Genome_T genome,
  ...
  *nbytes = ((n+1) + 1)/2;
  ...
  for (i = 1; i <= n; i++) {
    ...
    char_before = Genome_get_char_lex(genome,/*left*/SA[i-1] + lcp_i,/*genomelength*/n);
    char_at = Genome_get_char_lex(genome,/*left*/SA[i] + lcp_i,/*genomelength*/n);
