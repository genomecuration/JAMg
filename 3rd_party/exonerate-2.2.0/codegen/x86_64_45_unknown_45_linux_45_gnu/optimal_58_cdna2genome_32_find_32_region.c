/* Code automatically generated by C4 DP library
 * Do not attempt to edit this code: it is spagetti.
 *
 * Model: cdna2genome
 */

 /* ---< START >--- */

#include "ungapped.h"
#include "submat.h"
#include "affine.h"

/* Exhaustive Viterbi DP implementation. */

#include "viterbi.h"

/* Viterbi model:
 * [optimal_58_cdna2genome_32_find_32_region]
 *
 * max query advance: 3
 * max target advance: 3
 *
 * scope [anywhere]->[anywhere]
 */

/* Implementing [4] explicit rows */
C4_Score optimal_58_cdna2genome_32_find_32_region(C4_Model *model, Region *region,               Viterbi_Data *vd, SubOpt_Index *soi,           gpointer user_data){
register C4_Score score = C4_IMPOSSIBLY_LOW_SCORE;
register gint i, j = 0;
register C4_Score ***prev_row_0 = vd->vr->score_matrix[0];
register C4_Score ***prev_row_1 = vd->vr->score_matrix[1];
register C4_Score ***prev_row_2 = vd->vr->score_matrix[2];
register C4_Score ***prev_row_3 = vd->vr->score_matrix[3];
register C4_Score ***swap_row, *src, *dst, t;
register gboolean *state_is_set
 = g_new(gboolean, 22);
register gboolean end_is_set = FALSE;
register Ungapped_Data *ud = (Ungapped_Data*)user_data;
register Affine_Data *ad = (Affine_Data*)user_data;
register Frameshift_Data *fd = ud->frameshift_data;
register Intron_Data *id = ud->intron_data;
model->init_func(region, user_data);
/* Transition calc init */
Intron_ChainData_init_splice_prediction(id->target_data, ud->target, id->ias->sps,                                        SpliceType_ss5_forward,        ((id->curr_region->target_start == 0)        && (id->curr_region->target_length == ud->target->len)));;
Intron_ChainData_init_splice_prediction(id->target_data, ud->target, id->ias->sps,                                        SpliceType_ss3_forward,        ((id->curr_region->target_start == 0)        && (id->curr_region->target_length == ud->target->len)));;
SubOpt_Index_set_row(soi, 0);
/* explicit row [0] */
/* start block */
i = -1;
/* explicit cell [0] */
/* CELL [0,0] */
i++;
prev_row_0[i][0][0] = -987654321;
prev_row_0[i][1][0] = -987654321;
prev_row_0[i][2][0] = -987654321;
prev_row_0[i][3][0] = -987654321;
prev_row_0[i][4][0] = -987654321;
prev_row_0[i][5][0] = -987654321;
prev_row_0[i][6][0] = -987654321;
prev_row_0[i][7][0] = -987654321;
prev_row_0[i][8][0] = -987654321;
prev_row_0[i][9][0] = -987654321;
prev_row_0[i][10][0] = -987654321;
prev_row_0[i][11][0] = -987654321;
prev_row_0[i][12][0] = -987654321;
prev_row_0[i][13][0] = -987654321;
prev_row_0[i][14][0] = -987654321;
prev_row_0[i][15][0] = -987654321;
prev_row_0[i][16][0] = -987654321;
prev_row_0[i][17][0] = -987654321;
prev_row_0[i][18][0] = -987654321;
prev_row_0[i][19][0] = -987654321;
prev_row_0[i][20][0] = -987654321;
prev_row_0[i][21][0] = -987654321;
state_is_set[0] = FALSE;
state_is_set[1] = FALSE;
state_is_set[2] = FALSE;
state_is_set[3] = FALSE;
state_is_set[4] = FALSE;
state_is_set[5] = FALSE;
state_is_set[6] = FALSE;
state_is_set[7] = FALSE;
state_is_set[8] = FALSE;
state_is_set[9] = FALSE;
state_is_set[10] = FALSE;
state_is_set[11] = FALSE;
state_is_set[12] = FALSE;
state_is_set[13] = FALSE;
state_is_set[14] = FALSE;
state_is_set[15] = FALSE;
state_is_set[16] = FALSE;
state_is_set[17] = FALSE;
state_is_set[18] = FALSE;
state_is_set[19] = FALSE;
state_is_set[20] = FALSE;
state_is_set[21] = FALSE;
/* transition [start to match] */
src = prev_row_0[i-0][0];
dst = prev_row_0[i][14];
t = 0;
if(state_is_set[14]){
if(dst[0] < t){
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete to match] */
src = prev_row_0[i-0][16];
dst = prev_row_0[i][14];
t = src[0];
if(state_is_set[14]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert to match] */
src = prev_row_0[i-0][15];
dst = prev_row_0[i][14];
t = src[0];
if(state_is_set[14]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to end] */
src = prev_row_0[i-0][14];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert to match] */
src = prev_row_0[i-0][3];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete to match] */
src = prev_row_0[i-0][4];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [start to match] */
src = prev_row_0[i-0][0];
dst = prev_row_0[i][2];
t = 0;
if(state_is_set[2]){
if(dst[0] < t){
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift close 0 target] */
src = prev_row_0[i-0][6];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift close 0 query] */
src = prev_row_0[i-0][5];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [start to match] */
src = prev_row_0[i-0][2];
dst = prev_row_0[i][18];
t = src[0];
if(state_is_set[18]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete to match] */
src = prev_row_0[i-0][20];
dst = prev_row_0[i][18];
t = src[0];
if(state_is_set[18]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert to match] */
src = prev_row_0[i-0][19];
dst = prev_row_0[i][18];
t = src[0];
if(state_is_set[18]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to end] */
src = prev_row_0[i-0][18];
dst = prev_row_0[i][1];
t = src[0];
if(state_is_set[1]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[1] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to end] */
src = prev_row_0[i-0][2];
dst = prev_row_0[i][1];
t = src[0];
if(state_is_set[1]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[1] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
if(state_is_set[1]){
if(end_is_set){
if(score < prev_row_0[i][1][0]){
score = prev_row_0[i][1][0];
vd->curr_query_end = i;
vd->curr_target_end = j;
vd->curr_query_start = prev_row_0[i][1][vd->vr->region_start_query_id];
vd->curr_target_start = prev_row_0[i][1][vd->vr->region_start_target_id];
}
} else {
score = prev_row_0[i][1][0];
end_is_set = TRUE;
vd->curr_query_end = i;
vd->curr_target_end = j;
vd->curr_query_start = prev_row_0[i][1][vd->vr->region_start_query_id];
vd->curr_target_start = prev_row_0[i][1][vd->vr->region_start_target_id];
}
}
if(j == region->target_length){
if(i == region->query_length){
goto end_of_dp;
}
} else {
if(i == region->query_length){
goto row_1;
}
}
/* explicit cell [1] */
/* CELL [0,1] */
i++;
prev_row_0[i][0][0] = -987654321;
prev_row_0[i][1][0] = -987654321;
prev_row_0[i][2][0] = -987654321;
prev_row_0[i][3][0] = -987654321;
prev_row_0[i][4][0] = -987654321;
prev_row_0[i][5][0] = -987654321;
prev_row_0[i][6][0] = -987654321;
prev_row_0[i][7][0] = -987654321;
prev_row_0[i][8][0] = -987654321;
prev_row_0[i][9][0] = -987654321;
prev_row_0[i][10][0] = -987654321;
prev_row_0[i][11][0] = -987654321;
prev_row_0[i][12][0] = -987654321;
prev_row_0[i][13][0] = -987654321;
prev_row_0[i][14][0] = -987654321;
prev_row_0[i][15][0] = -987654321;
prev_row_0[i][16][0] = -987654321;
prev_row_0[i][17][0] = -987654321;
prev_row_0[i][18][0] = -987654321;
prev_row_0[i][19][0] = -987654321;
prev_row_0[i][20][0] = -987654321;
prev_row_0[i][21][0] = -987654321;
state_is_set[0] = FALSE;
state_is_set[1] = FALSE;
state_is_set[2] = FALSE;
state_is_set[3] = FALSE;
state_is_set[4] = FALSE;
state_is_set[5] = FALSE;
state_is_set[6] = FALSE;
state_is_set[7] = FALSE;
state_is_set[8] = FALSE;
state_is_set[9] = FALSE;
state_is_set[10] = FALSE;
state_is_set[11] = FALSE;
state_is_set[12] = FALSE;
state_is_set[13] = FALSE;
state_is_set[14] = FALSE;
state_is_set[15] = FALSE;
state_is_set[16] = FALSE;
state_is_set[17] = FALSE;
state_is_set[18] = FALSE;
state_is_set[19] = FALSE;
state_is_set[20] = FALSE;
state_is_set[21] = FALSE;
/* transition [insert] */
src = prev_row_0[i-1][19];
dst = prev_row_0[i][19];
t = (ad->aas->gap_extend);
t += src[0];
if(state_is_set[19]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[19] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to insert] */
src = prev_row_0[i-1][18];
dst = prev_row_0[i][19];
t = (ad->aas->gap_open);
t += src[0];
if(state_is_set[19]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[19] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert] */
src = prev_row_0[i-1][15];
dst = prev_row_0[i][15];
t = (ad->aas->gap_extend);
t += src[0];
if(state_is_set[15]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[15] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to insert] */
src = prev_row_0[i-1][14];
dst = prev_row_0[i][15];
t = (ad->aas->gap_open);
t += src[0];
if(state_is_set[15]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[15] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift open 1 query] */
src = prev_row_0[i-1][2];
dst = prev_row_0[i][5];
t = (fd->fas->frameshift_penalty);
t += src[0];
if(state_is_set[5]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[5] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [start to match] */
src = prev_row_0[i-0][0];
dst = prev_row_0[i][14];
t = 0;
if(state_is_set[14]){
if(dst[0] < t){
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete to match] */
src = prev_row_0[i-0][16];
dst = prev_row_0[i][14];
t = src[0];
if(state_is_set[14]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert to match] */
src = prev_row_0[i-0][15];
dst = prev_row_0[i][14];
t = src[0];
if(state_is_set[14]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to end] */
src = prev_row_0[i-0][14];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert to match] */
src = prev_row_0[i-0][3];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete to match] */
src = prev_row_0[i-0][4];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [start to match] */
src = prev_row_0[i-0][0];
dst = prev_row_0[i][2];
t = 0;
if(state_is_set[2]){
if(dst[0] < t){
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift close 0 target] */
src = prev_row_0[i-0][6];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift close 0 query] */
src = prev_row_0[i-0][5];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [start to match] */
src = prev_row_0[i-0][2];
dst = prev_row_0[i][18];
t = src[0];
if(state_is_set[18]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete to match] */
src = prev_row_0[i-0][20];
dst = prev_row_0[i][18];
t = src[0];
if(state_is_set[18]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert to match] */
src = prev_row_0[i-0][19];
dst = prev_row_0[i][18];
t = src[0];
if(state_is_set[18]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to end] */
src = prev_row_0[i-0][18];
dst = prev_row_0[i][1];
t = src[0];
if(state_is_set[1]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[1] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to end] */
src = prev_row_0[i-0][2];
dst = prev_row_0[i][1];
t = src[0];
if(state_is_set[1]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[1] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
if(state_is_set[1]){
if(end_is_set){
if(score < prev_row_0[i][1][0]){
score = prev_row_0[i][1][0];
vd->curr_query_end = i;
vd->curr_target_end = j;
vd->curr_query_start = prev_row_0[i][1][vd->vr->region_start_query_id];
vd->curr_target_start = prev_row_0[i][1][vd->vr->region_start_target_id];
}
} else {
score = prev_row_0[i][1][0];
end_is_set = TRUE;
vd->curr_query_end = i;
vd->curr_target_end = j;
vd->curr_query_start = prev_row_0[i][1][vd->vr->region_start_query_id];
vd->curr_target_start = prev_row_0[i][1][vd->vr->region_start_target_id];
}
}
if(j == region->target_length){
if(i == region->query_length){
goto end_of_dp;
}
} else {
if(i == region->query_length){
goto row_1;
}
}
/* explicit cell [2] */
/* CELL [0,2] */
i++;
prev_row_0[i][0][0] = -987654321;
prev_row_0[i][1][0] = -987654321;
prev_row_0[i][2][0] = -987654321;
prev_row_0[i][3][0] = -987654321;
prev_row_0[i][4][0] = -987654321;
prev_row_0[i][5][0] = -987654321;
prev_row_0[i][6][0] = -987654321;
prev_row_0[i][7][0] = -987654321;
prev_row_0[i][8][0] = -987654321;
prev_row_0[i][9][0] = -987654321;
prev_row_0[i][10][0] = -987654321;
prev_row_0[i][11][0] = -987654321;
prev_row_0[i][12][0] = -987654321;
prev_row_0[i][13][0] = -987654321;
prev_row_0[i][14][0] = -987654321;
prev_row_0[i][15][0] = -987654321;
prev_row_0[i][16][0] = -987654321;
prev_row_0[i][17][0] = -987654321;
prev_row_0[i][18][0] = -987654321;
prev_row_0[i][19][0] = -987654321;
prev_row_0[i][20][0] = -987654321;
prev_row_0[i][21][0] = -987654321;
state_is_set[0] = FALSE;
state_is_set[1] = FALSE;
state_is_set[2] = FALSE;
state_is_set[3] = FALSE;
state_is_set[4] = FALSE;
state_is_set[5] = FALSE;
state_is_set[6] = FALSE;
state_is_set[7] = FALSE;
state_is_set[8] = FALSE;
state_is_set[9] = FALSE;
state_is_set[10] = FALSE;
state_is_set[11] = FALSE;
state_is_set[12] = FALSE;
state_is_set[13] = FALSE;
state_is_set[14] = FALSE;
state_is_set[15] = FALSE;
state_is_set[16] = FALSE;
state_is_set[17] = FALSE;
state_is_set[18] = FALSE;
state_is_set[19] = FALSE;
state_is_set[20] = FALSE;
state_is_set[21] = FALSE;
/* transition [insert] */
src = prev_row_0[i-1][19];
dst = prev_row_0[i][19];
t = (ad->aas->gap_extend);
t += src[0];
if(state_is_set[19]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[19] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to insert] */
src = prev_row_0[i-1][18];
dst = prev_row_0[i][19];
t = (ad->aas->gap_open);
t += src[0];
if(state_is_set[19]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[19] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert] */
src = prev_row_0[i-1][15];
dst = prev_row_0[i][15];
t = (ad->aas->gap_extend);
t += src[0];
if(state_is_set[15]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[15] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to insert] */
src = prev_row_0[i-1][14];
dst = prev_row_0[i][15];
t = (ad->aas->gap_open);
t += src[0];
if(state_is_set[15]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[15] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift open 2 query] */
src = prev_row_0[i-2][2];
dst = prev_row_0[i][5];
t = (fd->fas->frameshift_penalty);
t += src[0];
if(state_is_set[5]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[5] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift open 1 query] */
src = prev_row_0[i-1][2];
dst = prev_row_0[i][5];
t = (fd->fas->frameshift_penalty);
t += src[0];
if(state_is_set[5]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[5] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [start to match] */
src = prev_row_0[i-0][0];
dst = prev_row_0[i][14];
t = 0;
if(state_is_set[14]){
if(dst[0] < t){
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete to match] */
src = prev_row_0[i-0][16];
dst = prev_row_0[i][14];
t = src[0];
if(state_is_set[14]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert to match] */
src = prev_row_0[i-0][15];
dst = prev_row_0[i][14];
t = src[0];
if(state_is_set[14]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to end] */
src = prev_row_0[i-0][14];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert to match] */
src = prev_row_0[i-0][3];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete to match] */
src = prev_row_0[i-0][4];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [start to match] */
src = prev_row_0[i-0][0];
dst = prev_row_0[i][2];
t = 0;
if(state_is_set[2]){
if(dst[0] < t){
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift close 0 target] */
src = prev_row_0[i-0][6];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift close 0 query] */
src = prev_row_0[i-0][5];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [start to match] */
src = prev_row_0[i-0][2];
dst = prev_row_0[i][18];
t = src[0];
if(state_is_set[18]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete to match] */
src = prev_row_0[i-0][20];
dst = prev_row_0[i][18];
t = src[0];
if(state_is_set[18]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert to match] */
src = prev_row_0[i-0][19];
dst = prev_row_0[i][18];
t = src[0];
if(state_is_set[18]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to end] */
src = prev_row_0[i-0][18];
dst = prev_row_0[i][1];
t = src[0];
if(state_is_set[1]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[1] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to end] */
src = prev_row_0[i-0][2];
dst = prev_row_0[i][1];
t = src[0];
if(state_is_set[1]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[1] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
if(state_is_set[1]){
if(end_is_set){
if(score < prev_row_0[i][1][0]){
score = prev_row_0[i][1][0];
vd->curr_query_end = i;
vd->curr_target_end = j;
vd->curr_query_start = prev_row_0[i][1][vd->vr->region_start_query_id];
vd->curr_target_start = prev_row_0[i][1][vd->vr->region_start_target_id];
}
} else {
score = prev_row_0[i][1][0];
end_is_set = TRUE;
vd->curr_query_end = i;
vd->curr_target_end = j;
vd->curr_query_start = prev_row_0[i][1][vd->vr->region_start_query_id];
vd->curr_target_start = prev_row_0[i][1][vd->vr->region_start_target_id];
}
}
if(j == region->target_length){
if(i == region->query_length){
goto end_of_dp;
}
} else {
if(i == region->query_length){
goto row_1;
}
}
/* last cell [3] */
do {
/* CELL [0,3] */
i++;
prev_row_0[i][0][0] = -987654321;
prev_row_0[i][1][0] = -987654321;
prev_row_0[i][2][0] = -987654321;
prev_row_0[i][3][0] = -987654321;
prev_row_0[i][4][0] = -987654321;
prev_row_0[i][5][0] = -987654321;
prev_row_0[i][6][0] = -987654321;
prev_row_0[i][7][0] = -987654321;
prev_row_0[i][8][0] = -987654321;
prev_row_0[i][9][0] = -987654321;
prev_row_0[i][10][0] = -987654321;
prev_row_0[i][11][0] = -987654321;
prev_row_0[i][12][0] = -987654321;
prev_row_0[i][13][0] = -987654321;
prev_row_0[i][14][0] = -987654321;
prev_row_0[i][15][0] = -987654321;
prev_row_0[i][16][0] = -987654321;
prev_row_0[i][17][0] = -987654321;
prev_row_0[i][18][0] = -987654321;
prev_row_0[i][19][0] = -987654321;
prev_row_0[i][20][0] = -987654321;
prev_row_0[i][21][0] = -987654321;
state_is_set[0] = FALSE;
state_is_set[1] = FALSE;
state_is_set[2] = FALSE;
state_is_set[3] = FALSE;
state_is_set[4] = FALSE;
state_is_set[5] = FALSE;
state_is_set[6] = FALSE;
state_is_set[7] = FALSE;
state_is_set[8] = FALSE;
state_is_set[9] = FALSE;
state_is_set[10] = FALSE;
state_is_set[11] = FALSE;
state_is_set[12] = FALSE;
state_is_set[13] = FALSE;
state_is_set[14] = FALSE;
state_is_set[15] = FALSE;
state_is_set[16] = FALSE;
state_is_set[17] = FALSE;
state_is_set[18] = FALSE;
state_is_set[19] = FALSE;
state_is_set[20] = FALSE;
state_is_set[21] = FALSE;
/* transition [insert] */
src = prev_row_0[i-1][19];
dst = prev_row_0[i][19];
t = (ad->aas->gap_extend);
t += src[0];
if(state_is_set[19]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[19] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to insert] */
src = prev_row_0[i-1][18];
dst = prev_row_0[i][19];
t = (ad->aas->gap_open);
t += src[0];
if(state_is_set[19]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[19] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert] */
src = prev_row_0[i-1][15];
dst = prev_row_0[i][15];
t = (ad->aas->gap_extend);
t += src[0];
if(state_is_set[15]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[15] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to insert] */
src = prev_row_0[i-1][14];
dst = prev_row_0[i][15];
t = (ad->aas->gap_open);
t += src[0];
if(state_is_set[15]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[15] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift close 3 query] */
src = prev_row_0[i-3][5];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift open 2 query] */
src = prev_row_0[i-2][2];
dst = prev_row_0[i][5];
t = (fd->fas->frameshift_penalty);
t += src[0];
if(state_is_set[5]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[5] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift open 1 query] */
src = prev_row_0[i-1][2];
dst = prev_row_0[i][5];
t = (fd->fas->frameshift_penalty);
t += src[0];
if(state_is_set[5]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[5] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to insert] */
src = prev_row_0[i-3][2];
dst = prev_row_0[i][3];
t = (ad->aas->codon_gap_open);
t += src[0];
if(state_is_set[3]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[3] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert] */
src = prev_row_0[i-3][3];
dst = prev_row_0[i][3];
t = (ad->aas->codon_gap_extend);
t += src[0];
if(state_is_set[3]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[3] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [start to match] */
src = prev_row_0[i-0][0];
dst = prev_row_0[i][14];
t = 0;
if(state_is_set[14]){
if(dst[0] < t){
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete to match] */
src = prev_row_0[i-0][16];
dst = prev_row_0[i][14];
t = src[0];
if(state_is_set[14]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert to match] */
src = prev_row_0[i-0][15];
dst = prev_row_0[i][14];
t = src[0];
if(state_is_set[14]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to end] */
src = prev_row_0[i-0][14];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert to match] */
src = prev_row_0[i-0][3];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete to match] */
src = prev_row_0[i-0][4];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [start to match] */
src = prev_row_0[i-0][0];
dst = prev_row_0[i][2];
t = 0;
if(state_is_set[2]){
if(dst[0] < t){
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift close 0 target] */
src = prev_row_0[i-0][6];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift close 0 query] */
src = prev_row_0[i-0][5];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [start to match] */
src = prev_row_0[i-0][2];
dst = prev_row_0[i][18];
t = src[0];
if(state_is_set[18]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete to match] */
src = prev_row_0[i-0][20];
dst = prev_row_0[i][18];
t = src[0];
if(state_is_set[18]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert to match] */
src = prev_row_0[i-0][19];
dst = prev_row_0[i][18];
t = src[0];
if(state_is_set[18]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to end] */
src = prev_row_0[i-0][18];
dst = prev_row_0[i][1];
t = src[0];
if(state_is_set[1]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[1] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to end] */
src = prev_row_0[i-0][2];
dst = prev_row_0[i][1];
t = src[0];
if(state_is_set[1]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[1] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
if(state_is_set[1]){
if(end_is_set){
if(score < prev_row_0[i][1][0]){
score = prev_row_0[i][1][0];
vd->curr_query_end = i;
vd->curr_target_end = j;
vd->curr_query_start = prev_row_0[i][1][vd->vr->region_start_query_id];
vd->curr_target_start = prev_row_0[i][1][vd->vr->region_start_target_id];
}
} else {
score = prev_row_0[i][1][0];
end_is_set = TRUE;
vd->curr_query_end = i;
vd->curr_target_end = j;
vd->curr_query_start = prev_row_0[i][1][vd->vr->region_start_query_id];
vd->curr_target_start = prev_row_0[i][1][vd->vr->region_start_target_id];
}
}
if(j == region->target_length){
if(i == region->query_length){
goto end_of_dp;
}
} else {
if(i == region->query_length){
goto row_1;
}
}
} while(TRUE);
/* explicit row [1] */
row_1:
j++;
SubOpt_Index_set_row(soi, j);
swap_row = prev_row_3;
prev_row_3 = prev_row_2;
prev_row_2 = prev_row_1;
prev_row_1 = prev_row_0;
prev_row_0 = swap_row;
/* start block */
i = -1;
/* explicit cell [0] */
/* CELL [1,0] */
i++;
prev_row_0[i][0][0] = -987654321;
prev_row_0[i][1][0] = -987654321;
prev_row_0[i][2][0] = -987654321;
prev_row_0[i][3][0] = -987654321;
prev_row_0[i][4][0] = -987654321;
prev_row_0[i][5][0] = -987654321;
prev_row_0[i][6][0] = -987654321;
prev_row_0[i][7][0] = -987654321;
prev_row_0[i][8][0] = -987654321;
prev_row_0[i][9][0] = -987654321;
prev_row_0[i][10][0] = -987654321;
prev_row_0[i][11][0] = -987654321;
prev_row_0[i][12][0] = -987654321;
prev_row_0[i][13][0] = -987654321;
prev_row_0[i][14][0] = -987654321;
prev_row_0[i][15][0] = -987654321;
prev_row_0[i][16][0] = -987654321;
prev_row_0[i][17][0] = -987654321;
prev_row_0[i][18][0] = -987654321;
prev_row_0[i][19][0] = -987654321;
prev_row_0[i][20][0] = -987654321;
prev_row_0[i][21][0] = -987654321;
state_is_set[0] = FALSE;
state_is_set[1] = FALSE;
state_is_set[2] = FALSE;
state_is_set[3] = FALSE;
state_is_set[4] = FALSE;
state_is_set[5] = FALSE;
state_is_set[6] = FALSE;
state_is_set[7] = FALSE;
state_is_set[8] = FALSE;
state_is_set[9] = FALSE;
state_is_set[10] = FALSE;
state_is_set[11] = FALSE;
state_is_set[12] = FALSE;
state_is_set[13] = FALSE;
state_is_set[14] = FALSE;
state_is_set[15] = FALSE;
state_is_set[16] = FALSE;
state_is_set[17] = FALSE;
state_is_set[18] = FALSE;
state_is_set[19] = FALSE;
state_is_set[20] = FALSE;
state_is_set[21] = FALSE;
/* transition [delete] */
src = prev_row_1[i-0][20];
dst = prev_row_0[i][20];
t = (ad->aas->gap_extend);
t += src[0];
if(state_is_set[20]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[20] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to delete] */
src = prev_row_1[i-0][18];
dst = prev_row_0[i][20];
t = (ad->aas->gap_open);
t += src[0];
if(state_is_set[20]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[20] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [target intron loop forward] */
src = prev_row_1[i-0][21];
dst = prev_row_0[i][21];
t = src[0];
if(state_is_set[21]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[21] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete] */
src = prev_row_1[i-0][16];
dst = prev_row_0[i][16];
t = (ad->aas->gap_extend);
t += src[0];
if(state_is_set[16]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[16] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to delete] */
src = prev_row_1[i-0][14];
dst = prev_row_0[i][16];
t = (ad->aas->gap_open);
t += src[0];
if(state_is_set[16]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[16] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [target intron loop forward] */
src = prev_row_1[i-0][17];
dst = prev_row_0[i][17];
t = src[0];
if(state_is_set[17]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[17] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift open 1 target] */
src = prev_row_1[i-0][2];
dst = prev_row_0[i][6];
t = (fd->fas->frameshift_penalty);
t += src[0];
if(state_is_set[6]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[6] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [target intron loop 2:1 phase target intron -T] */
src = prev_row_1[i-0][13];
dst = prev_row_0[i][13];
t = src[0];
if(state_is_set[13]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[13] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [target intron loop 1:2 phase target intron -T] */
src = prev_row_1[i-0][12];
dst = prev_row_0[i][12];
t = src[0];
if(state_is_set[12]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[12] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [target intron loop 0:0 phase target intron -T] */
src = prev_row_1[i-0][11];
dst = prev_row_0[i][11];
t = src[0];
if(state_is_set[11]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[11] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [start to match] */
src = prev_row_0[i-0][0];
dst = prev_row_0[i][14];
t = 0;
if(state_is_set[14]){
if(dst[0] < t){
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete to match] */
src = prev_row_0[i-0][16];
dst = prev_row_0[i][14];
t = src[0];
if(state_is_set[14]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert to match] */
src = prev_row_0[i-0][15];
dst = prev_row_0[i][14];
t = src[0];
if(state_is_set[14]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to end] */
src = prev_row_0[i-0][14];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert to match] */
src = prev_row_0[i-0][3];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete to match] */
src = prev_row_0[i-0][4];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [start to match] */
src = prev_row_0[i-0][0];
dst = prev_row_0[i][2];
t = 0;
if(state_is_set[2]){
if(dst[0] < t){
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift close 0 target] */
src = prev_row_0[i-0][6];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift close 0 query] */
src = prev_row_0[i-0][5];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [start to match] */
src = prev_row_0[i-0][2];
dst = prev_row_0[i][18];
t = src[0];
if(state_is_set[18]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete to match] */
src = prev_row_0[i-0][20];
dst = prev_row_0[i][18];
t = src[0];
if(state_is_set[18]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert to match] */
src = prev_row_0[i-0][19];
dst = prev_row_0[i][18];
t = src[0];
if(state_is_set[18]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to end] */
src = prev_row_0[i-0][18];
dst = prev_row_0[i][1];
t = src[0];
if(state_is_set[1]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[1] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to end] */
src = prev_row_0[i-0][2];
dst = prev_row_0[i][1];
t = src[0];
if(state_is_set[1]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[1] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
if(state_is_set[1]){
if(end_is_set){
if(score < prev_row_0[i][1][0]){
score = prev_row_0[i][1][0];
vd->curr_query_end = i;
vd->curr_target_end = j;
vd->curr_query_start = prev_row_0[i][1][vd->vr->region_start_query_id];
vd->curr_target_start = prev_row_0[i][1][vd->vr->region_start_target_id];
}
} else {
score = prev_row_0[i][1][0];
end_is_set = TRUE;
vd->curr_query_end = i;
vd->curr_target_end = j;
vd->curr_query_start = prev_row_0[i][1][vd->vr->region_start_query_id];
vd->curr_target_start = prev_row_0[i][1][vd->vr->region_start_target_id];
}
}
if(j == region->target_length){
if(i == region->query_length){
goto end_of_dp;
}
} else {
if(i == region->query_length){
goto row_2;
}
}
/* explicit cell [1] */
/* CELL [1,1] */
i++;
prev_row_0[i][0][0] = -987654321;
prev_row_0[i][1][0] = -987654321;
prev_row_0[i][2][0] = -987654321;
prev_row_0[i][3][0] = -987654321;
prev_row_0[i][4][0] = -987654321;
prev_row_0[i][5][0] = -987654321;
prev_row_0[i][6][0] = -987654321;
prev_row_0[i][7][0] = -987654321;
prev_row_0[i][8][0] = -987654321;
prev_row_0[i][9][0] = -987654321;
prev_row_0[i][10][0] = -987654321;
prev_row_0[i][11][0] = -987654321;
prev_row_0[i][12][0] = -987654321;
prev_row_0[i][13][0] = -987654321;
prev_row_0[i][14][0] = -987654321;
prev_row_0[i][15][0] = -987654321;
prev_row_0[i][16][0] = -987654321;
prev_row_0[i][17][0] = -987654321;
prev_row_0[i][18][0] = -987654321;
prev_row_0[i][19][0] = -987654321;
prev_row_0[i][20][0] = -987654321;
prev_row_0[i][21][0] = -987654321;
state_is_set[0] = FALSE;
state_is_set[1] = FALSE;
state_is_set[2] = FALSE;
state_is_set[3] = FALSE;
state_is_set[4] = FALSE;
state_is_set[5] = FALSE;
state_is_set[6] = FALSE;
state_is_set[7] = FALSE;
state_is_set[8] = FALSE;
state_is_set[9] = FALSE;
state_is_set[10] = FALSE;
state_is_set[11] = FALSE;
state_is_set[12] = FALSE;
state_is_set[13] = FALSE;
state_is_set[14] = FALSE;
state_is_set[15] = FALSE;
state_is_set[16] = FALSE;
state_is_set[17] = FALSE;
state_is_set[18] = FALSE;
state_is_set[19] = FALSE;
state_is_set[20] = FALSE;
state_is_set[21] = FALSE;
/* transition [delete] */
src = prev_row_1[i-0][20];
dst = prev_row_0[i][20];
t = (ad->aas->gap_extend);
t += src[0];
if(state_is_set[20]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[20] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert] */
src = prev_row_0[i-1][19];
dst = prev_row_0[i][19];
t = (ad->aas->gap_extend);
t += src[0];
if(state_is_set[19]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[19] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to delete] */
src = prev_row_1[i-0][18];
dst = prev_row_0[i][20];
t = (ad->aas->gap_open);
t += src[0];
if(state_is_set[20]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[20] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to insert] */
src = prev_row_0[i-1][18];
dst = prev_row_0[i][19];
t = (ad->aas->gap_open);
t += src[0];
if(state_is_set[19]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[19] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
if(!(soi && SubOpt_Index_is_blocked_fast(soi, i))){
/* transition [match] */
src = prev_row_1[i-1][18];
dst = prev_row_0[i][18];
t = ((ud->query->annotation)
&& (region->query_start+i-1 >= ud->query->annotation->cds_start)
&& (region->query_start+i-1 < (ud->query->annotation->cds_start
          +ud->query->annotation->cds_length)))
?MATCH_IMPOSSIBLY_LOW_SCORE
:Submat_lookup(ud->mas->dna_submat,
     Sequence_get_symbol(ud->query, region->query_start+i-1),
     Sequence_get_symbol(ud->target, region->target_start+j-1));
t += src[0];
if(state_is_set[18]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
}
/* transition [target intron loop forward] */
src = prev_row_1[i-0][21];
dst = prev_row_0[i][21];
t = src[0];
if(state_is_set[21]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[21] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete] */
src = prev_row_1[i-0][16];
dst = prev_row_0[i][16];
t = (ad->aas->gap_extend);
t += src[0];
if(state_is_set[16]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[16] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert] */
src = prev_row_0[i-1][15];
dst = prev_row_0[i][15];
t = (ad->aas->gap_extend);
t += src[0];
if(state_is_set[15]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[15] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to delete] */
src = prev_row_1[i-0][14];
dst = prev_row_0[i][16];
t = (ad->aas->gap_open);
t += src[0];
if(state_is_set[16]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[16] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to insert] */
src = prev_row_0[i-1][14];
dst = prev_row_0[i][15];
t = (ad->aas->gap_open);
t += src[0];
if(state_is_set[15]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[15] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
if(!(soi && SubOpt_Index_is_blocked_fast(soi, i))){
/* transition [match] */
src = prev_row_1[i-1][14];
dst = prev_row_0[i][14];
t = ((ud->query->annotation)
&& (region->query_start+i-1 >= ud->query->annotation->cds_start)
&& (region->query_start+i-1 < (ud->query->annotation->cds_start
          +ud->query->annotation->cds_length)))
?MATCH_IMPOSSIBLY_LOW_SCORE
:Submat_lookup(ud->mas->dna_submat,
     Sequence_get_symbol(ud->query, region->query_start+i-1),
     Sequence_get_symbol(ud->target, region->target_start+j-1));
t += src[0];
if(state_is_set[14]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
}
/* transition [target intron loop forward] */
src = prev_row_1[i-0][17];
dst = prev_row_0[i][17];
t = src[0];
if(state_is_set[17]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[17] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift open 1 target] */
src = prev_row_1[i-0][2];
dst = prev_row_0[i][6];
t = (fd->fas->frameshift_penalty);
t += src[0];
if(state_is_set[6]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[6] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift open 1 query] */
src = prev_row_0[i-1][2];
dst = prev_row_0[i][5];
t = (fd->fas->frameshift_penalty);
t += src[0];
if(state_is_set[5]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[5] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [target intron loop 2:1 phase target intron -T] */
src = prev_row_1[i-0][13];
dst = prev_row_0[i][13];
t = src[0];
if(state_is_set[13]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[13] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [target intron loop 1:2 phase target intron -T] */
src = prev_row_1[i-0][12];
dst = prev_row_0[i][12];
t = src[0];
if(state_is_set[12]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[12] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [target intron loop 0:0 phase target intron -T] */
src = prev_row_1[i-0][11];
dst = prev_row_0[i][11];
t = src[0];
if(state_is_set[11]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[11] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [phase2post phase target intron -T to (END)] */
src = prev_row_1[i-1][10];
dst = prev_row_0[i][2];
id->target_data->curr_intron_start = (gint)src[1];
t = (((region->query_start+i-1 >= 2) && (id->target_data->curr_intron_start >= 2)))?ud->match_list[4]->split_score_func(
ud->match_list[4], ud->query, ud->target,
region->query_start+i-1-2, region->query_start+i-1-1, region->query_start+i-1, (id->target_data->curr_intron_start - 2), (id->target_data->curr_intron_start - 1), region->target_start+j-1):-987654321;
t += src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [(START) to phase1pre phase target intron -T] */
src = prev_row_1[i-1][2];
dst = prev_row_0[i][7];
t = src[0];
if(state_is_set[7]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[7] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [start to match] */
src = prev_row_0[i-0][0];
dst = prev_row_0[i][14];
t = 0;
if(state_is_set[14]){
if(dst[0] < t){
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete to match] */
src = prev_row_0[i-0][16];
dst = prev_row_0[i][14];
t = src[0];
if(state_is_set[14]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert to match] */
src = prev_row_0[i-0][15];
dst = prev_row_0[i][14];
t = src[0];
if(state_is_set[14]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to end] */
src = prev_row_0[i-0][14];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert to match] */
src = prev_row_0[i-0][3];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete to match] */
src = prev_row_0[i-0][4];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [start to match] */
src = prev_row_0[i-0][0];
dst = prev_row_0[i][2];
t = 0;
if(state_is_set[2]){
if(dst[0] < t){
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift close 0 target] */
src = prev_row_0[i-0][6];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift close 0 query] */
src = prev_row_0[i-0][5];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [start to match] */
src = prev_row_0[i-0][2];
dst = prev_row_0[i][18];
t = src[0];
if(state_is_set[18]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete to match] */
src = prev_row_0[i-0][20];
dst = prev_row_0[i][18];
t = src[0];
if(state_is_set[18]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert to match] */
src = prev_row_0[i-0][19];
dst = prev_row_0[i][18];
t = src[0];
if(state_is_set[18]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to end] */
src = prev_row_0[i-0][18];
dst = prev_row_0[i][1];
t = src[0];
if(state_is_set[1]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[1] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to end] */
src = prev_row_0[i-0][2];
dst = prev_row_0[i][1];
t = src[0];
if(state_is_set[1]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[1] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
if(state_is_set[1]){
if(end_is_set){
if(score < prev_row_0[i][1][0]){
score = prev_row_0[i][1][0];
vd->curr_query_end = i;
vd->curr_target_end = j;
vd->curr_query_start = prev_row_0[i][1][vd->vr->region_start_query_id];
vd->curr_target_start = prev_row_0[i][1][vd->vr->region_start_target_id];
}
} else {
score = prev_row_0[i][1][0];
end_is_set = TRUE;
vd->curr_query_end = i;
vd->curr_target_end = j;
vd->curr_query_start = prev_row_0[i][1][vd->vr->region_start_query_id];
vd->curr_target_start = prev_row_0[i][1][vd->vr->region_start_target_id];
}
}
if(j == region->target_length){
if(i == region->query_length){
goto end_of_dp;
}
} else {
if(i == region->query_length){
goto row_2;
}
}
/* explicit cell [2] */
/* CELL [1,2] */
i++;
prev_row_0[i][0][0] = -987654321;
prev_row_0[i][1][0] = -987654321;
prev_row_0[i][2][0] = -987654321;
prev_row_0[i][3][0] = -987654321;
prev_row_0[i][4][0] = -987654321;
prev_row_0[i][5][0] = -987654321;
prev_row_0[i][6][0] = -987654321;
prev_row_0[i][7][0] = -987654321;
prev_row_0[i][8][0] = -987654321;
prev_row_0[i][9][0] = -987654321;
prev_row_0[i][10][0] = -987654321;
prev_row_0[i][11][0] = -987654321;
prev_row_0[i][12][0] = -987654321;
prev_row_0[i][13][0] = -987654321;
prev_row_0[i][14][0] = -987654321;
prev_row_0[i][15][0] = -987654321;
prev_row_0[i][16][0] = -987654321;
prev_row_0[i][17][0] = -987654321;
prev_row_0[i][18][0] = -987654321;
prev_row_0[i][19][0] = -987654321;
prev_row_0[i][20][0] = -987654321;
prev_row_0[i][21][0] = -987654321;
state_is_set[0] = FALSE;
state_is_set[1] = FALSE;
state_is_set[2] = FALSE;
state_is_set[3] = FALSE;
state_is_set[4] = FALSE;
state_is_set[5] = FALSE;
state_is_set[6] = FALSE;
state_is_set[7] = FALSE;
state_is_set[8] = FALSE;
state_is_set[9] = FALSE;
state_is_set[10] = FALSE;
state_is_set[11] = FALSE;
state_is_set[12] = FALSE;
state_is_set[13] = FALSE;
state_is_set[14] = FALSE;
state_is_set[15] = FALSE;
state_is_set[16] = FALSE;
state_is_set[17] = FALSE;
state_is_set[18] = FALSE;
state_is_set[19] = FALSE;
state_is_set[20] = FALSE;
state_is_set[21] = FALSE;
/* transition [delete] */
src = prev_row_1[i-0][20];
dst = prev_row_0[i][20];
t = (ad->aas->gap_extend);
t += src[0];
if(state_is_set[20]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[20] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert] */
src = prev_row_0[i-1][19];
dst = prev_row_0[i][19];
t = (ad->aas->gap_extend);
t += src[0];
if(state_is_set[19]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[19] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to delete] */
src = prev_row_1[i-0][18];
dst = prev_row_0[i][20];
t = (ad->aas->gap_open);
t += src[0];
if(state_is_set[20]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[20] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to insert] */
src = prev_row_0[i-1][18];
dst = prev_row_0[i][19];
t = (ad->aas->gap_open);
t += src[0];
if(state_is_set[19]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[19] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
if(!(soi && SubOpt_Index_is_blocked_fast(soi, i))){
/* transition [match] */
src = prev_row_1[i-1][18];
dst = prev_row_0[i][18];
t = ((ud->query->annotation)
&& (region->query_start+i-1 >= ud->query->annotation->cds_start)
&& (region->query_start+i-1 < (ud->query->annotation->cds_start
          +ud->query->annotation->cds_length)))
?MATCH_IMPOSSIBLY_LOW_SCORE
:Submat_lookup(ud->mas->dna_submat,
     Sequence_get_symbol(ud->query, region->query_start+i-1),
     Sequence_get_symbol(ud->target, region->target_start+j-1));
t += src[0];
if(state_is_set[18]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
}
/* transition [target intron loop forward] */
src = prev_row_1[i-0][21];
dst = prev_row_0[i][21];
t = src[0];
if(state_is_set[21]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[21] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete] */
src = prev_row_1[i-0][16];
dst = prev_row_0[i][16];
t = (ad->aas->gap_extend);
t += src[0];
if(state_is_set[16]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[16] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert] */
src = prev_row_0[i-1][15];
dst = prev_row_0[i][15];
t = (ad->aas->gap_extend);
t += src[0];
if(state_is_set[15]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[15] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to delete] */
src = prev_row_1[i-0][14];
dst = prev_row_0[i][16];
t = (ad->aas->gap_open);
t += src[0];
if(state_is_set[16]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[16] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to insert] */
src = prev_row_0[i-1][14];
dst = prev_row_0[i][15];
t = (ad->aas->gap_open);
t += src[0];
if(state_is_set[15]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[15] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
if(!(soi && SubOpt_Index_is_blocked_fast(soi, i))){
/* transition [match] */
src = prev_row_1[i-1][14];
dst = prev_row_0[i][14];
t = ((ud->query->annotation)
&& (region->query_start+i-1 >= ud->query->annotation->cds_start)
&& (region->query_start+i-1 < (ud->query->annotation->cds_start
          +ud->query->annotation->cds_length)))
?MATCH_IMPOSSIBLY_LOW_SCORE
:Submat_lookup(ud->mas->dna_submat,
     Sequence_get_symbol(ud->query, region->query_start+i-1),
     Sequence_get_symbol(ud->target, region->target_start+j-1));
t += src[0];
if(state_is_set[14]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
}
/* transition [target intron loop forward] */
src = prev_row_1[i-0][17];
dst = prev_row_0[i][17];
t = src[0];
if(state_is_set[17]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[17] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift open 1 target] */
src = prev_row_1[i-0][2];
dst = prev_row_0[i][6];
t = (fd->fas->frameshift_penalty);
t += src[0];
if(state_is_set[6]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[6] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift open 2 query] */
src = prev_row_0[i-2][2];
dst = prev_row_0[i][5];
t = (fd->fas->frameshift_penalty);
t += src[0];
if(state_is_set[5]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[5] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift open 1 query] */
src = prev_row_0[i-1][2];
dst = prev_row_0[i][5];
t = (fd->fas->frameshift_penalty);
t += src[0];
if(state_is_set[5]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[5] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [target intron loop 2:1 phase target intron -T] */
src = prev_row_1[i-0][13];
dst = prev_row_0[i][13];
t = src[0];
if(state_is_set[13]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[13] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [target intron loop 1:2 phase target intron -T] */
src = prev_row_1[i-0][12];
dst = prev_row_0[i][12];
t = src[0];
if(state_is_set[12]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[12] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [target intron loop 0:0 phase target intron -T] */
src = prev_row_1[i-0][11];
dst = prev_row_0[i][11];
t = src[0];
if(state_is_set[11]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[11] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [phase2post phase target intron -T to (END)] */
src = prev_row_1[i-1][10];
dst = prev_row_0[i][2];
id->target_data->curr_intron_start = (gint)src[1];
t = (((region->query_start+i-1 >= 2) && (id->target_data->curr_intron_start >= 2)))?ud->match_list[4]->split_score_func(
ud->match_list[4], ud->query, ud->target,
region->query_start+i-1-2, region->query_start+i-1-1, region->query_start+i-1, (id->target_data->curr_intron_start - 2), (id->target_data->curr_intron_start - 1), region->target_start+j-1):-987654321;
t += src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [(START) to phase1pre phase target intron -T] */
src = prev_row_1[i-1][2];
dst = prev_row_0[i][7];
t = src[0];
if(state_is_set[7]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[7] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [start to match] */
src = prev_row_0[i-0][0];
dst = prev_row_0[i][14];
t = 0;
if(state_is_set[14]){
if(dst[0] < t){
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete to match] */
src = prev_row_0[i-0][16];
dst = prev_row_0[i][14];
t = src[0];
if(state_is_set[14]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert to match] */
src = prev_row_0[i-0][15];
dst = prev_row_0[i][14];
t = src[0];
if(state_is_set[14]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to end] */
src = prev_row_0[i-0][14];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert to match] */
src = prev_row_0[i-0][3];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete to match] */
src = prev_row_0[i-0][4];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [start to match] */
src = prev_row_0[i-0][0];
dst = prev_row_0[i][2];
t = 0;
if(state_is_set[2]){
if(dst[0] < t){
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift close 0 target] */
src = prev_row_0[i-0][6];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift close 0 query] */
src = prev_row_0[i-0][5];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [start to match] */
src = prev_row_0[i-0][2];
dst = prev_row_0[i][18];
t = src[0];
if(state_is_set[18]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete to match] */
src = prev_row_0[i-0][20];
dst = prev_row_0[i][18];
t = src[0];
if(state_is_set[18]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert to match] */
src = prev_row_0[i-0][19];
dst = prev_row_0[i][18];
t = src[0];
if(state_is_set[18]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to end] */
src = prev_row_0[i-0][18];
dst = prev_row_0[i][1];
t = src[0];
if(state_is_set[1]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[1] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to end] */
src = prev_row_0[i-0][2];
dst = prev_row_0[i][1];
t = src[0];
if(state_is_set[1]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[1] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
if(state_is_set[1]){
if(end_is_set){
if(score < prev_row_0[i][1][0]){
score = prev_row_0[i][1][0];
vd->curr_query_end = i;
vd->curr_target_end = j;
vd->curr_query_start = prev_row_0[i][1][vd->vr->region_start_query_id];
vd->curr_target_start = prev_row_0[i][1][vd->vr->region_start_target_id];
}
} else {
score = prev_row_0[i][1][0];
end_is_set = TRUE;
vd->curr_query_end = i;
vd->curr_target_end = j;
vd->curr_query_start = prev_row_0[i][1][vd->vr->region_start_query_id];
vd->curr_target_start = prev_row_0[i][1][vd->vr->region_start_target_id];
}
}
if(j == region->target_length){
if(i == region->query_length){
goto end_of_dp;
}
} else {
if(i == region->query_length){
goto row_2;
}
}
/* last cell [3] */
do {
/* CELL [1,3] */
i++;
prev_row_0[i][0][0] = -987654321;
prev_row_0[i][1][0] = -987654321;
prev_row_0[i][2][0] = -987654321;
prev_row_0[i][3][0] = -987654321;
prev_row_0[i][4][0] = -987654321;
prev_row_0[i][5][0] = -987654321;
prev_row_0[i][6][0] = -987654321;
prev_row_0[i][7][0] = -987654321;
prev_row_0[i][8][0] = -987654321;
prev_row_0[i][9][0] = -987654321;
prev_row_0[i][10][0] = -987654321;
prev_row_0[i][11][0] = -987654321;
prev_row_0[i][12][0] = -987654321;
prev_row_0[i][13][0] = -987654321;
prev_row_0[i][14][0] = -987654321;
prev_row_0[i][15][0] = -987654321;
prev_row_0[i][16][0] = -987654321;
prev_row_0[i][17][0] = -987654321;
prev_row_0[i][18][0] = -987654321;
prev_row_0[i][19][0] = -987654321;
prev_row_0[i][20][0] = -987654321;
prev_row_0[i][21][0] = -987654321;
state_is_set[0] = FALSE;
state_is_set[1] = FALSE;
state_is_set[2] = FALSE;
state_is_set[3] = FALSE;
state_is_set[4] = FALSE;
state_is_set[5] = FALSE;
state_is_set[6] = FALSE;
state_is_set[7] = FALSE;
state_is_set[8] = FALSE;
state_is_set[9] = FALSE;
state_is_set[10] = FALSE;
state_is_set[11] = FALSE;
state_is_set[12] = FALSE;
state_is_set[13] = FALSE;
state_is_set[14] = FALSE;
state_is_set[15] = FALSE;
state_is_set[16] = FALSE;
state_is_set[17] = FALSE;
state_is_set[18] = FALSE;
state_is_set[19] = FALSE;
state_is_set[20] = FALSE;
state_is_set[21] = FALSE;
/* transition [delete] */
src = prev_row_1[i-0][20];
dst = prev_row_0[i][20];
t = (ad->aas->gap_extend);
t += src[0];
if(state_is_set[20]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[20] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert] */
src = prev_row_0[i-1][19];
dst = prev_row_0[i][19];
t = (ad->aas->gap_extend);
t += src[0];
if(state_is_set[19]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[19] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to delete] */
src = prev_row_1[i-0][18];
dst = prev_row_0[i][20];
t = (ad->aas->gap_open);
t += src[0];
if(state_is_set[20]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[20] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to insert] */
src = prev_row_0[i-1][18];
dst = prev_row_0[i][19];
t = (ad->aas->gap_open);
t += src[0];
if(state_is_set[19]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[19] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
if(!(soi && SubOpt_Index_is_blocked_fast(soi, i))){
/* transition [match] */
src = prev_row_1[i-1][18];
dst = prev_row_0[i][18];
t = ((ud->query->annotation)
&& (region->query_start+i-1 >= ud->query->annotation->cds_start)
&& (region->query_start+i-1 < (ud->query->annotation->cds_start
          +ud->query->annotation->cds_length)))
?MATCH_IMPOSSIBLY_LOW_SCORE
:Submat_lookup(ud->mas->dna_submat,
     Sequence_get_symbol(ud->query, region->query_start+i-1),
     Sequence_get_symbol(ud->target, region->target_start+j-1));
t += src[0];
if(state_is_set[18]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
}
/* transition [target intron loop forward] */
src = prev_row_1[i-0][21];
dst = prev_row_0[i][21];
t = src[0];
if(state_is_set[21]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[21] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete] */
src = prev_row_1[i-0][16];
dst = prev_row_0[i][16];
t = (ad->aas->gap_extend);
t += src[0];
if(state_is_set[16]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[16] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert] */
src = prev_row_0[i-1][15];
dst = prev_row_0[i][15];
t = (ad->aas->gap_extend);
t += src[0];
if(state_is_set[15]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[15] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to delete] */
src = prev_row_1[i-0][14];
dst = prev_row_0[i][16];
t = (ad->aas->gap_open);
t += src[0];
if(state_is_set[16]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[16] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to insert] */
src = prev_row_0[i-1][14];
dst = prev_row_0[i][15];
t = (ad->aas->gap_open);
t += src[0];
if(state_is_set[15]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[15] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
if(!(soi && SubOpt_Index_is_blocked_fast(soi, i))){
/* transition [match] */
src = prev_row_1[i-1][14];
dst = prev_row_0[i][14];
t = ((ud->query->annotation)
&& (region->query_start+i-1 >= ud->query->annotation->cds_start)
&& (region->query_start+i-1 < (ud->query->annotation->cds_start
          +ud->query->annotation->cds_length)))
?MATCH_IMPOSSIBLY_LOW_SCORE
:Submat_lookup(ud->mas->dna_submat,
     Sequence_get_symbol(ud->query, region->query_start+i-1),
     Sequence_get_symbol(ud->target, region->target_start+j-1));
t += src[0];
if(state_is_set[14]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
}
/* transition [target intron loop forward] */
src = prev_row_1[i-0][17];
dst = prev_row_0[i][17];
t = src[0];
if(state_is_set[17]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[17] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift open 1 target] */
src = prev_row_1[i-0][2];
dst = prev_row_0[i][6];
t = (fd->fas->frameshift_penalty);
t += src[0];
if(state_is_set[6]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[6] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift close 3 query] */
src = prev_row_0[i-3][5];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift open 2 query] */
src = prev_row_0[i-2][2];
dst = prev_row_0[i][5];
t = (fd->fas->frameshift_penalty);
t += src[0];
if(state_is_set[5]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[5] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift open 1 query] */
src = prev_row_0[i-1][2];
dst = prev_row_0[i][5];
t = (fd->fas->frameshift_penalty);
t += src[0];
if(state_is_set[5]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[5] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to insert] */
src = prev_row_0[i-3][2];
dst = prev_row_0[i][3];
t = (ad->aas->codon_gap_open);
t += src[0];
if(state_is_set[3]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[3] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert] */
src = prev_row_0[i-3][3];
dst = prev_row_0[i][3];
t = (ad->aas->codon_gap_extend);
t += src[0];
if(state_is_set[3]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[3] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [target intron loop 2:1 phase target intron -T] */
src = prev_row_1[i-0][13];
dst = prev_row_0[i][13];
t = src[0];
if(state_is_set[13]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[13] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [target intron loop 1:2 phase target intron -T] */
src = prev_row_1[i-0][12];
dst = prev_row_0[i][12];
t = src[0];
if(state_is_set[12]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[12] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [target intron loop 0:0 phase target intron -T] */
src = prev_row_1[i-0][11];
dst = prev_row_0[i][11];
t = src[0];
if(state_is_set[11]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[11] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [phase2post phase target intron -T to (END)] */
src = prev_row_1[i-1][10];
dst = prev_row_0[i][2];
id->target_data->curr_intron_start = (gint)src[1];
t = (((region->query_start+i-1 >= 2) && (id->target_data->curr_intron_start >= 2)))?ud->match_list[4]->split_score_func(
ud->match_list[4], ud->query, ud->target,
region->query_start+i-1-2, region->query_start+i-1-1, region->query_start+i-1, (id->target_data->curr_intron_start - 2), (id->target_data->curr_intron_start - 1), region->target_start+j-1):-987654321;
t += src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [(START) to phase1pre phase target intron -T] */
src = prev_row_1[i-1][2];
dst = prev_row_0[i][7];
t = src[0];
if(state_is_set[7]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[7] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [start to match] */
src = prev_row_0[i-0][0];
dst = prev_row_0[i][14];
t = 0;
if(state_is_set[14]){
if(dst[0] < t){
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete to match] */
src = prev_row_0[i-0][16];
dst = prev_row_0[i][14];
t = src[0];
if(state_is_set[14]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert to match] */
src = prev_row_0[i-0][15];
dst = prev_row_0[i][14];
t = src[0];
if(state_is_set[14]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to end] */
src = prev_row_0[i-0][14];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert to match] */
src = prev_row_0[i-0][3];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete to match] */
src = prev_row_0[i-0][4];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [start to match] */
src = prev_row_0[i-0][0];
dst = prev_row_0[i][2];
t = 0;
if(state_is_set[2]){
if(dst[0] < t){
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift close 0 target] */
src = prev_row_0[i-0][6];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift close 0 query] */
src = prev_row_0[i-0][5];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [start to match] */
src = prev_row_0[i-0][2];
dst = prev_row_0[i][18];
t = src[0];
if(state_is_set[18]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete to match] */
src = prev_row_0[i-0][20];
dst = prev_row_0[i][18];
t = src[0];
if(state_is_set[18]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert to match] */
src = prev_row_0[i-0][19];
dst = prev_row_0[i][18];
t = src[0];
if(state_is_set[18]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to end] */
src = prev_row_0[i-0][18];
dst = prev_row_0[i][1];
t = src[0];
if(state_is_set[1]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[1] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to end] */
src = prev_row_0[i-0][2];
dst = prev_row_0[i][1];
t = src[0];
if(state_is_set[1]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[1] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
if(state_is_set[1]){
if(end_is_set){
if(score < prev_row_0[i][1][0]){
score = prev_row_0[i][1][0];
vd->curr_query_end = i;
vd->curr_target_end = j;
vd->curr_query_start = prev_row_0[i][1][vd->vr->region_start_query_id];
vd->curr_target_start = prev_row_0[i][1][vd->vr->region_start_target_id];
}
} else {
score = prev_row_0[i][1][0];
end_is_set = TRUE;
vd->curr_query_end = i;
vd->curr_target_end = j;
vd->curr_query_start = prev_row_0[i][1][vd->vr->region_start_query_id];
vd->curr_target_start = prev_row_0[i][1][vd->vr->region_start_target_id];
}
}
if(j == region->target_length){
if(i == region->query_length){
goto end_of_dp;
}
} else {
if(i == region->query_length){
goto row_2;
}
}
} while(TRUE);
/* explicit row [2] */
row_2:
j++;
SubOpt_Index_set_row(soi, j);
swap_row = prev_row_3;
prev_row_3 = prev_row_2;
prev_row_2 = prev_row_1;
prev_row_1 = prev_row_0;
prev_row_0 = swap_row;
/* start block */
i = -1;
/* explicit cell [0] */
/* CELL [2,0] */
i++;
prev_row_0[i][0][0] = -987654321;
prev_row_0[i][1][0] = -987654321;
prev_row_0[i][2][0] = -987654321;
prev_row_0[i][3][0] = -987654321;
prev_row_0[i][4][0] = -987654321;
prev_row_0[i][5][0] = -987654321;
prev_row_0[i][6][0] = -987654321;
prev_row_0[i][7][0] = -987654321;
prev_row_0[i][8][0] = -987654321;
prev_row_0[i][9][0] = -987654321;
prev_row_0[i][10][0] = -987654321;
prev_row_0[i][11][0] = -987654321;
prev_row_0[i][12][0] = -987654321;
prev_row_0[i][13][0] = -987654321;
prev_row_0[i][14][0] = -987654321;
prev_row_0[i][15][0] = -987654321;
prev_row_0[i][16][0] = -987654321;
prev_row_0[i][17][0] = -987654321;
prev_row_0[i][18][0] = -987654321;
prev_row_0[i][19][0] = -987654321;
prev_row_0[i][20][0] = -987654321;
prev_row_0[i][21][0] = -987654321;
state_is_set[0] = FALSE;
state_is_set[1] = FALSE;
state_is_set[2] = FALSE;
state_is_set[3] = FALSE;
state_is_set[4] = FALSE;
state_is_set[5] = FALSE;
state_is_set[6] = FALSE;
state_is_set[7] = FALSE;
state_is_set[8] = FALSE;
state_is_set[9] = FALSE;
state_is_set[10] = FALSE;
state_is_set[11] = FALSE;
state_is_set[12] = FALSE;
state_is_set[13] = FALSE;
state_is_set[14] = FALSE;
state_is_set[15] = FALSE;
state_is_set[16] = FALSE;
state_is_set[17] = FALSE;
state_is_set[18] = FALSE;
state_is_set[19] = FALSE;
state_is_set[20] = FALSE;
state_is_set[21] = FALSE;
/* transition [delete] */
src = prev_row_1[i-0][20];
dst = prev_row_0[i][20];
t = (ad->aas->gap_extend);
t += src[0];
if(state_is_set[20]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[20] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to delete] */
src = prev_row_1[i-0][18];
dst = prev_row_0[i][20];
t = (ad->aas->gap_open);
t += src[0];
if(state_is_set[20]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[20] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [intron forward to (END)] */
src = prev_row_2[i-0][21];
dst = prev_row_0[i][18];
id->target_data->curr_intron_start = (gint)src[1];
t = ((((region->target_start+j-2 - id->target_data->curr_intron_start + 2) < id->ias->min_intron)||((region->target_start+j-2 - id->target_data->curr_intron_start + 2) > id->ias->max_intron))?C4_IMPOSSIBLY_LOW_SCORE:SplicePrediction_get(id->target_data->sps->ss3_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[18]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [target intron loop forward] */
src = prev_row_1[i-0][21];
dst = prev_row_0[i][21];
t = src[0];
if(state_is_set[21]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[21] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [(START) to intron forward] */
src = prev_row_2[i-0][18];
dst = prev_row_0[i][21];
t = (id->ias->intron_open_penalty + SplicePrediction_get(id->target_data->sps->ss5_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[21]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[21] = TRUE;
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete] */
src = prev_row_1[i-0][16];
dst = prev_row_0[i][16];
t = (ad->aas->gap_extend);
t += src[0];
if(state_is_set[16]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[16] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to delete] */
src = prev_row_1[i-0][14];
dst = prev_row_0[i][16];
t = (ad->aas->gap_open);
t += src[0];
if(state_is_set[16]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[16] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [intron forward to (END)] */
src = prev_row_2[i-0][17];
dst = prev_row_0[i][14];
id->target_data->curr_intron_start = (gint)src[1];
t = ((((region->target_start+j-2 - id->target_data->curr_intron_start + 2) < id->ias->min_intron)||((region->target_start+j-2 - id->target_data->curr_intron_start + 2) > id->ias->max_intron))?C4_IMPOSSIBLY_LOW_SCORE:SplicePrediction_get(id->target_data->sps->ss3_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[14]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [target intron loop forward] */
src = prev_row_1[i-0][17];
dst = prev_row_0[i][17];
t = src[0];
if(state_is_set[17]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[17] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [(START) to intron forward] */
src = prev_row_2[i-0][14];
dst = prev_row_0[i][17];
t = (id->ias->intron_open_penalty + SplicePrediction_get(id->target_data->sps->ss5_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[17]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[17] = TRUE;
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift open 2 target] */
src = prev_row_2[i-0][2];
dst = prev_row_0[i][6];
t = (fd->fas->frameshift_penalty);
t += src[0];
if(state_is_set[6]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[6] = TRUE;
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift open 1 target] */
src = prev_row_1[i-0][2];
dst = prev_row_0[i][6];
t = (fd->fas->frameshift_penalty);
t += src[0];
if(state_is_set[6]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[6] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [(START) to intron 2:1 phase target intron -T] */
src = prev_row_2[i-0][9];
dst = prev_row_0[i][13];
t = (id->ias->intron_open_penalty + SplicePrediction_get(id->target_data->sps->ss5_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[13]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[13] = TRUE;
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [target intron loop 2:1 phase target intron -T] */
src = prev_row_1[i-0][13];
dst = prev_row_0[i][13];
t = src[0];
if(state_is_set[13]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[13] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [intron 2:1 phase target intron -T to (END)] */
src = prev_row_2[i-0][13];
dst = prev_row_0[i][10];
id->target_data->curr_intron_start = (gint)src[1];
t = ((((region->target_start+j-2 - id->target_data->curr_intron_start + 2) < id->ias->min_intron)||((region->target_start+j-2 - id->target_data->curr_intron_start + 2) > id->ias->max_intron))?C4_IMPOSSIBLY_LOW_SCORE:SplicePrediction_get(id->target_data->sps->ss3_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[10]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[10] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [(START) to intron 1:2 phase target intron -T] */
src = prev_row_2[i-0][7];
dst = prev_row_0[i][12];
t = (id->ias->intron_open_penalty + SplicePrediction_get(id->target_data->sps->ss5_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[12]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[12] = TRUE;
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [target intron loop 1:2 phase target intron -T] */
src = prev_row_1[i-0][12];
dst = prev_row_0[i][12];
t = src[0];
if(state_is_set[12]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[12] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [intron 1:2 phase target intron -T to (END)] */
src = prev_row_2[i-0][12];
dst = prev_row_0[i][8];
id->target_data->curr_intron_start = (gint)src[1];
t = ((((region->target_start+j-2 - id->target_data->curr_intron_start + 2) < id->ias->min_intron)||((region->target_start+j-2 - id->target_data->curr_intron_start + 2) > id->ias->max_intron))?C4_IMPOSSIBLY_LOW_SCORE:SplicePrediction_get(id->target_data->sps->ss3_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[8]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[8] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [(START) to intron 0:0 phase target intron -T] */
src = prev_row_2[i-0][2];
dst = prev_row_0[i][11];
t = (id->ias->intron_open_penalty + SplicePrediction_get(id->target_data->sps->ss5_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[11]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[11] = TRUE;
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [target intron loop 0:0 phase target intron -T] */
src = prev_row_1[i-0][11];
dst = prev_row_0[i][11];
t = src[0];
if(state_is_set[11]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[11] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [intron 0:0 phase target intron -T to (END)] */
src = prev_row_2[i-0][11];
dst = prev_row_0[i][2];
id->target_data->curr_intron_start = (gint)src[1];
t = ((((region->target_start+j-2 - id->target_data->curr_intron_start + 2) < id->ias->min_intron)||((region->target_start+j-2 - id->target_data->curr_intron_start + 2) > id->ias->max_intron))?C4_IMPOSSIBLY_LOW_SCORE:SplicePrediction_get(id->target_data->sps->ss3_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [start to match] */
src = prev_row_0[i-0][0];
dst = prev_row_0[i][14];
t = 0;
if(state_is_set[14]){
if(dst[0] < t){
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete to match] */
src = prev_row_0[i-0][16];
dst = prev_row_0[i][14];
t = src[0];
if(state_is_set[14]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert to match] */
src = prev_row_0[i-0][15];
dst = prev_row_0[i][14];
t = src[0];
if(state_is_set[14]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to end] */
src = prev_row_0[i-0][14];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert to match] */
src = prev_row_0[i-0][3];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete to match] */
src = prev_row_0[i-0][4];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [start to match] */
src = prev_row_0[i-0][0];
dst = prev_row_0[i][2];
t = 0;
if(state_is_set[2]){
if(dst[0] < t){
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift close 0 target] */
src = prev_row_0[i-0][6];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift close 0 query] */
src = prev_row_0[i-0][5];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [start to match] */
src = prev_row_0[i-0][2];
dst = prev_row_0[i][18];
t = src[0];
if(state_is_set[18]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete to match] */
src = prev_row_0[i-0][20];
dst = prev_row_0[i][18];
t = src[0];
if(state_is_set[18]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert to match] */
src = prev_row_0[i-0][19];
dst = prev_row_0[i][18];
t = src[0];
if(state_is_set[18]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to end] */
src = prev_row_0[i-0][18];
dst = prev_row_0[i][1];
t = src[0];
if(state_is_set[1]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[1] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to end] */
src = prev_row_0[i-0][2];
dst = prev_row_0[i][1];
t = src[0];
if(state_is_set[1]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[1] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
if(state_is_set[1]){
if(end_is_set){
if(score < prev_row_0[i][1][0]){
score = prev_row_0[i][1][0];
vd->curr_query_end = i;
vd->curr_target_end = j;
vd->curr_query_start = prev_row_0[i][1][vd->vr->region_start_query_id];
vd->curr_target_start = prev_row_0[i][1][vd->vr->region_start_target_id];
}
} else {
score = prev_row_0[i][1][0];
end_is_set = TRUE;
vd->curr_query_end = i;
vd->curr_target_end = j;
vd->curr_query_start = prev_row_0[i][1][vd->vr->region_start_query_id];
vd->curr_target_start = prev_row_0[i][1][vd->vr->region_start_target_id];
}
}
if(j == region->target_length){
if(i == region->query_length){
goto end_of_dp;
}
} else {
if(i == region->query_length){
goto nth_row;
}
}
/* explicit cell [1] */
/* CELL [2,1] */
i++;
prev_row_0[i][0][0] = -987654321;
prev_row_0[i][1][0] = -987654321;
prev_row_0[i][2][0] = -987654321;
prev_row_0[i][3][0] = -987654321;
prev_row_0[i][4][0] = -987654321;
prev_row_0[i][5][0] = -987654321;
prev_row_0[i][6][0] = -987654321;
prev_row_0[i][7][0] = -987654321;
prev_row_0[i][8][0] = -987654321;
prev_row_0[i][9][0] = -987654321;
prev_row_0[i][10][0] = -987654321;
prev_row_0[i][11][0] = -987654321;
prev_row_0[i][12][0] = -987654321;
prev_row_0[i][13][0] = -987654321;
prev_row_0[i][14][0] = -987654321;
prev_row_0[i][15][0] = -987654321;
prev_row_0[i][16][0] = -987654321;
prev_row_0[i][17][0] = -987654321;
prev_row_0[i][18][0] = -987654321;
prev_row_0[i][19][0] = -987654321;
prev_row_0[i][20][0] = -987654321;
prev_row_0[i][21][0] = -987654321;
state_is_set[0] = FALSE;
state_is_set[1] = FALSE;
state_is_set[2] = FALSE;
state_is_set[3] = FALSE;
state_is_set[4] = FALSE;
state_is_set[5] = FALSE;
state_is_set[6] = FALSE;
state_is_set[7] = FALSE;
state_is_set[8] = FALSE;
state_is_set[9] = FALSE;
state_is_set[10] = FALSE;
state_is_set[11] = FALSE;
state_is_set[12] = FALSE;
state_is_set[13] = FALSE;
state_is_set[14] = FALSE;
state_is_set[15] = FALSE;
state_is_set[16] = FALSE;
state_is_set[17] = FALSE;
state_is_set[18] = FALSE;
state_is_set[19] = FALSE;
state_is_set[20] = FALSE;
state_is_set[21] = FALSE;
/* transition [delete] */
src = prev_row_1[i-0][20];
dst = prev_row_0[i][20];
t = (ad->aas->gap_extend);
t += src[0];
if(state_is_set[20]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[20] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert] */
src = prev_row_0[i-1][19];
dst = prev_row_0[i][19];
t = (ad->aas->gap_extend);
t += src[0];
if(state_is_set[19]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[19] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to delete] */
src = prev_row_1[i-0][18];
dst = prev_row_0[i][20];
t = (ad->aas->gap_open);
t += src[0];
if(state_is_set[20]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[20] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to insert] */
src = prev_row_0[i-1][18];
dst = prev_row_0[i][19];
t = (ad->aas->gap_open);
t += src[0];
if(state_is_set[19]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[19] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
if(!(soi && SubOpt_Index_is_blocked_fast(soi, i))){
/* transition [match] */
src = prev_row_1[i-1][18];
dst = prev_row_0[i][18];
t = ((ud->query->annotation)
&& (region->query_start+i-1 >= ud->query->annotation->cds_start)
&& (region->query_start+i-1 < (ud->query->annotation->cds_start
          +ud->query->annotation->cds_length)))
?MATCH_IMPOSSIBLY_LOW_SCORE
:Submat_lookup(ud->mas->dna_submat,
     Sequence_get_symbol(ud->query, region->query_start+i-1),
     Sequence_get_symbol(ud->target, region->target_start+j-1));
t += src[0];
if(state_is_set[18]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
}
/* transition [intron forward to (END)] */
src = prev_row_2[i-0][21];
dst = prev_row_0[i][18];
id->target_data->curr_intron_start = (gint)src[1];
t = ((((region->target_start+j-2 - id->target_data->curr_intron_start + 2) < id->ias->min_intron)||((region->target_start+j-2 - id->target_data->curr_intron_start + 2) > id->ias->max_intron))?C4_IMPOSSIBLY_LOW_SCORE:SplicePrediction_get(id->target_data->sps->ss3_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[18]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [target intron loop forward] */
src = prev_row_1[i-0][21];
dst = prev_row_0[i][21];
t = src[0];
if(state_is_set[21]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[21] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [(START) to intron forward] */
src = prev_row_2[i-0][18];
dst = prev_row_0[i][21];
t = (id->ias->intron_open_penalty + SplicePrediction_get(id->target_data->sps->ss5_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[21]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[21] = TRUE;
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete] */
src = prev_row_1[i-0][16];
dst = prev_row_0[i][16];
t = (ad->aas->gap_extend);
t += src[0];
if(state_is_set[16]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[16] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert] */
src = prev_row_0[i-1][15];
dst = prev_row_0[i][15];
t = (ad->aas->gap_extend);
t += src[0];
if(state_is_set[15]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[15] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to delete] */
src = prev_row_1[i-0][14];
dst = prev_row_0[i][16];
t = (ad->aas->gap_open);
t += src[0];
if(state_is_set[16]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[16] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to insert] */
src = prev_row_0[i-1][14];
dst = prev_row_0[i][15];
t = (ad->aas->gap_open);
t += src[0];
if(state_is_set[15]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[15] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
if(!(soi && SubOpt_Index_is_blocked_fast(soi, i))){
/* transition [match] */
src = prev_row_1[i-1][14];
dst = prev_row_0[i][14];
t = ((ud->query->annotation)
&& (region->query_start+i-1 >= ud->query->annotation->cds_start)
&& (region->query_start+i-1 < (ud->query->annotation->cds_start
          +ud->query->annotation->cds_length)))
?MATCH_IMPOSSIBLY_LOW_SCORE
:Submat_lookup(ud->mas->dna_submat,
     Sequence_get_symbol(ud->query, region->query_start+i-1),
     Sequence_get_symbol(ud->target, region->target_start+j-1));
t += src[0];
if(state_is_set[14]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
}
/* transition [intron forward to (END)] */
src = prev_row_2[i-0][17];
dst = prev_row_0[i][14];
id->target_data->curr_intron_start = (gint)src[1];
t = ((((region->target_start+j-2 - id->target_data->curr_intron_start + 2) < id->ias->min_intron)||((region->target_start+j-2 - id->target_data->curr_intron_start + 2) > id->ias->max_intron))?C4_IMPOSSIBLY_LOW_SCORE:SplicePrediction_get(id->target_data->sps->ss3_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[14]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [target intron loop forward] */
src = prev_row_1[i-0][17];
dst = prev_row_0[i][17];
t = src[0];
if(state_is_set[17]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[17] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [(START) to intron forward] */
src = prev_row_2[i-0][14];
dst = prev_row_0[i][17];
t = (id->ias->intron_open_penalty + SplicePrediction_get(id->target_data->sps->ss5_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[17]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[17] = TRUE;
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift open 2 target] */
src = prev_row_2[i-0][2];
dst = prev_row_0[i][6];
t = (fd->fas->frameshift_penalty);
t += src[0];
if(state_is_set[6]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[6] = TRUE;
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift open 1 target] */
src = prev_row_1[i-0][2];
dst = prev_row_0[i][6];
t = (fd->fas->frameshift_penalty);
t += src[0];
if(state_is_set[6]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[6] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift open 1 query] */
src = prev_row_0[i-1][2];
dst = prev_row_0[i][5];
t = (fd->fas->frameshift_penalty);
t += src[0];
if(state_is_set[5]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[5] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [(START) to intron 2:1 phase target intron -T] */
src = prev_row_2[i-0][9];
dst = prev_row_0[i][13];
t = (id->ias->intron_open_penalty + SplicePrediction_get(id->target_data->sps->ss5_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[13]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[13] = TRUE;
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [target intron loop 2:1 phase target intron -T] */
src = prev_row_1[i-0][13];
dst = prev_row_0[i][13];
t = src[0];
if(state_is_set[13]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[13] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [intron 2:1 phase target intron -T to (END)] */
src = prev_row_2[i-0][13];
dst = prev_row_0[i][10];
id->target_data->curr_intron_start = (gint)src[1];
t = ((((region->target_start+j-2 - id->target_data->curr_intron_start + 2) < id->ias->min_intron)||((region->target_start+j-2 - id->target_data->curr_intron_start + 2) > id->ias->max_intron))?C4_IMPOSSIBLY_LOW_SCORE:SplicePrediction_get(id->target_data->sps->ss3_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[10]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[10] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [(START) to intron 1:2 phase target intron -T] */
src = prev_row_2[i-0][7];
dst = prev_row_0[i][12];
t = (id->ias->intron_open_penalty + SplicePrediction_get(id->target_data->sps->ss5_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[12]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[12] = TRUE;
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [target intron loop 1:2 phase target intron -T] */
src = prev_row_1[i-0][12];
dst = prev_row_0[i][12];
t = src[0];
if(state_is_set[12]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[12] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [intron 1:2 phase target intron -T to (END)] */
src = prev_row_2[i-0][12];
dst = prev_row_0[i][8];
id->target_data->curr_intron_start = (gint)src[1];
t = ((((region->target_start+j-2 - id->target_data->curr_intron_start + 2) < id->ias->min_intron)||((region->target_start+j-2 - id->target_data->curr_intron_start + 2) > id->ias->max_intron))?C4_IMPOSSIBLY_LOW_SCORE:SplicePrediction_get(id->target_data->sps->ss3_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[8]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[8] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [(START) to intron 0:0 phase target intron -T] */
src = prev_row_2[i-0][2];
dst = prev_row_0[i][11];
t = (id->ias->intron_open_penalty + SplicePrediction_get(id->target_data->sps->ss5_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[11]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[11] = TRUE;
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [target intron loop 0:0 phase target intron -T] */
src = prev_row_1[i-0][11];
dst = prev_row_0[i][11];
t = src[0];
if(state_is_set[11]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[11] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [intron 0:0 phase target intron -T to (END)] */
src = prev_row_2[i-0][11];
dst = prev_row_0[i][2];
id->target_data->curr_intron_start = (gint)src[1];
t = ((((region->target_start+j-2 - id->target_data->curr_intron_start + 2) < id->ias->min_intron)||((region->target_start+j-2 - id->target_data->curr_intron_start + 2) > id->ias->max_intron))?C4_IMPOSSIBLY_LOW_SCORE:SplicePrediction_get(id->target_data->sps->ss3_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [phase2post phase target intron -T to (END)] */
src = prev_row_1[i-1][10];
dst = prev_row_0[i][2];
id->target_data->curr_intron_start = (gint)src[1];
t = (((region->query_start+i-1 >= 2) && (id->target_data->curr_intron_start >= 2)))?ud->match_list[4]->split_score_func(
ud->match_list[4], ud->query, ud->target,
region->query_start+i-1-2, region->query_start+i-1-1, region->query_start+i-1, (id->target_data->curr_intron_start - 2), (id->target_data->curr_intron_start - 1), region->target_start+j-1):-987654321;
t += src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [(START) to phase1pre phase target intron -T] */
src = prev_row_1[i-1][2];
dst = prev_row_0[i][7];
t = src[0];
if(state_is_set[7]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[7] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [start to match] */
src = prev_row_0[i-0][0];
dst = prev_row_0[i][14];
t = 0;
if(state_is_set[14]){
if(dst[0] < t){
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete to match] */
src = prev_row_0[i-0][16];
dst = prev_row_0[i][14];
t = src[0];
if(state_is_set[14]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert to match] */
src = prev_row_0[i-0][15];
dst = prev_row_0[i][14];
t = src[0];
if(state_is_set[14]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to end] */
src = prev_row_0[i-0][14];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert to match] */
src = prev_row_0[i-0][3];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete to match] */
src = prev_row_0[i-0][4];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [start to match] */
src = prev_row_0[i-0][0];
dst = prev_row_0[i][2];
t = 0;
if(state_is_set[2]){
if(dst[0] < t){
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift close 0 target] */
src = prev_row_0[i-0][6];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift close 0 query] */
src = prev_row_0[i-0][5];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [start to match] */
src = prev_row_0[i-0][2];
dst = prev_row_0[i][18];
t = src[0];
if(state_is_set[18]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete to match] */
src = prev_row_0[i-0][20];
dst = prev_row_0[i][18];
t = src[0];
if(state_is_set[18]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert to match] */
src = prev_row_0[i-0][19];
dst = prev_row_0[i][18];
t = src[0];
if(state_is_set[18]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to end] */
src = prev_row_0[i-0][18];
dst = prev_row_0[i][1];
t = src[0];
if(state_is_set[1]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[1] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to end] */
src = prev_row_0[i-0][2];
dst = prev_row_0[i][1];
t = src[0];
if(state_is_set[1]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[1] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
if(state_is_set[1]){
if(end_is_set){
if(score < prev_row_0[i][1][0]){
score = prev_row_0[i][1][0];
vd->curr_query_end = i;
vd->curr_target_end = j;
vd->curr_query_start = prev_row_0[i][1][vd->vr->region_start_query_id];
vd->curr_target_start = prev_row_0[i][1][vd->vr->region_start_target_id];
}
} else {
score = prev_row_0[i][1][0];
end_is_set = TRUE;
vd->curr_query_end = i;
vd->curr_target_end = j;
vd->curr_query_start = prev_row_0[i][1][vd->vr->region_start_query_id];
vd->curr_target_start = prev_row_0[i][1][vd->vr->region_start_target_id];
}
}
if(j == region->target_length){
if(i == region->query_length){
goto end_of_dp;
}
} else {
if(i == region->query_length){
goto nth_row;
}
}
/* explicit cell [2] */
/* CELL [2,2] */
i++;
prev_row_0[i][0][0] = -987654321;
prev_row_0[i][1][0] = -987654321;
prev_row_0[i][2][0] = -987654321;
prev_row_0[i][3][0] = -987654321;
prev_row_0[i][4][0] = -987654321;
prev_row_0[i][5][0] = -987654321;
prev_row_0[i][6][0] = -987654321;
prev_row_0[i][7][0] = -987654321;
prev_row_0[i][8][0] = -987654321;
prev_row_0[i][9][0] = -987654321;
prev_row_0[i][10][0] = -987654321;
prev_row_0[i][11][0] = -987654321;
prev_row_0[i][12][0] = -987654321;
prev_row_0[i][13][0] = -987654321;
prev_row_0[i][14][0] = -987654321;
prev_row_0[i][15][0] = -987654321;
prev_row_0[i][16][0] = -987654321;
prev_row_0[i][17][0] = -987654321;
prev_row_0[i][18][0] = -987654321;
prev_row_0[i][19][0] = -987654321;
prev_row_0[i][20][0] = -987654321;
prev_row_0[i][21][0] = -987654321;
state_is_set[0] = FALSE;
state_is_set[1] = FALSE;
state_is_set[2] = FALSE;
state_is_set[3] = FALSE;
state_is_set[4] = FALSE;
state_is_set[5] = FALSE;
state_is_set[6] = FALSE;
state_is_set[7] = FALSE;
state_is_set[8] = FALSE;
state_is_set[9] = FALSE;
state_is_set[10] = FALSE;
state_is_set[11] = FALSE;
state_is_set[12] = FALSE;
state_is_set[13] = FALSE;
state_is_set[14] = FALSE;
state_is_set[15] = FALSE;
state_is_set[16] = FALSE;
state_is_set[17] = FALSE;
state_is_set[18] = FALSE;
state_is_set[19] = FALSE;
state_is_set[20] = FALSE;
state_is_set[21] = FALSE;
/* transition [delete] */
src = prev_row_1[i-0][20];
dst = prev_row_0[i][20];
t = (ad->aas->gap_extend);
t += src[0];
if(state_is_set[20]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[20] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert] */
src = prev_row_0[i-1][19];
dst = prev_row_0[i][19];
t = (ad->aas->gap_extend);
t += src[0];
if(state_is_set[19]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[19] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to delete] */
src = prev_row_1[i-0][18];
dst = prev_row_0[i][20];
t = (ad->aas->gap_open);
t += src[0];
if(state_is_set[20]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[20] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to insert] */
src = prev_row_0[i-1][18];
dst = prev_row_0[i][19];
t = (ad->aas->gap_open);
t += src[0];
if(state_is_set[19]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[19] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
if(!(soi && SubOpt_Index_is_blocked_fast(soi, i))){
/* transition [match] */
src = prev_row_1[i-1][18];
dst = prev_row_0[i][18];
t = ((ud->query->annotation)
&& (region->query_start+i-1 >= ud->query->annotation->cds_start)
&& (region->query_start+i-1 < (ud->query->annotation->cds_start
          +ud->query->annotation->cds_length)))
?MATCH_IMPOSSIBLY_LOW_SCORE
:Submat_lookup(ud->mas->dna_submat,
     Sequence_get_symbol(ud->query, region->query_start+i-1),
     Sequence_get_symbol(ud->target, region->target_start+j-1));
t += src[0];
if(state_is_set[18]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
}
/* transition [intron forward to (END)] */
src = prev_row_2[i-0][21];
dst = prev_row_0[i][18];
id->target_data->curr_intron_start = (gint)src[1];
t = ((((region->target_start+j-2 - id->target_data->curr_intron_start + 2) < id->ias->min_intron)||((region->target_start+j-2 - id->target_data->curr_intron_start + 2) > id->ias->max_intron))?C4_IMPOSSIBLY_LOW_SCORE:SplicePrediction_get(id->target_data->sps->ss3_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[18]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [target intron loop forward] */
src = prev_row_1[i-0][21];
dst = prev_row_0[i][21];
t = src[0];
if(state_is_set[21]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[21] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [(START) to intron forward] */
src = prev_row_2[i-0][18];
dst = prev_row_0[i][21];
t = (id->ias->intron_open_penalty + SplicePrediction_get(id->target_data->sps->ss5_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[21]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[21] = TRUE;
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete] */
src = prev_row_1[i-0][16];
dst = prev_row_0[i][16];
t = (ad->aas->gap_extend);
t += src[0];
if(state_is_set[16]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[16] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert] */
src = prev_row_0[i-1][15];
dst = prev_row_0[i][15];
t = (ad->aas->gap_extend);
t += src[0];
if(state_is_set[15]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[15] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to delete] */
src = prev_row_1[i-0][14];
dst = prev_row_0[i][16];
t = (ad->aas->gap_open);
t += src[0];
if(state_is_set[16]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[16] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to insert] */
src = prev_row_0[i-1][14];
dst = prev_row_0[i][15];
t = (ad->aas->gap_open);
t += src[0];
if(state_is_set[15]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[15] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
if(!(soi && SubOpt_Index_is_blocked_fast(soi, i))){
/* transition [match] */
src = prev_row_1[i-1][14];
dst = prev_row_0[i][14];
t = ((ud->query->annotation)
&& (region->query_start+i-1 >= ud->query->annotation->cds_start)
&& (region->query_start+i-1 < (ud->query->annotation->cds_start
          +ud->query->annotation->cds_length)))
?MATCH_IMPOSSIBLY_LOW_SCORE
:Submat_lookup(ud->mas->dna_submat,
     Sequence_get_symbol(ud->query, region->query_start+i-1),
     Sequence_get_symbol(ud->target, region->target_start+j-1));
t += src[0];
if(state_is_set[14]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
}
/* transition [intron forward to (END)] */
src = prev_row_2[i-0][17];
dst = prev_row_0[i][14];
id->target_data->curr_intron_start = (gint)src[1];
t = ((((region->target_start+j-2 - id->target_data->curr_intron_start + 2) < id->ias->min_intron)||((region->target_start+j-2 - id->target_data->curr_intron_start + 2) > id->ias->max_intron))?C4_IMPOSSIBLY_LOW_SCORE:SplicePrediction_get(id->target_data->sps->ss3_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[14]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [target intron loop forward] */
src = prev_row_1[i-0][17];
dst = prev_row_0[i][17];
t = src[0];
if(state_is_set[17]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[17] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [(START) to intron forward] */
src = prev_row_2[i-0][14];
dst = prev_row_0[i][17];
t = (id->ias->intron_open_penalty + SplicePrediction_get(id->target_data->sps->ss5_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[17]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[17] = TRUE;
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift open 2 target] */
src = prev_row_2[i-0][2];
dst = prev_row_0[i][6];
t = (fd->fas->frameshift_penalty);
t += src[0];
if(state_is_set[6]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[6] = TRUE;
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift open 1 target] */
src = prev_row_1[i-0][2];
dst = prev_row_0[i][6];
t = (fd->fas->frameshift_penalty);
t += src[0];
if(state_is_set[6]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[6] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift open 2 query] */
src = prev_row_0[i-2][2];
dst = prev_row_0[i][5];
t = (fd->fas->frameshift_penalty);
t += src[0];
if(state_is_set[5]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[5] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift open 1 query] */
src = prev_row_0[i-1][2];
dst = prev_row_0[i][5];
t = (fd->fas->frameshift_penalty);
t += src[0];
if(state_is_set[5]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[5] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [(START) to intron 2:1 phase target intron -T] */
src = prev_row_2[i-0][9];
dst = prev_row_0[i][13];
t = (id->ias->intron_open_penalty + SplicePrediction_get(id->target_data->sps->ss5_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[13]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[13] = TRUE;
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [target intron loop 2:1 phase target intron -T] */
src = prev_row_1[i-0][13];
dst = prev_row_0[i][13];
t = src[0];
if(state_is_set[13]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[13] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [intron 2:1 phase target intron -T to (END)] */
src = prev_row_2[i-0][13];
dst = prev_row_0[i][10];
id->target_data->curr_intron_start = (gint)src[1];
t = ((((region->target_start+j-2 - id->target_data->curr_intron_start + 2) < id->ias->min_intron)||((region->target_start+j-2 - id->target_data->curr_intron_start + 2) > id->ias->max_intron))?C4_IMPOSSIBLY_LOW_SCORE:SplicePrediction_get(id->target_data->sps->ss3_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[10]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[10] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [(START) to intron 1:2 phase target intron -T] */
src = prev_row_2[i-0][7];
dst = prev_row_0[i][12];
t = (id->ias->intron_open_penalty + SplicePrediction_get(id->target_data->sps->ss5_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[12]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[12] = TRUE;
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [target intron loop 1:2 phase target intron -T] */
src = prev_row_1[i-0][12];
dst = prev_row_0[i][12];
t = src[0];
if(state_is_set[12]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[12] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [intron 1:2 phase target intron -T to (END)] */
src = prev_row_2[i-0][12];
dst = prev_row_0[i][8];
id->target_data->curr_intron_start = (gint)src[1];
t = ((((region->target_start+j-2 - id->target_data->curr_intron_start + 2) < id->ias->min_intron)||((region->target_start+j-2 - id->target_data->curr_intron_start + 2) > id->ias->max_intron))?C4_IMPOSSIBLY_LOW_SCORE:SplicePrediction_get(id->target_data->sps->ss3_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[8]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[8] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [(START) to intron 0:0 phase target intron -T] */
src = prev_row_2[i-0][2];
dst = prev_row_0[i][11];
t = (id->ias->intron_open_penalty + SplicePrediction_get(id->target_data->sps->ss5_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[11]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[11] = TRUE;
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [target intron loop 0:0 phase target intron -T] */
src = prev_row_1[i-0][11];
dst = prev_row_0[i][11];
t = src[0];
if(state_is_set[11]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[11] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [intron 0:0 phase target intron -T to (END)] */
src = prev_row_2[i-0][11];
dst = prev_row_0[i][2];
id->target_data->curr_intron_start = (gint)src[1];
t = ((((region->target_start+j-2 - id->target_data->curr_intron_start + 2) < id->ias->min_intron)||((region->target_start+j-2 - id->target_data->curr_intron_start + 2) > id->ias->max_intron))?C4_IMPOSSIBLY_LOW_SCORE:SplicePrediction_get(id->target_data->sps->ss3_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [phase2post phase target intron -T to (END)] */
src = prev_row_1[i-1][10];
dst = prev_row_0[i][2];
id->target_data->curr_intron_start = (gint)src[1];
t = (((region->query_start+i-1 >= 2) && (id->target_data->curr_intron_start >= 2)))?ud->match_list[4]->split_score_func(
ud->match_list[4], ud->query, ud->target,
region->query_start+i-1-2, region->query_start+i-1-1, region->query_start+i-1, (id->target_data->curr_intron_start - 2), (id->target_data->curr_intron_start - 1), region->target_start+j-1):-987654321;
t += src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [phase1post phase target intron -T to (END)] */
src = prev_row_2[i-2][8];
dst = prev_row_0[i][2];
id->target_data->curr_intron_start = (gint)src[1];
t = (((region->query_start+i-2 >= 1) && (id->target_data->curr_intron_start >= 1)))?ud->match_list[4]->split_score_func(
ud->match_list[4], ud->query, ud->target,
region->query_start+i-2-1, region->query_start+i-2, region->query_start+i-2+1, (id->target_data->curr_intron_start - 1), region->target_start+j-2, region->target_start+j-2+1):-987654321;
t += src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [(START) to phase2pre phase target intron -T] */
src = prev_row_2[i-2][2];
dst = prev_row_0[i][9];
t = src[0];
if(state_is_set[9]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[9] = TRUE;
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [(START) to phase1pre phase target intron -T] */
src = prev_row_1[i-1][2];
dst = prev_row_0[i][7];
t = src[0];
if(state_is_set[7]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[7] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [start to match] */
src = prev_row_0[i-0][0];
dst = prev_row_0[i][14];
t = 0;
if(state_is_set[14]){
if(dst[0] < t){
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete to match] */
src = prev_row_0[i-0][16];
dst = prev_row_0[i][14];
t = src[0];
if(state_is_set[14]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert to match] */
src = prev_row_0[i-0][15];
dst = prev_row_0[i][14];
t = src[0];
if(state_is_set[14]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to end] */
src = prev_row_0[i-0][14];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert to match] */
src = prev_row_0[i-0][3];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete to match] */
src = prev_row_0[i-0][4];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [start to match] */
src = prev_row_0[i-0][0];
dst = prev_row_0[i][2];
t = 0;
if(state_is_set[2]){
if(dst[0] < t){
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift close 0 target] */
src = prev_row_0[i-0][6];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift close 0 query] */
src = prev_row_0[i-0][5];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [start to match] */
src = prev_row_0[i-0][2];
dst = prev_row_0[i][18];
t = src[0];
if(state_is_set[18]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete to match] */
src = prev_row_0[i-0][20];
dst = prev_row_0[i][18];
t = src[0];
if(state_is_set[18]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert to match] */
src = prev_row_0[i-0][19];
dst = prev_row_0[i][18];
t = src[0];
if(state_is_set[18]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to end] */
src = prev_row_0[i-0][18];
dst = prev_row_0[i][1];
t = src[0];
if(state_is_set[1]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[1] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to end] */
src = prev_row_0[i-0][2];
dst = prev_row_0[i][1];
t = src[0];
if(state_is_set[1]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[1] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
if(state_is_set[1]){
if(end_is_set){
if(score < prev_row_0[i][1][0]){
score = prev_row_0[i][1][0];
vd->curr_query_end = i;
vd->curr_target_end = j;
vd->curr_query_start = prev_row_0[i][1][vd->vr->region_start_query_id];
vd->curr_target_start = prev_row_0[i][1][vd->vr->region_start_target_id];
}
} else {
score = prev_row_0[i][1][0];
end_is_set = TRUE;
vd->curr_query_end = i;
vd->curr_target_end = j;
vd->curr_query_start = prev_row_0[i][1][vd->vr->region_start_query_id];
vd->curr_target_start = prev_row_0[i][1][vd->vr->region_start_target_id];
}
}
if(j == region->target_length){
if(i == region->query_length){
goto end_of_dp;
}
} else {
if(i == region->query_length){
goto nth_row;
}
}
/* last cell [3] */
do {
/* CELL [2,3] */
i++;
prev_row_0[i][0][0] = -987654321;
prev_row_0[i][1][0] = -987654321;
prev_row_0[i][2][0] = -987654321;
prev_row_0[i][3][0] = -987654321;
prev_row_0[i][4][0] = -987654321;
prev_row_0[i][5][0] = -987654321;
prev_row_0[i][6][0] = -987654321;
prev_row_0[i][7][0] = -987654321;
prev_row_0[i][8][0] = -987654321;
prev_row_0[i][9][0] = -987654321;
prev_row_0[i][10][0] = -987654321;
prev_row_0[i][11][0] = -987654321;
prev_row_0[i][12][0] = -987654321;
prev_row_0[i][13][0] = -987654321;
prev_row_0[i][14][0] = -987654321;
prev_row_0[i][15][0] = -987654321;
prev_row_0[i][16][0] = -987654321;
prev_row_0[i][17][0] = -987654321;
prev_row_0[i][18][0] = -987654321;
prev_row_0[i][19][0] = -987654321;
prev_row_0[i][20][0] = -987654321;
prev_row_0[i][21][0] = -987654321;
state_is_set[0] = FALSE;
state_is_set[1] = FALSE;
state_is_set[2] = FALSE;
state_is_set[3] = FALSE;
state_is_set[4] = FALSE;
state_is_set[5] = FALSE;
state_is_set[6] = FALSE;
state_is_set[7] = FALSE;
state_is_set[8] = FALSE;
state_is_set[9] = FALSE;
state_is_set[10] = FALSE;
state_is_set[11] = FALSE;
state_is_set[12] = FALSE;
state_is_set[13] = FALSE;
state_is_set[14] = FALSE;
state_is_set[15] = FALSE;
state_is_set[16] = FALSE;
state_is_set[17] = FALSE;
state_is_set[18] = FALSE;
state_is_set[19] = FALSE;
state_is_set[20] = FALSE;
state_is_set[21] = FALSE;
/* transition [delete] */
src = prev_row_1[i-0][20];
dst = prev_row_0[i][20];
t = (ad->aas->gap_extend);
t += src[0];
if(state_is_set[20]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[20] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert] */
src = prev_row_0[i-1][19];
dst = prev_row_0[i][19];
t = (ad->aas->gap_extend);
t += src[0];
if(state_is_set[19]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[19] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to delete] */
src = prev_row_1[i-0][18];
dst = prev_row_0[i][20];
t = (ad->aas->gap_open);
t += src[0];
if(state_is_set[20]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[20] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to insert] */
src = prev_row_0[i-1][18];
dst = prev_row_0[i][19];
t = (ad->aas->gap_open);
t += src[0];
if(state_is_set[19]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[19] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
if(!(soi && SubOpt_Index_is_blocked_fast(soi, i))){
/* transition [match] */
src = prev_row_1[i-1][18];
dst = prev_row_0[i][18];
t = ((ud->query->annotation)
&& (region->query_start+i-1 >= ud->query->annotation->cds_start)
&& (region->query_start+i-1 < (ud->query->annotation->cds_start
          +ud->query->annotation->cds_length)))
?MATCH_IMPOSSIBLY_LOW_SCORE
:Submat_lookup(ud->mas->dna_submat,
     Sequence_get_symbol(ud->query, region->query_start+i-1),
     Sequence_get_symbol(ud->target, region->target_start+j-1));
t += src[0];
if(state_is_set[18]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
}
/* transition [intron forward to (END)] */
src = prev_row_2[i-0][21];
dst = prev_row_0[i][18];
id->target_data->curr_intron_start = (gint)src[1];
t = ((((region->target_start+j-2 - id->target_data->curr_intron_start + 2) < id->ias->min_intron)||((region->target_start+j-2 - id->target_data->curr_intron_start + 2) > id->ias->max_intron))?C4_IMPOSSIBLY_LOW_SCORE:SplicePrediction_get(id->target_data->sps->ss3_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[18]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [target intron loop forward] */
src = prev_row_1[i-0][21];
dst = prev_row_0[i][21];
t = src[0];
if(state_is_set[21]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[21] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [(START) to intron forward] */
src = prev_row_2[i-0][18];
dst = prev_row_0[i][21];
t = (id->ias->intron_open_penalty + SplicePrediction_get(id->target_data->sps->ss5_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[21]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[21] = TRUE;
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete] */
src = prev_row_1[i-0][16];
dst = prev_row_0[i][16];
t = (ad->aas->gap_extend);
t += src[0];
if(state_is_set[16]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[16] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert] */
src = prev_row_0[i-1][15];
dst = prev_row_0[i][15];
t = (ad->aas->gap_extend);
t += src[0];
if(state_is_set[15]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[15] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to delete] */
src = prev_row_1[i-0][14];
dst = prev_row_0[i][16];
t = (ad->aas->gap_open);
t += src[0];
if(state_is_set[16]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[16] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to insert] */
src = prev_row_0[i-1][14];
dst = prev_row_0[i][15];
t = (ad->aas->gap_open);
t += src[0];
if(state_is_set[15]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[15] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
if(!(soi && SubOpt_Index_is_blocked_fast(soi, i))){
/* transition [match] */
src = prev_row_1[i-1][14];
dst = prev_row_0[i][14];
t = ((ud->query->annotation)
&& (region->query_start+i-1 >= ud->query->annotation->cds_start)
&& (region->query_start+i-1 < (ud->query->annotation->cds_start
          +ud->query->annotation->cds_length)))
?MATCH_IMPOSSIBLY_LOW_SCORE
:Submat_lookup(ud->mas->dna_submat,
     Sequence_get_symbol(ud->query, region->query_start+i-1),
     Sequence_get_symbol(ud->target, region->target_start+j-1));
t += src[0];
if(state_is_set[14]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
}
/* transition [intron forward to (END)] */
src = prev_row_2[i-0][17];
dst = prev_row_0[i][14];
id->target_data->curr_intron_start = (gint)src[1];
t = ((((region->target_start+j-2 - id->target_data->curr_intron_start + 2) < id->ias->min_intron)||((region->target_start+j-2 - id->target_data->curr_intron_start + 2) > id->ias->max_intron))?C4_IMPOSSIBLY_LOW_SCORE:SplicePrediction_get(id->target_data->sps->ss3_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[14]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [target intron loop forward] */
src = prev_row_1[i-0][17];
dst = prev_row_0[i][17];
t = src[0];
if(state_is_set[17]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[17] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [(START) to intron forward] */
src = prev_row_2[i-0][14];
dst = prev_row_0[i][17];
t = (id->ias->intron_open_penalty + SplicePrediction_get(id->target_data->sps->ss5_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[17]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[17] = TRUE;
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift open 2 target] */
src = prev_row_2[i-0][2];
dst = prev_row_0[i][6];
t = (fd->fas->frameshift_penalty);
t += src[0];
if(state_is_set[6]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[6] = TRUE;
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift open 1 target] */
src = prev_row_1[i-0][2];
dst = prev_row_0[i][6];
t = (fd->fas->frameshift_penalty);
t += src[0];
if(state_is_set[6]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[6] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift close 3 query] */
src = prev_row_0[i-3][5];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift open 2 query] */
src = prev_row_0[i-2][2];
dst = prev_row_0[i][5];
t = (fd->fas->frameshift_penalty);
t += src[0];
if(state_is_set[5]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[5] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift open 1 query] */
src = prev_row_0[i-1][2];
dst = prev_row_0[i][5];
t = (fd->fas->frameshift_penalty);
t += src[0];
if(state_is_set[5]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[5] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to insert] */
src = prev_row_0[i-3][2];
dst = prev_row_0[i][3];
t = (ad->aas->codon_gap_open);
t += src[0];
if(state_is_set[3]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[3] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert] */
src = prev_row_0[i-3][3];
dst = prev_row_0[i][3];
t = (ad->aas->codon_gap_extend);
t += src[0];
if(state_is_set[3]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[3] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [(START) to intron 2:1 phase target intron -T] */
src = prev_row_2[i-0][9];
dst = prev_row_0[i][13];
t = (id->ias->intron_open_penalty + SplicePrediction_get(id->target_data->sps->ss5_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[13]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[13] = TRUE;
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [target intron loop 2:1 phase target intron -T] */
src = prev_row_1[i-0][13];
dst = prev_row_0[i][13];
t = src[0];
if(state_is_set[13]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[13] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [intron 2:1 phase target intron -T to (END)] */
src = prev_row_2[i-0][13];
dst = prev_row_0[i][10];
id->target_data->curr_intron_start = (gint)src[1];
t = ((((region->target_start+j-2 - id->target_data->curr_intron_start + 2) < id->ias->min_intron)||((region->target_start+j-2 - id->target_data->curr_intron_start + 2) > id->ias->max_intron))?C4_IMPOSSIBLY_LOW_SCORE:SplicePrediction_get(id->target_data->sps->ss3_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[10]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[10] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [(START) to intron 1:2 phase target intron -T] */
src = prev_row_2[i-0][7];
dst = prev_row_0[i][12];
t = (id->ias->intron_open_penalty + SplicePrediction_get(id->target_data->sps->ss5_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[12]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[12] = TRUE;
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [target intron loop 1:2 phase target intron -T] */
src = prev_row_1[i-0][12];
dst = prev_row_0[i][12];
t = src[0];
if(state_is_set[12]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[12] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [intron 1:2 phase target intron -T to (END)] */
src = prev_row_2[i-0][12];
dst = prev_row_0[i][8];
id->target_data->curr_intron_start = (gint)src[1];
t = ((((region->target_start+j-2 - id->target_data->curr_intron_start + 2) < id->ias->min_intron)||((region->target_start+j-2 - id->target_data->curr_intron_start + 2) > id->ias->max_intron))?C4_IMPOSSIBLY_LOW_SCORE:SplicePrediction_get(id->target_data->sps->ss3_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[8]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[8] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [(START) to intron 0:0 phase target intron -T] */
src = prev_row_2[i-0][2];
dst = prev_row_0[i][11];
t = (id->ias->intron_open_penalty + SplicePrediction_get(id->target_data->sps->ss5_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[11]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[11] = TRUE;
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [target intron loop 0:0 phase target intron -T] */
src = prev_row_1[i-0][11];
dst = prev_row_0[i][11];
t = src[0];
if(state_is_set[11]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[11] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [intron 0:0 phase target intron -T to (END)] */
src = prev_row_2[i-0][11];
dst = prev_row_0[i][2];
id->target_data->curr_intron_start = (gint)src[1];
t = ((((region->target_start+j-2 - id->target_data->curr_intron_start + 2) < id->ias->min_intron)||((region->target_start+j-2 - id->target_data->curr_intron_start + 2) > id->ias->max_intron))?C4_IMPOSSIBLY_LOW_SCORE:SplicePrediction_get(id->target_data->sps->ss3_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [phase2post phase target intron -T to (END)] */
src = prev_row_1[i-1][10];
dst = prev_row_0[i][2];
id->target_data->curr_intron_start = (gint)src[1];
t = (((region->query_start+i-1 >= 2) && (id->target_data->curr_intron_start >= 2)))?ud->match_list[4]->split_score_func(
ud->match_list[4], ud->query, ud->target,
region->query_start+i-1-2, region->query_start+i-1-1, region->query_start+i-1, (id->target_data->curr_intron_start - 2), (id->target_data->curr_intron_start - 1), region->target_start+j-1):-987654321;
t += src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [phase1post phase target intron -T to (END)] */
src = prev_row_2[i-2][8];
dst = prev_row_0[i][2];
id->target_data->curr_intron_start = (gint)src[1];
t = (((region->query_start+i-2 >= 1) && (id->target_data->curr_intron_start >= 1)))?ud->match_list[4]->split_score_func(
ud->match_list[4], ud->query, ud->target,
region->query_start+i-2-1, region->query_start+i-2, region->query_start+i-2+1, (id->target_data->curr_intron_start - 1), region->target_start+j-2, region->target_start+j-2+1):-987654321;
t += src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [(START) to phase2pre phase target intron -T] */
src = prev_row_2[i-2][2];
dst = prev_row_0[i][9];
t = src[0];
if(state_is_set[9]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[9] = TRUE;
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [(START) to phase1pre phase target intron -T] */
src = prev_row_1[i-1][2];
dst = prev_row_0[i][7];
t = src[0];
if(state_is_set[7]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[7] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [start to match] */
src = prev_row_0[i-0][0];
dst = prev_row_0[i][14];
t = 0;
if(state_is_set[14]){
if(dst[0] < t){
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete to match] */
src = prev_row_0[i-0][16];
dst = prev_row_0[i][14];
t = src[0];
if(state_is_set[14]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert to match] */
src = prev_row_0[i-0][15];
dst = prev_row_0[i][14];
t = src[0];
if(state_is_set[14]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to end] */
src = prev_row_0[i-0][14];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert to match] */
src = prev_row_0[i-0][3];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete to match] */
src = prev_row_0[i-0][4];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [start to match] */
src = prev_row_0[i-0][0];
dst = prev_row_0[i][2];
t = 0;
if(state_is_set[2]){
if(dst[0] < t){
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift close 0 target] */
src = prev_row_0[i-0][6];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift close 0 query] */
src = prev_row_0[i-0][5];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [start to match] */
src = prev_row_0[i-0][2];
dst = prev_row_0[i][18];
t = src[0];
if(state_is_set[18]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete to match] */
src = prev_row_0[i-0][20];
dst = prev_row_0[i][18];
t = src[0];
if(state_is_set[18]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert to match] */
src = prev_row_0[i-0][19];
dst = prev_row_0[i][18];
t = src[0];
if(state_is_set[18]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to end] */
src = prev_row_0[i-0][18];
dst = prev_row_0[i][1];
t = src[0];
if(state_is_set[1]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[1] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to end] */
src = prev_row_0[i-0][2];
dst = prev_row_0[i][1];
t = src[0];
if(state_is_set[1]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[1] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
if(state_is_set[1]){
if(end_is_set){
if(score < prev_row_0[i][1][0]){
score = prev_row_0[i][1][0];
vd->curr_query_end = i;
vd->curr_target_end = j;
vd->curr_query_start = prev_row_0[i][1][vd->vr->region_start_query_id];
vd->curr_target_start = prev_row_0[i][1][vd->vr->region_start_target_id];
}
} else {
score = prev_row_0[i][1][0];
end_is_set = TRUE;
vd->curr_query_end = i;
vd->curr_target_end = j;
vd->curr_query_start = prev_row_0[i][1][vd->vr->region_start_query_id];
vd->curr_target_start = prev_row_0[i][1][vd->vr->region_start_target_id];
}
}
if(j == region->target_length){
if(i == region->query_length){
goto end_of_dp;
}
} else {
if(i == region->query_length){
goto nth_row;
}
}
} while(TRUE);
/* last row [3] */
do {
nth_row:
j++;
SubOpt_Index_set_row(soi, j);
swap_row = prev_row_3;
prev_row_3 = prev_row_2;
prev_row_2 = prev_row_1;
prev_row_1 = prev_row_0;
prev_row_0 = swap_row;
/* start block */
i = -1;
/* explicit cell [0] */
/* CELL [3,0] */
i++;
prev_row_0[i][0][0] = -987654321;
prev_row_0[i][1][0] = -987654321;
prev_row_0[i][2][0] = -987654321;
prev_row_0[i][3][0] = -987654321;
prev_row_0[i][4][0] = -987654321;
prev_row_0[i][5][0] = -987654321;
prev_row_0[i][6][0] = -987654321;
prev_row_0[i][7][0] = -987654321;
prev_row_0[i][8][0] = -987654321;
prev_row_0[i][9][0] = -987654321;
prev_row_0[i][10][0] = -987654321;
prev_row_0[i][11][0] = -987654321;
prev_row_0[i][12][0] = -987654321;
prev_row_0[i][13][0] = -987654321;
prev_row_0[i][14][0] = -987654321;
prev_row_0[i][15][0] = -987654321;
prev_row_0[i][16][0] = -987654321;
prev_row_0[i][17][0] = -987654321;
prev_row_0[i][18][0] = -987654321;
prev_row_0[i][19][0] = -987654321;
prev_row_0[i][20][0] = -987654321;
prev_row_0[i][21][0] = -987654321;
state_is_set[0] = FALSE;
state_is_set[1] = FALSE;
state_is_set[2] = FALSE;
state_is_set[3] = FALSE;
state_is_set[4] = FALSE;
state_is_set[5] = FALSE;
state_is_set[6] = FALSE;
state_is_set[7] = FALSE;
state_is_set[8] = FALSE;
state_is_set[9] = FALSE;
state_is_set[10] = FALSE;
state_is_set[11] = FALSE;
state_is_set[12] = FALSE;
state_is_set[13] = FALSE;
state_is_set[14] = FALSE;
state_is_set[15] = FALSE;
state_is_set[16] = FALSE;
state_is_set[17] = FALSE;
state_is_set[18] = FALSE;
state_is_set[19] = FALSE;
state_is_set[20] = FALSE;
state_is_set[21] = FALSE;
/* transition [delete] */
src = prev_row_1[i-0][20];
dst = prev_row_0[i][20];
t = (ad->aas->gap_extend);
t += src[0];
if(state_is_set[20]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[20] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to delete] */
src = prev_row_1[i-0][18];
dst = prev_row_0[i][20];
t = (ad->aas->gap_open);
t += src[0];
if(state_is_set[20]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[20] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [intron forward to (END)] */
src = prev_row_2[i-0][21];
dst = prev_row_0[i][18];
id->target_data->curr_intron_start = (gint)src[1];
t = ((((region->target_start+j-2 - id->target_data->curr_intron_start + 2) < id->ias->min_intron)||((region->target_start+j-2 - id->target_data->curr_intron_start + 2) > id->ias->max_intron))?C4_IMPOSSIBLY_LOW_SCORE:SplicePrediction_get(id->target_data->sps->ss3_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[18]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [target intron loop forward] */
src = prev_row_1[i-0][21];
dst = prev_row_0[i][21];
t = src[0];
if(state_is_set[21]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[21] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [(START) to intron forward] */
src = prev_row_2[i-0][18];
dst = prev_row_0[i][21];
t = (id->ias->intron_open_penalty + SplicePrediction_get(id->target_data->sps->ss5_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[21]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[21] = TRUE;
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete] */
src = prev_row_1[i-0][16];
dst = prev_row_0[i][16];
t = (ad->aas->gap_extend);
t += src[0];
if(state_is_set[16]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[16] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to delete] */
src = prev_row_1[i-0][14];
dst = prev_row_0[i][16];
t = (ad->aas->gap_open);
t += src[0];
if(state_is_set[16]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[16] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [intron forward to (END)] */
src = prev_row_2[i-0][17];
dst = prev_row_0[i][14];
id->target_data->curr_intron_start = (gint)src[1];
t = ((((region->target_start+j-2 - id->target_data->curr_intron_start + 2) < id->ias->min_intron)||((region->target_start+j-2 - id->target_data->curr_intron_start + 2) > id->ias->max_intron))?C4_IMPOSSIBLY_LOW_SCORE:SplicePrediction_get(id->target_data->sps->ss3_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[14]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [target intron loop forward] */
src = prev_row_1[i-0][17];
dst = prev_row_0[i][17];
t = src[0];
if(state_is_set[17]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[17] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [(START) to intron forward] */
src = prev_row_2[i-0][14];
dst = prev_row_0[i][17];
t = (id->ias->intron_open_penalty + SplicePrediction_get(id->target_data->sps->ss5_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[17]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[17] = TRUE;
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift close 3 target] */
src = prev_row_3[i-0][6];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift open 2 target] */
src = prev_row_2[i-0][2];
dst = prev_row_0[i][6];
t = (fd->fas->frameshift_penalty);
t += src[0];
if(state_is_set[6]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[6] = TRUE;
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift open 1 target] */
src = prev_row_1[i-0][2];
dst = prev_row_0[i][6];
t = (fd->fas->frameshift_penalty);
t += src[0];
if(state_is_set[6]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[6] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to delete] */
src = prev_row_3[i-0][2];
dst = prev_row_0[i][4];
t = (ad->aas->codon_gap_open);
t += src[0];
if(state_is_set[4]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-3);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[4] = TRUE;
src[1] = (C4_Score)(region->target_start+j-3);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete] */
src = prev_row_3[i-0][4];
dst = prev_row_0[i][4];
t = (ad->aas->codon_gap_extend);
t += src[0];
if(state_is_set[4]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[4] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [(START) to intron 2:1 phase target intron -T] */
src = prev_row_2[i-0][9];
dst = prev_row_0[i][13];
t = (id->ias->intron_open_penalty + SplicePrediction_get(id->target_data->sps->ss5_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[13]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[13] = TRUE;
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [target intron loop 2:1 phase target intron -T] */
src = prev_row_1[i-0][13];
dst = prev_row_0[i][13];
t = src[0];
if(state_is_set[13]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[13] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [intron 2:1 phase target intron -T to (END)] */
src = prev_row_2[i-0][13];
dst = prev_row_0[i][10];
id->target_data->curr_intron_start = (gint)src[1];
t = ((((region->target_start+j-2 - id->target_data->curr_intron_start + 2) < id->ias->min_intron)||((region->target_start+j-2 - id->target_data->curr_intron_start + 2) > id->ias->max_intron))?C4_IMPOSSIBLY_LOW_SCORE:SplicePrediction_get(id->target_data->sps->ss3_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[10]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[10] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [(START) to intron 1:2 phase target intron -T] */
src = prev_row_2[i-0][7];
dst = prev_row_0[i][12];
t = (id->ias->intron_open_penalty + SplicePrediction_get(id->target_data->sps->ss5_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[12]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[12] = TRUE;
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [target intron loop 1:2 phase target intron -T] */
src = prev_row_1[i-0][12];
dst = prev_row_0[i][12];
t = src[0];
if(state_is_set[12]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[12] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [intron 1:2 phase target intron -T to (END)] */
src = prev_row_2[i-0][12];
dst = prev_row_0[i][8];
id->target_data->curr_intron_start = (gint)src[1];
t = ((((region->target_start+j-2 - id->target_data->curr_intron_start + 2) < id->ias->min_intron)||((region->target_start+j-2 - id->target_data->curr_intron_start + 2) > id->ias->max_intron))?C4_IMPOSSIBLY_LOW_SCORE:SplicePrediction_get(id->target_data->sps->ss3_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[8]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[8] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [(START) to intron 0:0 phase target intron -T] */
src = prev_row_2[i-0][2];
dst = prev_row_0[i][11];
t = (id->ias->intron_open_penalty + SplicePrediction_get(id->target_data->sps->ss5_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[11]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[11] = TRUE;
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [target intron loop 0:0 phase target intron -T] */
src = prev_row_1[i-0][11];
dst = prev_row_0[i][11];
t = src[0];
if(state_is_set[11]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[11] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [intron 0:0 phase target intron -T to (END)] */
src = prev_row_2[i-0][11];
dst = prev_row_0[i][2];
id->target_data->curr_intron_start = (gint)src[1];
t = ((((region->target_start+j-2 - id->target_data->curr_intron_start + 2) < id->ias->min_intron)||((region->target_start+j-2 - id->target_data->curr_intron_start + 2) > id->ias->max_intron))?C4_IMPOSSIBLY_LOW_SCORE:SplicePrediction_get(id->target_data->sps->ss3_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [start to match] */
src = prev_row_0[i-0][0];
dst = prev_row_0[i][14];
t = 0;
if(state_is_set[14]){
if(dst[0] < t){
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete to match] */
src = prev_row_0[i-0][16];
dst = prev_row_0[i][14];
t = src[0];
if(state_is_set[14]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert to match] */
src = prev_row_0[i-0][15];
dst = prev_row_0[i][14];
t = src[0];
if(state_is_set[14]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to end] */
src = prev_row_0[i-0][14];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert to match] */
src = prev_row_0[i-0][3];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete to match] */
src = prev_row_0[i-0][4];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [start to match] */
src = prev_row_0[i-0][0];
dst = prev_row_0[i][2];
t = 0;
if(state_is_set[2]){
if(dst[0] < t){
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift close 0 target] */
src = prev_row_0[i-0][6];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift close 0 query] */
src = prev_row_0[i-0][5];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [start to match] */
src = prev_row_0[i-0][2];
dst = prev_row_0[i][18];
t = src[0];
if(state_is_set[18]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete to match] */
src = prev_row_0[i-0][20];
dst = prev_row_0[i][18];
t = src[0];
if(state_is_set[18]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert to match] */
src = prev_row_0[i-0][19];
dst = prev_row_0[i][18];
t = src[0];
if(state_is_set[18]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to end] */
src = prev_row_0[i-0][18];
dst = prev_row_0[i][1];
t = src[0];
if(state_is_set[1]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[1] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to end] */
src = prev_row_0[i-0][2];
dst = prev_row_0[i][1];
t = src[0];
if(state_is_set[1]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[1] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
if(state_is_set[1]){
if(end_is_set){
if(score < prev_row_0[i][1][0]){
score = prev_row_0[i][1][0];
vd->curr_query_end = i;
vd->curr_target_end = j;
vd->curr_query_start = prev_row_0[i][1][vd->vr->region_start_query_id];
vd->curr_target_start = prev_row_0[i][1][vd->vr->region_start_target_id];
}
} else {
score = prev_row_0[i][1][0];
end_is_set = TRUE;
vd->curr_query_end = i;
vd->curr_target_end = j;
vd->curr_query_start = prev_row_0[i][1][vd->vr->region_start_query_id];
vd->curr_target_start = prev_row_0[i][1][vd->vr->region_start_target_id];
}
}
if(j == region->target_length){
if(i == region->query_length){
goto end_of_dp;
}
} else {
if(i == region->query_length){
goto nth_row;
}
}
/* explicit cell [1] */
/* CELL [3,1] */
i++;
prev_row_0[i][0][0] = -987654321;
prev_row_0[i][1][0] = -987654321;
prev_row_0[i][2][0] = -987654321;
prev_row_0[i][3][0] = -987654321;
prev_row_0[i][4][0] = -987654321;
prev_row_0[i][5][0] = -987654321;
prev_row_0[i][6][0] = -987654321;
prev_row_0[i][7][0] = -987654321;
prev_row_0[i][8][0] = -987654321;
prev_row_0[i][9][0] = -987654321;
prev_row_0[i][10][0] = -987654321;
prev_row_0[i][11][0] = -987654321;
prev_row_0[i][12][0] = -987654321;
prev_row_0[i][13][0] = -987654321;
prev_row_0[i][14][0] = -987654321;
prev_row_0[i][15][0] = -987654321;
prev_row_0[i][16][0] = -987654321;
prev_row_0[i][17][0] = -987654321;
prev_row_0[i][18][0] = -987654321;
prev_row_0[i][19][0] = -987654321;
prev_row_0[i][20][0] = -987654321;
prev_row_0[i][21][0] = -987654321;
state_is_set[0] = FALSE;
state_is_set[1] = FALSE;
state_is_set[2] = FALSE;
state_is_set[3] = FALSE;
state_is_set[4] = FALSE;
state_is_set[5] = FALSE;
state_is_set[6] = FALSE;
state_is_set[7] = FALSE;
state_is_set[8] = FALSE;
state_is_set[9] = FALSE;
state_is_set[10] = FALSE;
state_is_set[11] = FALSE;
state_is_set[12] = FALSE;
state_is_set[13] = FALSE;
state_is_set[14] = FALSE;
state_is_set[15] = FALSE;
state_is_set[16] = FALSE;
state_is_set[17] = FALSE;
state_is_set[18] = FALSE;
state_is_set[19] = FALSE;
state_is_set[20] = FALSE;
state_is_set[21] = FALSE;
/* transition [delete] */
src = prev_row_1[i-0][20];
dst = prev_row_0[i][20];
t = (ad->aas->gap_extend);
t += src[0];
if(state_is_set[20]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[20] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert] */
src = prev_row_0[i-1][19];
dst = prev_row_0[i][19];
t = (ad->aas->gap_extend);
t += src[0];
if(state_is_set[19]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[19] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to delete] */
src = prev_row_1[i-0][18];
dst = prev_row_0[i][20];
t = (ad->aas->gap_open);
t += src[0];
if(state_is_set[20]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[20] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to insert] */
src = prev_row_0[i-1][18];
dst = prev_row_0[i][19];
t = (ad->aas->gap_open);
t += src[0];
if(state_is_set[19]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[19] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
if(!(soi && SubOpt_Index_is_blocked_fast(soi, i))){
/* transition [match] */
src = prev_row_1[i-1][18];
dst = prev_row_0[i][18];
t = ((ud->query->annotation)
&& (region->query_start+i-1 >= ud->query->annotation->cds_start)
&& (region->query_start+i-1 < (ud->query->annotation->cds_start
          +ud->query->annotation->cds_length)))
?MATCH_IMPOSSIBLY_LOW_SCORE
:Submat_lookup(ud->mas->dna_submat,
     Sequence_get_symbol(ud->query, region->query_start+i-1),
     Sequence_get_symbol(ud->target, region->target_start+j-1));
t += src[0];
if(state_is_set[18]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
}
/* transition [intron forward to (END)] */
src = prev_row_2[i-0][21];
dst = prev_row_0[i][18];
id->target_data->curr_intron_start = (gint)src[1];
t = ((((region->target_start+j-2 - id->target_data->curr_intron_start + 2) < id->ias->min_intron)||((region->target_start+j-2 - id->target_data->curr_intron_start + 2) > id->ias->max_intron))?C4_IMPOSSIBLY_LOW_SCORE:SplicePrediction_get(id->target_data->sps->ss3_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[18]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [target intron loop forward] */
src = prev_row_1[i-0][21];
dst = prev_row_0[i][21];
t = src[0];
if(state_is_set[21]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[21] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [(START) to intron forward] */
src = prev_row_2[i-0][18];
dst = prev_row_0[i][21];
t = (id->ias->intron_open_penalty + SplicePrediction_get(id->target_data->sps->ss5_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[21]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[21] = TRUE;
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete] */
src = prev_row_1[i-0][16];
dst = prev_row_0[i][16];
t = (ad->aas->gap_extend);
t += src[0];
if(state_is_set[16]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[16] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert] */
src = prev_row_0[i-1][15];
dst = prev_row_0[i][15];
t = (ad->aas->gap_extend);
t += src[0];
if(state_is_set[15]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[15] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to delete] */
src = prev_row_1[i-0][14];
dst = prev_row_0[i][16];
t = (ad->aas->gap_open);
t += src[0];
if(state_is_set[16]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[16] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to insert] */
src = prev_row_0[i-1][14];
dst = prev_row_0[i][15];
t = (ad->aas->gap_open);
t += src[0];
if(state_is_set[15]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[15] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
if(!(soi && SubOpt_Index_is_blocked_fast(soi, i))){
/* transition [match] */
src = prev_row_1[i-1][14];
dst = prev_row_0[i][14];
t = ((ud->query->annotation)
&& (region->query_start+i-1 >= ud->query->annotation->cds_start)
&& (region->query_start+i-1 < (ud->query->annotation->cds_start
          +ud->query->annotation->cds_length)))
?MATCH_IMPOSSIBLY_LOW_SCORE
:Submat_lookup(ud->mas->dna_submat,
     Sequence_get_symbol(ud->query, region->query_start+i-1),
     Sequence_get_symbol(ud->target, region->target_start+j-1));
t += src[0];
if(state_is_set[14]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
}
/* transition [intron forward to (END)] */
src = prev_row_2[i-0][17];
dst = prev_row_0[i][14];
id->target_data->curr_intron_start = (gint)src[1];
t = ((((region->target_start+j-2 - id->target_data->curr_intron_start + 2) < id->ias->min_intron)||((region->target_start+j-2 - id->target_data->curr_intron_start + 2) > id->ias->max_intron))?C4_IMPOSSIBLY_LOW_SCORE:SplicePrediction_get(id->target_data->sps->ss3_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[14]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [target intron loop forward] */
src = prev_row_1[i-0][17];
dst = prev_row_0[i][17];
t = src[0];
if(state_is_set[17]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[17] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [(START) to intron forward] */
src = prev_row_2[i-0][14];
dst = prev_row_0[i][17];
t = (id->ias->intron_open_penalty + SplicePrediction_get(id->target_data->sps->ss5_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[17]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[17] = TRUE;
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift close 3 target] */
src = prev_row_3[i-0][6];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift open 2 target] */
src = prev_row_2[i-0][2];
dst = prev_row_0[i][6];
t = (fd->fas->frameshift_penalty);
t += src[0];
if(state_is_set[6]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[6] = TRUE;
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift open 1 target] */
src = prev_row_1[i-0][2];
dst = prev_row_0[i][6];
t = (fd->fas->frameshift_penalty);
t += src[0];
if(state_is_set[6]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[6] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift open 1 query] */
src = prev_row_0[i-1][2];
dst = prev_row_0[i][5];
t = (fd->fas->frameshift_penalty);
t += src[0];
if(state_is_set[5]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[5] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to delete] */
src = prev_row_3[i-0][2];
dst = prev_row_0[i][4];
t = (ad->aas->codon_gap_open);
t += src[0];
if(state_is_set[4]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-3);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[4] = TRUE;
src[1] = (C4_Score)(region->target_start+j-3);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete] */
src = prev_row_3[i-0][4];
dst = prev_row_0[i][4];
t = (ad->aas->codon_gap_extend);
t += src[0];
if(state_is_set[4]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[4] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [(START) to intron 2:1 phase target intron -T] */
src = prev_row_2[i-0][9];
dst = prev_row_0[i][13];
t = (id->ias->intron_open_penalty + SplicePrediction_get(id->target_data->sps->ss5_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[13]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[13] = TRUE;
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [target intron loop 2:1 phase target intron -T] */
src = prev_row_1[i-0][13];
dst = prev_row_0[i][13];
t = src[0];
if(state_is_set[13]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[13] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [intron 2:1 phase target intron -T to (END)] */
src = prev_row_2[i-0][13];
dst = prev_row_0[i][10];
id->target_data->curr_intron_start = (gint)src[1];
t = ((((region->target_start+j-2 - id->target_data->curr_intron_start + 2) < id->ias->min_intron)||((region->target_start+j-2 - id->target_data->curr_intron_start + 2) > id->ias->max_intron))?C4_IMPOSSIBLY_LOW_SCORE:SplicePrediction_get(id->target_data->sps->ss3_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[10]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[10] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [(START) to intron 1:2 phase target intron -T] */
src = prev_row_2[i-0][7];
dst = prev_row_0[i][12];
t = (id->ias->intron_open_penalty + SplicePrediction_get(id->target_data->sps->ss5_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[12]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[12] = TRUE;
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [target intron loop 1:2 phase target intron -T] */
src = prev_row_1[i-0][12];
dst = prev_row_0[i][12];
t = src[0];
if(state_is_set[12]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[12] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [intron 1:2 phase target intron -T to (END)] */
src = prev_row_2[i-0][12];
dst = prev_row_0[i][8];
id->target_data->curr_intron_start = (gint)src[1];
t = ((((region->target_start+j-2 - id->target_data->curr_intron_start + 2) < id->ias->min_intron)||((region->target_start+j-2 - id->target_data->curr_intron_start + 2) > id->ias->max_intron))?C4_IMPOSSIBLY_LOW_SCORE:SplicePrediction_get(id->target_data->sps->ss3_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[8]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[8] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [(START) to intron 0:0 phase target intron -T] */
src = prev_row_2[i-0][2];
dst = prev_row_0[i][11];
t = (id->ias->intron_open_penalty + SplicePrediction_get(id->target_data->sps->ss5_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[11]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[11] = TRUE;
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [target intron loop 0:0 phase target intron -T] */
src = prev_row_1[i-0][11];
dst = prev_row_0[i][11];
t = src[0];
if(state_is_set[11]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[11] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [intron 0:0 phase target intron -T to (END)] */
src = prev_row_2[i-0][11];
dst = prev_row_0[i][2];
id->target_data->curr_intron_start = (gint)src[1];
t = ((((region->target_start+j-2 - id->target_data->curr_intron_start + 2) < id->ias->min_intron)||((region->target_start+j-2 - id->target_data->curr_intron_start + 2) > id->ias->max_intron))?C4_IMPOSSIBLY_LOW_SCORE:SplicePrediction_get(id->target_data->sps->ss3_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [phase2post phase target intron -T to (END)] */
src = prev_row_1[i-1][10];
dst = prev_row_0[i][2];
id->target_data->curr_intron_start = (gint)src[1];
t = (((region->query_start+i-1 >= 2) && (id->target_data->curr_intron_start >= 2)))?ud->match_list[4]->split_score_func(
ud->match_list[4], ud->query, ud->target,
region->query_start+i-1-2, region->query_start+i-1-1, region->query_start+i-1, (id->target_data->curr_intron_start - 2), (id->target_data->curr_intron_start - 1), region->target_start+j-1):-987654321;
t += src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [(START) to phase1pre phase target intron -T] */
src = prev_row_1[i-1][2];
dst = prev_row_0[i][7];
t = src[0];
if(state_is_set[7]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[7] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [start to match] */
src = prev_row_0[i-0][0];
dst = prev_row_0[i][14];
t = 0;
if(state_is_set[14]){
if(dst[0] < t){
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete to match] */
src = prev_row_0[i-0][16];
dst = prev_row_0[i][14];
t = src[0];
if(state_is_set[14]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert to match] */
src = prev_row_0[i-0][15];
dst = prev_row_0[i][14];
t = src[0];
if(state_is_set[14]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to end] */
src = prev_row_0[i-0][14];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert to match] */
src = prev_row_0[i-0][3];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete to match] */
src = prev_row_0[i-0][4];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [start to match] */
src = prev_row_0[i-0][0];
dst = prev_row_0[i][2];
t = 0;
if(state_is_set[2]){
if(dst[0] < t){
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift close 0 target] */
src = prev_row_0[i-0][6];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift close 0 query] */
src = prev_row_0[i-0][5];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [start to match] */
src = prev_row_0[i-0][2];
dst = prev_row_0[i][18];
t = src[0];
if(state_is_set[18]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete to match] */
src = prev_row_0[i-0][20];
dst = prev_row_0[i][18];
t = src[0];
if(state_is_set[18]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert to match] */
src = prev_row_0[i-0][19];
dst = prev_row_0[i][18];
t = src[0];
if(state_is_set[18]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to end] */
src = prev_row_0[i-0][18];
dst = prev_row_0[i][1];
t = src[0];
if(state_is_set[1]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[1] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to end] */
src = prev_row_0[i-0][2];
dst = prev_row_0[i][1];
t = src[0];
if(state_is_set[1]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[1] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
if(state_is_set[1]){
if(end_is_set){
if(score < prev_row_0[i][1][0]){
score = prev_row_0[i][1][0];
vd->curr_query_end = i;
vd->curr_target_end = j;
vd->curr_query_start = prev_row_0[i][1][vd->vr->region_start_query_id];
vd->curr_target_start = prev_row_0[i][1][vd->vr->region_start_target_id];
}
} else {
score = prev_row_0[i][1][0];
end_is_set = TRUE;
vd->curr_query_end = i;
vd->curr_target_end = j;
vd->curr_query_start = prev_row_0[i][1][vd->vr->region_start_query_id];
vd->curr_target_start = prev_row_0[i][1][vd->vr->region_start_target_id];
}
}
if(j == region->target_length){
if(i == region->query_length){
goto end_of_dp;
}
} else {
if(i == region->query_length){
goto nth_row;
}
}
/* explicit cell [2] */
/* CELL [3,2] */
i++;
prev_row_0[i][0][0] = -987654321;
prev_row_0[i][1][0] = -987654321;
prev_row_0[i][2][0] = -987654321;
prev_row_0[i][3][0] = -987654321;
prev_row_0[i][4][0] = -987654321;
prev_row_0[i][5][0] = -987654321;
prev_row_0[i][6][0] = -987654321;
prev_row_0[i][7][0] = -987654321;
prev_row_0[i][8][0] = -987654321;
prev_row_0[i][9][0] = -987654321;
prev_row_0[i][10][0] = -987654321;
prev_row_0[i][11][0] = -987654321;
prev_row_0[i][12][0] = -987654321;
prev_row_0[i][13][0] = -987654321;
prev_row_0[i][14][0] = -987654321;
prev_row_0[i][15][0] = -987654321;
prev_row_0[i][16][0] = -987654321;
prev_row_0[i][17][0] = -987654321;
prev_row_0[i][18][0] = -987654321;
prev_row_0[i][19][0] = -987654321;
prev_row_0[i][20][0] = -987654321;
prev_row_0[i][21][0] = -987654321;
state_is_set[0] = FALSE;
state_is_set[1] = FALSE;
state_is_set[2] = FALSE;
state_is_set[3] = FALSE;
state_is_set[4] = FALSE;
state_is_set[5] = FALSE;
state_is_set[6] = FALSE;
state_is_set[7] = FALSE;
state_is_set[8] = FALSE;
state_is_set[9] = FALSE;
state_is_set[10] = FALSE;
state_is_set[11] = FALSE;
state_is_set[12] = FALSE;
state_is_set[13] = FALSE;
state_is_set[14] = FALSE;
state_is_set[15] = FALSE;
state_is_set[16] = FALSE;
state_is_set[17] = FALSE;
state_is_set[18] = FALSE;
state_is_set[19] = FALSE;
state_is_set[20] = FALSE;
state_is_set[21] = FALSE;
/* transition [delete] */
src = prev_row_1[i-0][20];
dst = prev_row_0[i][20];
t = (ad->aas->gap_extend);
t += src[0];
if(state_is_set[20]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[20] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert] */
src = prev_row_0[i-1][19];
dst = prev_row_0[i][19];
t = (ad->aas->gap_extend);
t += src[0];
if(state_is_set[19]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[19] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to delete] */
src = prev_row_1[i-0][18];
dst = prev_row_0[i][20];
t = (ad->aas->gap_open);
t += src[0];
if(state_is_set[20]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[20] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to insert] */
src = prev_row_0[i-1][18];
dst = prev_row_0[i][19];
t = (ad->aas->gap_open);
t += src[0];
if(state_is_set[19]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[19] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
if(!(soi && SubOpt_Index_is_blocked_fast(soi, i))){
/* transition [match] */
src = prev_row_1[i-1][18];
dst = prev_row_0[i][18];
t = ((ud->query->annotation)
&& (region->query_start+i-1 >= ud->query->annotation->cds_start)
&& (region->query_start+i-1 < (ud->query->annotation->cds_start
          +ud->query->annotation->cds_length)))
?MATCH_IMPOSSIBLY_LOW_SCORE
:Submat_lookup(ud->mas->dna_submat,
     Sequence_get_symbol(ud->query, region->query_start+i-1),
     Sequence_get_symbol(ud->target, region->target_start+j-1));
t += src[0];
if(state_is_set[18]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
}
/* transition [intron forward to (END)] */
src = prev_row_2[i-0][21];
dst = prev_row_0[i][18];
id->target_data->curr_intron_start = (gint)src[1];
t = ((((region->target_start+j-2 - id->target_data->curr_intron_start + 2) < id->ias->min_intron)||((region->target_start+j-2 - id->target_data->curr_intron_start + 2) > id->ias->max_intron))?C4_IMPOSSIBLY_LOW_SCORE:SplicePrediction_get(id->target_data->sps->ss3_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[18]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [target intron loop forward] */
src = prev_row_1[i-0][21];
dst = prev_row_0[i][21];
t = src[0];
if(state_is_set[21]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[21] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [(START) to intron forward] */
src = prev_row_2[i-0][18];
dst = prev_row_0[i][21];
t = (id->ias->intron_open_penalty + SplicePrediction_get(id->target_data->sps->ss5_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[21]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[21] = TRUE;
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete] */
src = prev_row_1[i-0][16];
dst = prev_row_0[i][16];
t = (ad->aas->gap_extend);
t += src[0];
if(state_is_set[16]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[16] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert] */
src = prev_row_0[i-1][15];
dst = prev_row_0[i][15];
t = (ad->aas->gap_extend);
t += src[0];
if(state_is_set[15]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[15] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to delete] */
src = prev_row_1[i-0][14];
dst = prev_row_0[i][16];
t = (ad->aas->gap_open);
t += src[0];
if(state_is_set[16]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[16] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to insert] */
src = prev_row_0[i-1][14];
dst = prev_row_0[i][15];
t = (ad->aas->gap_open);
t += src[0];
if(state_is_set[15]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[15] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
if(!(soi && SubOpt_Index_is_blocked_fast(soi, i))){
/* transition [match] */
src = prev_row_1[i-1][14];
dst = prev_row_0[i][14];
t = ((ud->query->annotation)
&& (region->query_start+i-1 >= ud->query->annotation->cds_start)
&& (region->query_start+i-1 < (ud->query->annotation->cds_start
          +ud->query->annotation->cds_length)))
?MATCH_IMPOSSIBLY_LOW_SCORE
:Submat_lookup(ud->mas->dna_submat,
     Sequence_get_symbol(ud->query, region->query_start+i-1),
     Sequence_get_symbol(ud->target, region->target_start+j-1));
t += src[0];
if(state_is_set[14]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
}
/* transition [intron forward to (END)] */
src = prev_row_2[i-0][17];
dst = prev_row_0[i][14];
id->target_data->curr_intron_start = (gint)src[1];
t = ((((region->target_start+j-2 - id->target_data->curr_intron_start + 2) < id->ias->min_intron)||((region->target_start+j-2 - id->target_data->curr_intron_start + 2) > id->ias->max_intron))?C4_IMPOSSIBLY_LOW_SCORE:SplicePrediction_get(id->target_data->sps->ss3_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[14]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [target intron loop forward] */
src = prev_row_1[i-0][17];
dst = prev_row_0[i][17];
t = src[0];
if(state_is_set[17]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[17] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [(START) to intron forward] */
src = prev_row_2[i-0][14];
dst = prev_row_0[i][17];
t = (id->ias->intron_open_penalty + SplicePrediction_get(id->target_data->sps->ss5_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[17]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[17] = TRUE;
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift close 3 target] */
src = prev_row_3[i-0][6];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift open 2 target] */
src = prev_row_2[i-0][2];
dst = prev_row_0[i][6];
t = (fd->fas->frameshift_penalty);
t += src[0];
if(state_is_set[6]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[6] = TRUE;
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift open 1 target] */
src = prev_row_1[i-0][2];
dst = prev_row_0[i][6];
t = (fd->fas->frameshift_penalty);
t += src[0];
if(state_is_set[6]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[6] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift open 2 query] */
src = prev_row_0[i-2][2];
dst = prev_row_0[i][5];
t = (fd->fas->frameshift_penalty);
t += src[0];
if(state_is_set[5]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[5] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift open 1 query] */
src = prev_row_0[i-1][2];
dst = prev_row_0[i][5];
t = (fd->fas->frameshift_penalty);
t += src[0];
if(state_is_set[5]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[5] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to delete] */
src = prev_row_3[i-0][2];
dst = prev_row_0[i][4];
t = (ad->aas->codon_gap_open);
t += src[0];
if(state_is_set[4]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-3);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[4] = TRUE;
src[1] = (C4_Score)(region->target_start+j-3);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete] */
src = prev_row_3[i-0][4];
dst = prev_row_0[i][4];
t = (ad->aas->codon_gap_extend);
t += src[0];
if(state_is_set[4]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[4] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [(START) to intron 2:1 phase target intron -T] */
src = prev_row_2[i-0][9];
dst = prev_row_0[i][13];
t = (id->ias->intron_open_penalty + SplicePrediction_get(id->target_data->sps->ss5_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[13]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[13] = TRUE;
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [target intron loop 2:1 phase target intron -T] */
src = prev_row_1[i-0][13];
dst = prev_row_0[i][13];
t = src[0];
if(state_is_set[13]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[13] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [intron 2:1 phase target intron -T to (END)] */
src = prev_row_2[i-0][13];
dst = prev_row_0[i][10];
id->target_data->curr_intron_start = (gint)src[1];
t = ((((region->target_start+j-2 - id->target_data->curr_intron_start + 2) < id->ias->min_intron)||((region->target_start+j-2 - id->target_data->curr_intron_start + 2) > id->ias->max_intron))?C4_IMPOSSIBLY_LOW_SCORE:SplicePrediction_get(id->target_data->sps->ss3_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[10]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[10] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [(START) to intron 1:2 phase target intron -T] */
src = prev_row_2[i-0][7];
dst = prev_row_0[i][12];
t = (id->ias->intron_open_penalty + SplicePrediction_get(id->target_data->sps->ss5_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[12]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[12] = TRUE;
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [target intron loop 1:2 phase target intron -T] */
src = prev_row_1[i-0][12];
dst = prev_row_0[i][12];
t = src[0];
if(state_is_set[12]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[12] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [intron 1:2 phase target intron -T to (END)] */
src = prev_row_2[i-0][12];
dst = prev_row_0[i][8];
id->target_data->curr_intron_start = (gint)src[1];
t = ((((region->target_start+j-2 - id->target_data->curr_intron_start + 2) < id->ias->min_intron)||((region->target_start+j-2 - id->target_data->curr_intron_start + 2) > id->ias->max_intron))?C4_IMPOSSIBLY_LOW_SCORE:SplicePrediction_get(id->target_data->sps->ss3_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[8]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[8] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [(START) to intron 0:0 phase target intron -T] */
src = prev_row_2[i-0][2];
dst = prev_row_0[i][11];
t = (id->ias->intron_open_penalty + SplicePrediction_get(id->target_data->sps->ss5_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[11]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[11] = TRUE;
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [target intron loop 0:0 phase target intron -T] */
src = prev_row_1[i-0][11];
dst = prev_row_0[i][11];
t = src[0];
if(state_is_set[11]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[11] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [intron 0:0 phase target intron -T to (END)] */
src = prev_row_2[i-0][11];
dst = prev_row_0[i][2];
id->target_data->curr_intron_start = (gint)src[1];
t = ((((region->target_start+j-2 - id->target_data->curr_intron_start + 2) < id->ias->min_intron)||((region->target_start+j-2 - id->target_data->curr_intron_start + 2) > id->ias->max_intron))?C4_IMPOSSIBLY_LOW_SCORE:SplicePrediction_get(id->target_data->sps->ss3_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [phase2post phase target intron -T to (END)] */
src = prev_row_1[i-1][10];
dst = prev_row_0[i][2];
id->target_data->curr_intron_start = (gint)src[1];
t = (((region->query_start+i-1 >= 2) && (id->target_data->curr_intron_start >= 2)))?ud->match_list[4]->split_score_func(
ud->match_list[4], ud->query, ud->target,
region->query_start+i-1-2, region->query_start+i-1-1, region->query_start+i-1, (id->target_data->curr_intron_start - 2), (id->target_data->curr_intron_start - 1), region->target_start+j-1):-987654321;
t += src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [phase1post phase target intron -T to (END)] */
src = prev_row_2[i-2][8];
dst = prev_row_0[i][2];
id->target_data->curr_intron_start = (gint)src[1];
t = (((region->query_start+i-2 >= 1) && (id->target_data->curr_intron_start >= 1)))?ud->match_list[4]->split_score_func(
ud->match_list[4], ud->query, ud->target,
region->query_start+i-2-1, region->query_start+i-2, region->query_start+i-2+1, (id->target_data->curr_intron_start - 1), region->target_start+j-2, region->target_start+j-2+1):-987654321;
t += src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [(START) to phase2pre phase target intron -T] */
src = prev_row_2[i-2][2];
dst = prev_row_0[i][9];
t = src[0];
if(state_is_set[9]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[9] = TRUE;
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [(START) to phase1pre phase target intron -T] */
src = prev_row_1[i-1][2];
dst = prev_row_0[i][7];
t = src[0];
if(state_is_set[7]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[7] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [start to match] */
src = prev_row_0[i-0][0];
dst = prev_row_0[i][14];
t = 0;
if(state_is_set[14]){
if(dst[0] < t){
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete to match] */
src = prev_row_0[i-0][16];
dst = prev_row_0[i][14];
t = src[0];
if(state_is_set[14]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert to match] */
src = prev_row_0[i-0][15];
dst = prev_row_0[i][14];
t = src[0];
if(state_is_set[14]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to end] */
src = prev_row_0[i-0][14];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert to match] */
src = prev_row_0[i-0][3];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete to match] */
src = prev_row_0[i-0][4];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [start to match] */
src = prev_row_0[i-0][0];
dst = prev_row_0[i][2];
t = 0;
if(state_is_set[2]){
if(dst[0] < t){
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift close 0 target] */
src = prev_row_0[i-0][6];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift close 0 query] */
src = prev_row_0[i-0][5];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [start to match] */
src = prev_row_0[i-0][2];
dst = prev_row_0[i][18];
t = src[0];
if(state_is_set[18]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete to match] */
src = prev_row_0[i-0][20];
dst = prev_row_0[i][18];
t = src[0];
if(state_is_set[18]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert to match] */
src = prev_row_0[i-0][19];
dst = prev_row_0[i][18];
t = src[0];
if(state_is_set[18]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to end] */
src = prev_row_0[i-0][18];
dst = prev_row_0[i][1];
t = src[0];
if(state_is_set[1]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[1] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to end] */
src = prev_row_0[i-0][2];
dst = prev_row_0[i][1];
t = src[0];
if(state_is_set[1]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[1] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
if(state_is_set[1]){
if(end_is_set){
if(score < prev_row_0[i][1][0]){
score = prev_row_0[i][1][0];
vd->curr_query_end = i;
vd->curr_target_end = j;
vd->curr_query_start = prev_row_0[i][1][vd->vr->region_start_query_id];
vd->curr_target_start = prev_row_0[i][1][vd->vr->region_start_target_id];
}
} else {
score = prev_row_0[i][1][0];
end_is_set = TRUE;
vd->curr_query_end = i;
vd->curr_target_end = j;
vd->curr_query_start = prev_row_0[i][1][vd->vr->region_start_query_id];
vd->curr_target_start = prev_row_0[i][1][vd->vr->region_start_target_id];
}
}
if(j == region->target_length){
if(i == region->query_length){
goto end_of_dp;
}
} else {
if(i == region->query_length){
goto nth_row;
}
}
/* last cell [3] */
do {
/* CELL [3,3] */
i++;
prev_row_0[i][0][0] = -987654321;
prev_row_0[i][1][0] = -987654321;
prev_row_0[i][2][0] = -987654321;
prev_row_0[i][3][0] = -987654321;
prev_row_0[i][4][0] = -987654321;
prev_row_0[i][5][0] = -987654321;
prev_row_0[i][6][0] = -987654321;
prev_row_0[i][7][0] = -987654321;
prev_row_0[i][8][0] = -987654321;
prev_row_0[i][9][0] = -987654321;
prev_row_0[i][10][0] = -987654321;
prev_row_0[i][11][0] = -987654321;
prev_row_0[i][12][0] = -987654321;
prev_row_0[i][13][0] = -987654321;
prev_row_0[i][14][0] = -987654321;
prev_row_0[i][15][0] = -987654321;
prev_row_0[i][16][0] = -987654321;
prev_row_0[i][17][0] = -987654321;
prev_row_0[i][18][0] = -987654321;
prev_row_0[i][19][0] = -987654321;
prev_row_0[i][20][0] = -987654321;
prev_row_0[i][21][0] = -987654321;
state_is_set[0] = FALSE;
state_is_set[1] = FALSE;
state_is_set[2] = FALSE;
state_is_set[3] = FALSE;
state_is_set[4] = FALSE;
state_is_set[5] = FALSE;
state_is_set[6] = FALSE;
state_is_set[7] = FALSE;
state_is_set[8] = FALSE;
state_is_set[9] = FALSE;
state_is_set[10] = FALSE;
state_is_set[11] = FALSE;
state_is_set[12] = FALSE;
state_is_set[13] = FALSE;
state_is_set[14] = FALSE;
state_is_set[15] = FALSE;
state_is_set[16] = FALSE;
state_is_set[17] = FALSE;
state_is_set[18] = FALSE;
state_is_set[19] = FALSE;
state_is_set[20] = FALSE;
state_is_set[21] = FALSE;
/* transition [delete] */
src = prev_row_1[i-0][20];
dst = prev_row_0[i][20];
t = (ad->aas->gap_extend);
t += src[0];
if(state_is_set[20]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[20] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert] */
src = prev_row_0[i-1][19];
dst = prev_row_0[i][19];
t = (ad->aas->gap_extend);
t += src[0];
if(state_is_set[19]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[19] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to delete] */
src = prev_row_1[i-0][18];
dst = prev_row_0[i][20];
t = (ad->aas->gap_open);
t += src[0];
if(state_is_set[20]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[20] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to insert] */
src = prev_row_0[i-1][18];
dst = prev_row_0[i][19];
t = (ad->aas->gap_open);
t += src[0];
if(state_is_set[19]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[19] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
if(!(soi && SubOpt_Index_is_blocked_fast(soi, i))){
/* transition [match] */
src = prev_row_1[i-1][18];
dst = prev_row_0[i][18];
t = ((ud->query->annotation)
&& (region->query_start+i-1 >= ud->query->annotation->cds_start)
&& (region->query_start+i-1 < (ud->query->annotation->cds_start
          +ud->query->annotation->cds_length)))
?MATCH_IMPOSSIBLY_LOW_SCORE
:Submat_lookup(ud->mas->dna_submat,
     Sequence_get_symbol(ud->query, region->query_start+i-1),
     Sequence_get_symbol(ud->target, region->target_start+j-1));
t += src[0];
if(state_is_set[18]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
}
/* transition [intron forward to (END)] */
src = prev_row_2[i-0][21];
dst = prev_row_0[i][18];
id->target_data->curr_intron_start = (gint)src[1];
t = ((((region->target_start+j-2 - id->target_data->curr_intron_start + 2) < id->ias->min_intron)||((region->target_start+j-2 - id->target_data->curr_intron_start + 2) > id->ias->max_intron))?C4_IMPOSSIBLY_LOW_SCORE:SplicePrediction_get(id->target_data->sps->ss3_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[18]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [target intron loop forward] */
src = prev_row_1[i-0][21];
dst = prev_row_0[i][21];
t = src[0];
if(state_is_set[21]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[21] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [(START) to intron forward] */
src = prev_row_2[i-0][18];
dst = prev_row_0[i][21];
t = (id->ias->intron_open_penalty + SplicePrediction_get(id->target_data->sps->ss5_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[21]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[21] = TRUE;
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete] */
src = prev_row_1[i-0][16];
dst = prev_row_0[i][16];
t = (ad->aas->gap_extend);
t += src[0];
if(state_is_set[16]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[16] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert] */
src = prev_row_0[i-1][15];
dst = prev_row_0[i][15];
t = (ad->aas->gap_extend);
t += src[0];
if(state_is_set[15]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[15] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to delete] */
src = prev_row_1[i-0][14];
dst = prev_row_0[i][16];
t = (ad->aas->gap_open);
t += src[0];
if(state_is_set[16]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[16] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to insert] */
src = prev_row_0[i-1][14];
dst = prev_row_0[i][15];
t = (ad->aas->gap_open);
t += src[0];
if(state_is_set[15]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[15] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
if(!(soi && SubOpt_Index_is_blocked_fast(soi, i))){
/* transition [match] */
src = prev_row_1[i-1][14];
dst = prev_row_0[i][14];
t = ((ud->query->annotation)
&& (region->query_start+i-1 >= ud->query->annotation->cds_start)
&& (region->query_start+i-1 < (ud->query->annotation->cds_start
          +ud->query->annotation->cds_length)))
?MATCH_IMPOSSIBLY_LOW_SCORE
:Submat_lookup(ud->mas->dna_submat,
     Sequence_get_symbol(ud->query, region->query_start+i-1),
     Sequence_get_symbol(ud->target, region->target_start+j-1));
t += src[0];
if(state_is_set[14]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
}
/* transition [intron forward to (END)] */
src = prev_row_2[i-0][17];
dst = prev_row_0[i][14];
id->target_data->curr_intron_start = (gint)src[1];
t = ((((region->target_start+j-2 - id->target_data->curr_intron_start + 2) < id->ias->min_intron)||((region->target_start+j-2 - id->target_data->curr_intron_start + 2) > id->ias->max_intron))?C4_IMPOSSIBLY_LOW_SCORE:SplicePrediction_get(id->target_data->sps->ss3_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[14]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [target intron loop forward] */
src = prev_row_1[i-0][17];
dst = prev_row_0[i][17];
t = src[0];
if(state_is_set[17]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[17] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [(START) to intron forward] */
src = prev_row_2[i-0][14];
dst = prev_row_0[i][17];
t = (id->ias->intron_open_penalty + SplicePrediction_get(id->target_data->sps->ss5_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[17]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[17] = TRUE;
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift close 3 target] */
src = prev_row_3[i-0][6];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift open 2 target] */
src = prev_row_2[i-0][2];
dst = prev_row_0[i][6];
t = (fd->fas->frameshift_penalty);
t += src[0];
if(state_is_set[6]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[6] = TRUE;
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift open 1 target] */
src = prev_row_1[i-0][2];
dst = prev_row_0[i][6];
t = (fd->fas->frameshift_penalty);
t += src[0];
if(state_is_set[6]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[6] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift close 3 query] */
src = prev_row_0[i-3][5];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift open 2 query] */
src = prev_row_0[i-2][2];
dst = prev_row_0[i][5];
t = (fd->fas->frameshift_penalty);
t += src[0];
if(state_is_set[5]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[5] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift open 1 query] */
src = prev_row_0[i-1][2];
dst = prev_row_0[i][5];
t = (fd->fas->frameshift_penalty);
t += src[0];
if(state_is_set[5]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[5] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
if(!(soi && SubOpt_Index_is_blocked_fast(soi, i))){
/* transition [match] */
src = prev_row_3[i-3][2];
dst = prev_row_0[i][2];
t = ((ud->query->annotation)
 && (ud->query->alphabet->type == Alphabet_Type_DNA)
 && (  ((region->query_start+i-3) < ud->query->annotation->cds_start)
    || ((region->query_start+i-3) >= (ud->query->annotation->cds_start
               +ud->query->annotation->cds_length))
    || (((region->query_start+i-3) % 3) != (ud->query->annotation->cds_start % 3))))
?MATCH_IMPOSSIBLY_LOW_SCORE
:Submat_lookup(ud->mas->protein_submat,
               Translate_base(ud->mas->translate, 
                  Sequence_get_symbol(ud->query, region->query_start+i-3),
                  Sequence_get_symbol(ud->query, region->query_start+i-3+1),
                  Sequence_get_symbol(ud->query, region->query_start+i-3+2)),
               Translate_base(ud->mas->translate, 
                  Sequence_get_symbol(ud->target, region->target_start+j-3),
                  Sequence_get_symbol(ud->target, region->target_start+j-3+1),
                  Sequence_get_symbol(ud->target, region->target_start+j-3+2)))
;
t += src[0];
if(state_is_set[2]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-3);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
src[1] = (C4_Score)(region->target_start+j-3);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
}
/* transition [match to insert] */
src = prev_row_0[i-3][2];
dst = prev_row_0[i][3];
t = (ad->aas->codon_gap_open);
t += src[0];
if(state_is_set[3]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[3] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to delete] */
src = prev_row_3[i-0][2];
dst = prev_row_0[i][4];
t = (ad->aas->codon_gap_open);
t += src[0];
if(state_is_set[4]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-3);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[4] = TRUE;
src[1] = (C4_Score)(region->target_start+j-3);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert] */
src = prev_row_0[i-3][3];
dst = prev_row_0[i][3];
t = (ad->aas->codon_gap_extend);
t += src[0];
if(state_is_set[3]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[3] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete] */
src = prev_row_3[i-0][4];
dst = prev_row_0[i][4];
t = (ad->aas->codon_gap_extend);
t += src[0];
if(state_is_set[4]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[4] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [(START) to intron 2:1 phase target intron -T] */
src = prev_row_2[i-0][9];
dst = prev_row_0[i][13];
t = (id->ias->intron_open_penalty + SplicePrediction_get(id->target_data->sps->ss5_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[13]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[13] = TRUE;
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [target intron loop 2:1 phase target intron -T] */
src = prev_row_1[i-0][13];
dst = prev_row_0[i][13];
t = src[0];
if(state_is_set[13]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[13] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [intron 2:1 phase target intron -T to (END)] */
src = prev_row_2[i-0][13];
dst = prev_row_0[i][10];
id->target_data->curr_intron_start = (gint)src[1];
t = ((((region->target_start+j-2 - id->target_data->curr_intron_start + 2) < id->ias->min_intron)||((region->target_start+j-2 - id->target_data->curr_intron_start + 2) > id->ias->max_intron))?C4_IMPOSSIBLY_LOW_SCORE:SplicePrediction_get(id->target_data->sps->ss3_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[10]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[10] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [(START) to intron 1:2 phase target intron -T] */
src = prev_row_2[i-0][7];
dst = prev_row_0[i][12];
t = (id->ias->intron_open_penalty + SplicePrediction_get(id->target_data->sps->ss5_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[12]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[12] = TRUE;
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [target intron loop 1:2 phase target intron -T] */
src = prev_row_1[i-0][12];
dst = prev_row_0[i][12];
t = src[0];
if(state_is_set[12]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[12] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [intron 1:2 phase target intron -T to (END)] */
src = prev_row_2[i-0][12];
dst = prev_row_0[i][8];
id->target_data->curr_intron_start = (gint)src[1];
t = ((((region->target_start+j-2 - id->target_data->curr_intron_start + 2) < id->ias->min_intron)||((region->target_start+j-2 - id->target_data->curr_intron_start + 2) > id->ias->max_intron))?C4_IMPOSSIBLY_LOW_SCORE:SplicePrediction_get(id->target_data->sps->ss3_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[8]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[8] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [(START) to intron 0:0 phase target intron -T] */
src = prev_row_2[i-0][2];
dst = prev_row_0[i][11];
t = (id->ias->intron_open_penalty + SplicePrediction_get(id->target_data->sps->ss5_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[11]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[11] = TRUE;
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [target intron loop 0:0 phase target intron -T] */
src = prev_row_1[i-0][11];
dst = prev_row_0[i][11];
t = src[0];
if(state_is_set[11]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[11] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [intron 0:0 phase target intron -T to (END)] */
src = prev_row_2[i-0][11];
dst = prev_row_0[i][2];
id->target_data->curr_intron_start = (gint)src[1];
t = ((((region->target_start+j-2 - id->target_data->curr_intron_start + 2) < id->ias->min_intron)||((region->target_start+j-2 - id->target_data->curr_intron_start + 2) > id->ias->max_intron))?C4_IMPOSSIBLY_LOW_SCORE:SplicePrediction_get(id->target_data->sps->ss3_forward, region->target_start+j-2));
t += src[0];
if(t < C4_IMPOSSIBLY_LOW_SCORE)
t = C4_IMPOSSIBLY_LOW_SCORE;
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [phase2post phase target intron -T to (END)] */
src = prev_row_1[i-1][10];
dst = prev_row_0[i][2];
id->target_data->curr_intron_start = (gint)src[1];
t = (((region->query_start+i-1 >= 2) && (id->target_data->curr_intron_start >= 2)))?ud->match_list[4]->split_score_func(
ud->match_list[4], ud->query, ud->target,
region->query_start+i-1-2, region->query_start+i-1-1, region->query_start+i-1, (id->target_data->curr_intron_start - 2), (id->target_data->curr_intron_start - 1), region->target_start+j-1):-987654321;
t += src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [phase1post phase target intron -T to (END)] */
src = prev_row_2[i-2][8];
dst = prev_row_0[i][2];
id->target_data->curr_intron_start = (gint)src[1];
t = (((region->query_start+i-2 >= 1) && (id->target_data->curr_intron_start >= 1)))?ud->match_list[4]->split_score_func(
ud->match_list[4], ud->query, ud->target,
region->query_start+i-2-1, region->query_start+i-2, region->query_start+i-2+1, (id->target_data->curr_intron_start - 1), region->target_start+j-2, region->target_start+j-2+1):-987654321;
t += src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [(START) to phase2pre phase target intron -T] */
src = prev_row_2[i-2][2];
dst = prev_row_0[i][9];
t = src[0];
if(state_is_set[9]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[9] = TRUE;
src[1] = (C4_Score)(region->target_start+j-2);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [(START) to phase1pre phase target intron -T] */
src = prev_row_1[i-1][2];
dst = prev_row_0[i][7];
t = src[0];
if(state_is_set[7]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[7] = TRUE;
src[1] = (C4_Score)(region->target_start+j-1);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [start to match] */
src = prev_row_0[i-0][0];
dst = prev_row_0[i][14];
t = 0;
if(state_is_set[14]){
if(dst[0] < t){
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete to match] */
src = prev_row_0[i-0][16];
dst = prev_row_0[i][14];
t = src[0];
if(state_is_set[14]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert to match] */
src = prev_row_0[i-0][15];
dst = prev_row_0[i][14];
t = src[0];
if(state_is_set[14]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[14] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to end] */
src = prev_row_0[i-0][14];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert to match] */
src = prev_row_0[i-0][3];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete to match] */
src = prev_row_0[i-0][4];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [start to match] */
src = prev_row_0[i-0][0];
dst = prev_row_0[i][2];
t = 0;
if(state_is_set[2]){
if(dst[0] < t){
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
src[vd->vr->region_start_query_id] = i-0;
src[vd->vr->region_start_target_id] = j-0;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift close 0 target] */
src = prev_row_0[i-0][6];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [frameshift close 0 query] */
src = prev_row_0[i-0][5];
dst = prev_row_0[i][2];
t = src[0];
if(state_is_set[2]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[2] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [start to match] */
src = prev_row_0[i-0][2];
dst = prev_row_0[i][18];
t = src[0];
if(state_is_set[18]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [delete to match] */
src = prev_row_0[i-0][20];
dst = prev_row_0[i][18];
t = src[0];
if(state_is_set[18]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [insert to match] */
src = prev_row_0[i-0][19];
dst = prev_row_0[i][18];
t = src[0];
if(state_is_set[18]){
if(dst[0] < t){
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[18] = TRUE;
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to end] */
src = prev_row_0[i-0][18];
dst = prev_row_0[i][1];
t = src[0];
if(state_is_set[1]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[1] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
/* transition [match to end] */
src = prev_row_0[i-0][2];
dst = prev_row_0[i][1];
t = src[0];
if(state_is_set[1]){
if(dst[0] < t){
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
} else {
state_is_set[1] = TRUE;
src[1] = (C4_Score)(region->target_start+j-0);
dst[0] = t;
dst[1] = src[1];
dst[2] = src[2];
dst[3] = src[3];
}
if(state_is_set[1]){
if(end_is_set){
if(score < prev_row_0[i][1][0]){
score = prev_row_0[i][1][0];
vd->curr_query_end = i;
vd->curr_target_end = j;
vd->curr_query_start = prev_row_0[i][1][vd->vr->region_start_query_id];
vd->curr_target_start = prev_row_0[i][1][vd->vr->region_start_target_id];
}
} else {
score = prev_row_0[i][1][0];
end_is_set = TRUE;
vd->curr_query_end = i;
vd->curr_target_end = j;
vd->curr_query_start = prev_row_0[i][1][vd->vr->region_start_query_id];
vd->curr_target_start = prev_row_0[i][1][vd->vr->region_start_target_id];
}
}
if(j == region->target_length){
if(i == region->query_length){
goto end_of_dp;
}
} else {
if(i == region->query_length){
goto nth_row;
}
}
} while(TRUE);
} while(TRUE);
end_of_dp:
/* Transition calc exit */
model->exit_func(region, user_data);
vd->alignment_region->query_start = vd->curr_query_start + region->query_start;
vd->alignment_region->target_start = vd->curr_target_start + region->target_start;
vd->alignment_region->query_length = vd->curr_query_end - vd->curr_query_start;
vd->alignment_region->target_length = vd->curr_target_end - vd->curr_target_start;
g_free(state_is_set);
return score;
}

/* ---< END >--- */

