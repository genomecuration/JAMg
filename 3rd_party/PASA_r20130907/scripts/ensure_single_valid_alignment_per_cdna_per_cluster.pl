...
foreach my $collection_aref (values %cluster_n_cdna_info) {
    ...
    if (scalar @alignments > 1) {
        ...
        foreach my $remaining_alignment (@alignments) {
            ...
            my $query = "update align_link set comment = comment + '; invalidating in preference for diff alignment at location', validate = 0 where align_id = " . $remaining_alignment->{align_id};
