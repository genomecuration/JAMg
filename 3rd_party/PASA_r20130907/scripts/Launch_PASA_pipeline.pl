...
my ($opt_c, $opt_C, $opt_R, $opt_A, $opt_g, $opt_t, $opt_f, $opt_T, $opt_u, $opt_d, $opt_h, $opt_x, $opt_s, $opt_e,
...
$ENV{MYSQLBIN} = &Pasa_conf::getParam("MYSQLBIN");
...
if ($RUN_PIPELINE && ! @PRIMARY_ALIGNERS) {
    die "Error, must specify --ALIGNERS when running the alignment assembly pipeline ";
}
        ...
        &process_cmd(
                     {
                         ...
                         params => "-M $mysql_db -S $ENV{PASAHOME}/schema/cdna_alignment_mysqlschema",
                         input => undef,
                         output => undef
                         }
                     );
...
if ($RUN_PIPELINE) {
    ...
    if ($TRANSDECODER) {
        ...
        my $transdecoder_gff3_file = "$transcript_db.transdecoder.gff3";        
...
if ($COMPARE_TO_ANNOT) {
    ...
    my $cmd = {
        ...
        params => "-G $genome_db -M $MYSQLstring $genetic_code_opt",
        input => undef,
