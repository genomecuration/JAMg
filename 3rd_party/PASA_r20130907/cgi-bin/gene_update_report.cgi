...
foreach my $result (@results) {
	...
	$assembly_text .= "</table>";
	print "<tr><td>$update_id</td><td>$gene_id<a href=\"gene_centric_image_generator.dbi?db=$db&compare_id=$compare_id&TU=$gene_id\">AllGeneUpd</a></td><td>$model_id</td><td>$assembly_text</td><td><img src=\"annot_update_image.cgi?db=$db&update_id=$update_id\" alt=update_image_$gene_id ></td></tr>\n";
    }
    if ($x > $stop) {
	last;
    }
