...
my (
     $fasta_file,       $reference_file, $blast_file,
     $coding_only,      $help,           $exclude_file,
     $is_protein,       $prep_only,      $no_refine,
     @aat_filter_files, $same_species,   $softmasked, @post_process_similarity_files,
     $separate,         @post_process_result_files, $not_exhaustive, $annotation_file
);
#variables
...
my ($cdbfasta_exec,$cdbyank_exec,$parafly_exec,$exonerate) = &check_program('cdbfasta','cdbyank','ParaFly','exonerate');
my ($makeblastdb_exec,$blastn_exec,$tblastn_exec) = &check_program('makeblastdb','blastn','tblastn');
...
if ( !$is_protein ) {
  ...
  unless ( -s "$fasta_file.orf" ) {
    ...
    &process_cmd("$getorf_exec $fasta_file $fasta_file.orf.u  -minsize $minorf -find 3");
  ...
  foreach my $id ( keys %queries ) {
   if ($queries{$id}{'length'} < $minaa){
    ...
    $queries{$id}{'seq'} .= '*' unless !$queries{$id}{'seq'} || $queries{$id}{'seq'} =~ /\*$/;
  }
}
...
if ($prep_only) {
  if ( $prep_only > 1 ) {
    ...
    system("split -d -l $file_lns -a 3 run_exonerate_commands.cmd run_exonerate_commands.cmd.");
...
print "Starting exonerate with $threads runs versus "
  ...
  . "_queries/\n";
&process_cmd("$parafly_exec -shuffle -CPU $threads -c run_exonerate_commands.cmd -failed_cmds run_exonerate_commands.cmd.failed -v") if -s "run_exonerate_commands.cmd";
...
sub parse_analyze_blast() {
  ...
  while ( my @data = split( "\t", <BLAST> ) ) {
    ...
    $queries{$query_id}{'lower_bound'} = $data[8] < $data[9] ? $data[8] : $data[9];
    $queries{$query_id}{'upper_bound'} = $data[8] < $data[8] ? $data[9] : $data[8];
...
sub parse_aat_filter() {
  ...
  $basename =~ /^(.+)\.aat.filter/;    #TODO GROSS ASSUMPTION no other way!!!
  my $ref = $1 || die;
...
/^\s*(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\S+)/;
    my (
         $dstart, $dend,  $score, $astart, $aend,
         $orient, $zero1, $zero2, $query
    ) = ( $1, $2, $3, $4, $5, $6, $7, $8, $9 );
    die unless $query;
...
sub separate_exonerate() {
  ...
  foreach my $target ( sort keys %reference_hash ) {
    ...
    my $scaffold_seq    = &get_id_seq_from_fasta($target,$reference_file);
    if ( !$scaffold_seq ) {
	...
	if (!$end){
      ...
      my $genome_region_length = length($scaffold_seq);
...
#      my $genome_region = $scaffold_obj->subseq( $start, $end );
      my $genome_region = substr($scaffold_seq, $start-1, $end-1);
      my $comp_q = $id;
      $comp_q =~ s/\W+/_/g;
      $comp_q =~ s/__+/_/g;
      $comp_q =~ s/_$//g;
...
sub process_exonerate() {
  ...
  foreach my $exonerate_file (@$exonerate_result_files) {
    ...
    $file_counter++;
    print STDERR "Processed $file_counter/$all_files_number files. Found $gene_counter genes....\r" if $file_counter=~/00$/;
  ...
  print STDERR "Processed $file_counter/$all_files_number files. Found $gene_counter genes....\n";
}
...
sub process_exonerate_similarity() {
  ...
  foreach my $exonerate_file (@$exonerate_result_files) {
    ...
    open( EXONERATE, $exonerate_file );
    $file_counter++;
    print STDERR "Processed $file_counter/$all_files_number files. Found $gene_counter genes....\r";    
    while ( my $ln = <EXONERATE> ) {
      next if $ln =~ /^\s*$/;
        my ( $gene_id, $gene_start, $gene_end );
        my $exon_counter   = 0;
