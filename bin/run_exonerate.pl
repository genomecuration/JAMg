...
if ($prep_only) {
	if ( $prep_only > 1 ) {
		...
		system(
			"rm -f run_exonerate_commands.cmd.? run_exonerate_commands.cmd.??");
		system(
...
### -D -M control memory usage!
		...
		if ($same_species) {
			$cmd = $coding_only
			  ...
			  ;                                         # removed --percent 70
			$cmd =
			  " -s 1000 -Q protein --exhaustive 1 -m protein2genome:bestfit "
...
" -s 100 -Q protein --exhaustive 1 -m protein2genome:bestfit --percent 30 "
		...
		print CMD (
			...
			"if [ ! -e $query_file.exonerate_results ]; then " 
			  . $common 
			  . $cmd
			  . " > $query_file.exonerate_results; fi\n"
		);
	}
	close(CMD);
}
...
sub separate_exonerate() {
	...
	foreach my $target ( sort keys %reference_hash ) {
		...
		if ( !$scaffold_seq ) {
			warn
			  "Cannot find reference sequence file for $target. Skipping...\n";
			next;
		}
				...
				while ( my $ln2 = <EXONERATE> ) {
					...
					if ( $data[8] ) {
						...
						if ( $data[2] eq 'gene' ) {
							...
							$exon_counter++;
							$data[8] =
"ID=$gene_id.exon.$exon_counter;Parent=$gene_id\n";
						}
						elsif ( $data[2] eq 'intron' ) {
							$data[8] =
"ID=$gene_id.intron.$intron_counter;Parent=$gene_id\n";
						}
						else {
