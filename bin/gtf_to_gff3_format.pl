...
my $usage = "usage: $0 file.GTF [genome.fasta] [source] > file.GFF3\n\n";

my $gtf_file = shift or die $usage;
...
main: {
    ...
    foreach my $asmbl_id (sort keys %$asmbl_id_to_gene_list_href) {
		...
		$gene_obj->{source} = $user_source if $user_source;
                
                my $gff3_text = $gene_obj->to_GFF3_format();
