...
my (
	...
	$transdecoder_assembly_file, $transdecoder_peptides,
	$mrna_file,                  $softmasked_genome,
	$stop_after_correction,      $norefine,
	$nodataprint
);
...
my ( $gff2gb_exec, $fathom_exec, $augustus_exec, $augustus_train_exec,
  ...
  = &check_program_optional(
	'gff2gbSmallDNA.pl', 'fathom', 'augustus', 'etraining',
	'filterGenes.pl'
  );
$augustus_dir =
...
# if it has already been printed, skip it. someitmes there is a bug and this creates non-unique IDs
			if ( $already_printed{$gene_id} ) {
				warn
				  "Warning: $gene_id was already printed before. skipping...\n";
				next;
			}
...
# by getting previous UTR line and adjust co-ordidates to by utr[end]+1 if + or utr[start]-1 if -.
					...
					$exon_counter++;
					$data[8] =
					  "ID=$gene_id.exon.$exon_counter;Parent=$mRNA_id\n";
				}
				elsif ( $data[2] eq 'intron' ) {
					$intron_counter++;
					$data[8] =
					  "ID=$gene_id.intron.$intron_counter;Parent=$mRNA_id\n";
				}
				$data[8] =~ s/\s+\;\s+/\;/g;
...
###########################################################################################
	...
	&gff3_fix_phase_GTF( $exonerate_file . '.corrected.gff3' )
	  unless -s $exonerate_file . '.corrected.gff3.cds.gtf';

	if ($stop_after_correction) {
		print "User asked to stop after correction\n";
		exit();
	}
	&create_golden_gffs( $exonerate_file . '.corrected',
		\%details, \%vulgar_data )
...
###########################################################################################
	...
	my $query_outfile =
	  $is_cdna
	  ? "$exonerate_file.passed.cDNA"
	  : "$exonerate_file.passed.protein";
...
"$query_id rejected: higher scoring alignment with same co-ordinates ("
		...
		unless ( $details_hashref->{$query_id}{'identical_frac'} >=
			$identical_fraction )
		{
			print LOG
"$query_id rejected: identical_frac less than $identical_fraction: "
		...
		unless (
			$details_hashref->{$query_id}{'similar_frac'} >= $similar_fraction )
		{
			print LOG
			  "$query_id rejected: similar_frac less than $similar_fraction: "
			  . $details_hashref->{$query_id}{'similar_frac'} . "\n";
			$failed_cutoff++;
			next;
		}
		unless ( $details_hashref->{$query_id}{'mismatch_number'} <=
			$mismatch_cutoff )
		{
			print LOG
			  "$query_id rejected: mismatches more than $mismatch_cutoff: "
			  . $details_hashref->{$query_id}{'mismatch_number'} . "\n";
			$failed_cutoff++;
			next;
		}
		unless ( $is_cdna
			|| substr( $details_hashref->{$query_id}{'query_seq'}, 0, 1 ) eq
			'M' )
		{
		...
		{
			print LOG
			  "$query_id rejected: query does not end with stop codon (*): "
		...
		unless (
			substr( $details_hashref->{$query_id}{'target_seq'}, 0, 3 ) eq
			'ATG' )
		{
		...
		unless (
			(
				substr( $details_hashref->{$query_id}{'target_seq'}, -3, 3 ) eq
				'TAG'
...
"$query_id rejected: target does not end with stop codon (TAG, TAA or TGA): "
		...
		my $scaffold_seq =
		  &get_id_seq_from_fasta( $details_hashref->{$query_id}{'target_id'},
			$genome_sequence_file );
		die "Cannot find genome entry "
		  . $details_hashref->{$query_id}{'target_id'} . "\n"
		  if ( !$scaffold_seq );
		...
		{
			print LOG
			  "$query_id had to be skipped: has issues with target end ("
			  . $details_hashref->{$query_id}{'target_id'}
		...
		if ( $number_of_ns > ( length($scaffold_subseq) * 0.40 ) ) {
			$failed_cutoff++;
			print LOG
			  "$query_id rejected: genome sequence is more than 40% Ns\n";
			next;
		}
...
sub process_for_gene_prediction() {
	...
	my $training_set_size = int( $number_of_passing_genes * 0.40 )
	  if $training_set_size > int( $number_of_passing_genes * 0.40 );
	...
	$aug_optimization_geneset =
	  scalar( keys %{$aug_optimization_genes_hashref} );
	...
	$failed_cutoff +=
	  &filter_gff( "$gff_file", $accepted_hashref, "$gff_file.golden.gff3", 1 );
	&gff2zff( "$gff_file.golden.gff3", "$gff_file.golden.zff" );

	my @failed =
`$fathom_exec $gff_file.golden.zff $gff_file.golden.gff3.fasta -validate 2>&1|grep 'error\b'`;
	if (@failed) {
		...
		&filter_gff( "$gff_file", $accepted_hashref, "$gff_file.golden.gff3" );
		&gff2zff( "$gff_file.golden.gff3", "$gff_file.golden.zff" );
	}
	my @warnings =
`$fathom_exec $gff_file.golden.zff $gff_file.golden.gff3.fasta -validate 2>&1|grep warnings|egrep -v 'GC\.\.AG|short'`;
	...
	if ( scalar(@warnings) > 1 ) {
		...
		print "Fathom gave us "
		  . scalar(@warnings)
		  . " warnings (possible non-canonical splice sites, see $gff_file.golden.zff.warnings). Fixing...\n";
	...
	&filter_gff2_geneid( "$gff_file.geneid.gff3", $accepted_hashref,
		"$gff_file.geneid.golden.gff3", 1 );
...
#    . ". more or less \n"
	...
	if ( -s "$gff_file" ) {
		...
		&filter_gff( "$gff_file.golden.gff3", $training_genes_hashref,
...
"$augustus_filterGenes_exec $gff_file.golden.optimization.gb.bad.lst $gff_file.golden.optimization.gb > $gff_file.golden.optimization.good.gb 2>/dev/null"
	...
	if ( -s "$gff_file.geneid.gff3" ) {
		&filter_gff2_geneid( "$gff_file.geneid.golden.gff3",
			$training_genes_hashref, "$gff_file.geneid.golden.train.gff3", 1 );
		rename(
			"$gff_file.geneid.golden.train.gff3.rest",
			"$gff_file.geneid.golden.test.gff3"
		);
		&order_fasta( $genome_sequence_file, "$gff_file.geneid.golden.gff3" );
...
# Promoter-:First-:Single-        Single-:Terminal-:aataaa-       150:Infinity
		&gb2geneid( $train_ref, "$gff_file.golden.train.good.gb.geneid" )
		  if $train_ref;
		&gb2geneid( $test_ref, "$gff_file.golden.test.good.gb.geneid" )
		  if $test_ref;
		&gb2geneid( $opt_ref, "$gff_file.golden.optimization.good.gb.geneid" )
		  if $opt_ref;
		&order_fasta(
			"$gff_file.golden.optimization.good.gb.fasta",
			"$gff_file.golden.optimization.good.gb.geneid"
		) if -s "$gff_file.golden.optimization.good.gb.geneid";
		&order_fasta(
			"$gff_file.golden.train.good.gb.fasta",
			"$gff_file.golden.train.good.gb.geneid"
		) if -s "$gff_file.golden.train.good.gb.geneid";
		&order_fasta(
			"$gff_file.golden.test.good.gb.fasta",
...
sub gff2zff() {
	...
	while ( my $record = <GFF> ) {
		...
		foreach my $line (@lines) {
			next if $line =~ /^\s*$/ || $line =~ /^##/;
			my @data = split( "\t", $line );
			die "GFF $gff had a line that didn't begin with the refence $ref"
			  . $line
...
sub order_fasta() {
	my $fasta = shift || die;
	my $gff   = shift || die;
	&process_cmd("$cdbfasta_exec $fasta 2>/dev/null")
	  unless -s $fasta . '.cidx';
...
sub remove_overlapping_gff_geneid() {
	...
	while ( my $ln = <IN> ) {
		...
		{
			print LOG2
			  "Gene $data[8] has more than 2 terminal exons! Excluding\n";
			$ignore{ $data[8] } = 1;
			next;
		}
	...
	close LOG2;
	print
	  "\tOverlaps checked. Skipped $skipped genes. See $out.log for details\n";
	return $skipped;
}
...
# as i had to loop through every gene (nested as well)
	...
	while ( my $ln = <IN> ) {
		...
		if ( $data[2] eq 'gene' ) {
			...
			{
				print LOG2
"Overlapping gene found $gene_id. Skipping it and keeping $previous_gene_id\n";
				$skipped++;
				$toprint = '';
				next;
			}
...
sub gb2gff3() {
	...
	foreach my $id ( sort keys %{$hash_ref} ) {
		...
		if ( $hash_ref->{$id}{'strand'} == 1 ) {
			my $strand = '+';
			my ( $gstart, $e ) =
			  @{ $hash_ref->{$id}{'exons'}->{ $sorted_exons[0] } };
			my ( $s, $gend ) =
			  @{ $hash_ref->{$id}{'exons'}->{ $sorted_exons[-1] } };
			print GFF $id
			  . "\tGB\texon\t"
			  . $gstart . "\t"
			  . $gend
			...
			foreach my $exon_counter (@sorted_exons) {
				my ( $start, $stop ) =
				  @{ $hash_ref->{$id}{'exons'}->{$exon_counter} };
				print GFF $id
				  . "\tGB\texon\t"
				  . $start . "\t"
				  . $stop
...
sub gff3_fix_phase_GTF() {
	...
	foreach my $asmbl_id ( keys %$asmbl_id_to_gene_list_href ) {
		my $genome_seq =
		  &get_id_seq_from_fasta( $asmbl_id, $genome_sequence_file );
		my @gene_ids = @{ $asmbl_id_to_gene_list_href->{$asmbl_id} };
		foreach my $gene_id (@gene_ids) {
			my $gene_obj_ref = $gene_obj_indexer->get_gene($gene_id);
			foreach my $isoform ( $gene_obj_ref,
				$gene_obj_ref->get_additional_isoforms() )
			{
				...
				my $gtf_text = "";
				eval { $gtf_text = $isoform->to_GTF_format( \$genome_seq ); };
				if ($@) {
...
sub convert_to_gtf() {
	...
	foreach my $asmbl_id ( sort keys %$asmbl_id_to_gene_list_href ) {
		...
		foreach my $gene_id (@gene_ids) {
			...
			my $gene_obj_ref = $gene_obj_indexer->get_gene($gene_id);
			foreach my $gene_obj ( $gene_obj_ref,
				$gene_obj_ref->get_additional_isoforms() )
			{
...
sub run_exonerate() {
	...
	if (
		(
			$peptide_file
			|| ( $transdecoder_gff && $transdecoder_assembly_file )
		)
		&& $genome_file
	  )
	{
...
" -minorf $minorf -protein -in $peptide_file -separate -aat $genome_dir/*filter -threads $threads -intron_max $intron_size  $same_species ";
