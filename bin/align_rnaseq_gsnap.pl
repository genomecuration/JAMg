...
if ($suffix){
 ...
 $align_cmd ="$gsnap_exec -B 4 -D $gmap_dir -d $genome_dbname --nthreads=$cpus  --pairmax-rna=$intron_length  --localsplicedist=$intron_length -N 1 -Q --npaths=$repeat_path_number --format=sam --sam-use-0M --no-sam-headers ";
}else{
 $build_cmd = "$gmap_build_exec -D $gmap_dir -d $genome_dbname -k 13 -q 1 -e 0 --no-sarray $genome >/dev/null";
 $align_cmd ="$gsnap_exec --use-sarray=0 -B 4 -D $gmap_dir -d $genome_dbname --nthreads=$cpus  --pairmax-rna=$intron_length  --localsplicedist=$intron_length -N 1 -Q --npaths=$repeat_path_number --format=sam --sam-use-0M --no-sam-headers ";
}
