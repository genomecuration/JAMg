...
3. **Full-DAG gate test (the empirical go/no-go for `ancient()` cascade-suppression in snakemake 9): RESOLVED — `ancient()` does NOT suppress the cascade.** Empirically observed across 8 iterations of `tests/e2e/test_full_dag.sh`: snakemake 9.21 with the slurm executor schedules `genemark` regardless of `ancient()` decorators once any of its upstream inputs (`rnaseq_hints`, `golden_hints`, `softmasked`) is produced fresh in the session. Reason field on the rule's job is `Input files updated by another job: golden_hints, softmasked, rnaseq_hints` — this is independent of `--rerun-triggers` and not gated by `ancient()`. **Resolution (commit `d3de6f91`):** the genemark rule's shell now starts with a guard that checks for `genemark.staged.gtf` + `genemark.staged.gff3`; when present (test_full_dag.sh / test_evm.sh / test_ogs.sh / regen_snapshot.sh all stage to that name now), the shell cps them into the declared output positions and exits 0. snakemake's habit of deleting *declared* output files before invoking a rule's shell makes staging by the canonical name destroyed; the `.staged.` infix sidesteps that. **Final run (job 10252, commit `2a370189`):** test_full_dag.sh `COMPLETED` 16:27 elapsed, 37/37 rules, OGS.gff3 with 9 genes / 9 mRNA / 49 CDS / 54 exon, all 6 OGS products (gff3/bed/gtf/mRNA.fasta/CDS.fasta/pep.fasta) produced, `OK: full DAG produced OGS.gff3 with no legacy artefacts`.
4. **Snapshot refresh: DONE.** `tools/snapshot_dag_outputs.sh` invoked against the test_full_dag.sh output of job 10252. `test_suite/fixtures/snapshot/` (147 MB) refreshed; `augustus/augustus_results.gff3` now carries 40 exon features (the `--UTR on` change captured); `evm/EVM.gff3` produced cleanly (not the manual-copy stopgap that was there before).
   Additional fixes captured in commit `2a370189`:
   - `ogs_create_features` rule's downstream sed pipeline targeted `EVM.combined.pasa2.gff3.renamed.gff3` but `bin/create_features_from_gff3.pl` actually emits `<input>.gff3` (i.e. `EVM.combined.pasa2.gff3.gff3`). The mismatch made the rule fail at the grep with `No such file or directory`. Rule now reads from the correct path and writes the stripped+code-prefixed result straight to the declared snakemake output `EVM.combined.pasa2.renamed.gff3`.
   - `tools/build_fixtures.sh` now removes any stale `.cidx` and `.fai` before (re)building the mini-genome. A stale `.cidx` from a prior 1 Mb fixture build had been the actual blocker for `ogs_pasa_compare_1`/`_2` (cdbyank reports `Error: invalid 4 database size - (1012508 vs 101258) please rerun cdbfasta`). PASA regenerates `.cidx` on first access when it doesn't exist.
