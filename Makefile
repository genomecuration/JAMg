...
all: 
	...
	cd 3rd_party/hhsuite && cp -fp lib64/libffindex.so.0.1 ../lib64/
	...
	cd 3rd_party/PASA && if [ ! -d bin ]; then mkdir bin; fi && $(MAKE) && cd seqclean && wget -qc ftp://ftp.ncbi.nih.gov/pub/UniVec/UniVec && find . -name "*.tar.gz" -exec tar xzf '{}' \; && cd mdust && $(MAKE) && cd ../psx && $(MAKE) && cd ../trimpoly && $(MAKE) && cd .. && find . -type f -executable -exec cp -pf '{}' ../../bin/ \;
  ...
  printf "\n\nYou have to now configure RepeatMasker. Answer the questions. DEFAULTS SHOULD WORK. Can configure RMBLAST AND HMMER. Use the RMBlast NCBI option as DEFAULT.\n" && sleep 10 && ./configure
	...
	cd 3rd_party/bin && ln -fs ../hhsuite/bin/* .
