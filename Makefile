...
all: 
	...
	cd 3rd_party/RepeatMasker && if [ -e Libraries/Dfam.hmm.bz2 ]; then bunzip2 -vd Libraries/Dfam.hmm.bz2; fi && if [ -e Libraries/taxonomy.dat.bz2 ]; then bunzip2 -vdf Libraries/taxonomy.dat.bz2 ; fi && echo "I will download TRF from http://tandem.bu.edu/trf. You must read and accept their (v. short) license (see http://tandem.bu.edu/trf/trf.license.html) otherwise exit now:" && sleep 5 && echo "The author of this software grants to any individual or organization the right to use and to make an unlimited number of copies of this software. You may not de-compile, disassemble, reverse engineer, or modify the software. This software cannot be sold, incorporated into commercial software or redistributed. The author of this software accepts no responsibility for damages resulting from the use of this software and makes no warranty or representation, either express or implied, including but not limited to, any implied warranty of merchantability or fitness for a particular purpose. This software is provided as is, and the user assumes all risks when using it." && sleep 10 && wget -r http://tandem.bu.edu/trf/downloads/trf407b.linux64 -O trf && chmod +x trf && echo "You have to now configure RepeatMasker (cd 3rd_party/RepeatMasker; ./configure). Answer the questions. The trf file is in `pwd`. Use the RMBlast NCBI option when asked 'Add a Search Engine', the files are in `pwd`/../bin/" 
...
clean:
	...
	cd 3rd_party/blatSrc && $(MAKE) clean
	cd 3rd_party/exonerate && $(MAKE) clean
	cd 3rd_party/snap && $(MAKE) clean
	cd 3rd_party/transdecoder && $(MAKE) clean
	cd 3rd_party/RepeatMasker && if [ -e Libraries/Dfam.hmm ] ; then bzip2 -v Libraries/Dfam.hmm; fi && if [ -e Libraries/taxonomy.dat ] ; then bzip2 -v Libraries/taxonomy.dat; fi && rm -f trf
