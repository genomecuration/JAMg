...
sub new {
 ...
 my $self = {
  ...
  mRNA_coords =>
    0, #assigned to anonymous hash of end5->end3 relative to the parent sequence
  CDS_coords =>
    0, #assigned to anonymous hash of end5->end3 relative to the parent sequence
  mRNA_exon_objs =>
    0,    # holds arrayref to mRNA_obj, retrieve only thru method: get_exons()
  ...
  CDS_sequence =>
    undef
  ,   #holds CDS sequence (translated to protein); based on CDS_exon coordinates
  CDS_seq_length => 0,
  ...
  num_additional_isoforms =>
    0,    # number of additional isoforms stored in additonal_isoform list below
  additional_isoforms =>
    []    # stores list of Gene_objs corresponding to the additional isoforms.
...
sub build_gene_obj_exons_n_cds_range {
 ...
 if ( $cds_lend > 0 && $cds_rend > 0 ) {
  ...
  unless (@cds_range) {
   warn "Error, no CDS exon coords built based on exon overlap";
  }
 }
 ...
 foreach my $cds_coords_aref (@cds_range) {
  ...
  my ( $cds_end5, $cds_end3 ) =
    ( $orient eq '+' ) ? ( $cds_lend, $cds_rend ) : ( $cds_rend, $cds_lend );
...
sub join_adjacent_exons {
 ...
 while (@exons) {
  ...
  if (
   ( $strand eq '+' && $prev_end3 == $next_end5 - 1 )    # adjacent
...
sub AAToNucleotideCoords {
   ...
   )
 {
  ...
  if ($cds) {
   ...
   if ( $strand eq "+" ) {
    $A2NMapping->{$currAA} =
      $cds_coords[0] < $cds_coords[1] ? $cds_coords[0] : $cds_coords[1];
   }
   else {
    $A2NMapping->{$currAA} =
      $cds_coords[0] < $cds_coords[1] ? $cds_coords[1] : $cds_coords[0];
   }
   print
"DEBUG: $strand $cds_count AA range ($currAA - $PCDS_coords->{$currAA}) nucleotide start($A2NMapping->{$currAA})\n"
     if ($debug);
 ...
 $cds_count = 0;
 foreach my $PCDS_end5 ( sort { $a <=> $b; } ( keys %$PCDS_coords ) ) {
  ...
  if ( $aacoord >= $PCDS_end5 && $aacoord <= $PCDS_end3 ) {
   ...
   print "DEBUG: Mapping $aacoord to $translated_bp in cds $cds_count\n"
     if ($debug);
   print
"DEBUG: CDS $PCDS_end5 - $PCDS_end3 nucleotide start $A2NMapping->{$PCDS_end5}, nuc offset $nucleotide_offset\n"
     if ($debug);
  }
 ...
 if ( $translated_bp == -1 ) {
  $translated_bp = undef;
  print STDERR
    "Unable to translate AA coordinate: $aacoord. Off end. Using undef\n"
    if ($debug);
 }
...
sub refine_gene_object {
 ...
 my ( $CDS_coords, $mRNA_coords ) =
   ( $self->{CDS_coords}, $self->{mRNA_coords} );
...
sub trivial_refinement {
 ...
 ( $gene_end5, $gene_end3 ) =
   ( $strand eq "+" ) ? ( $gene_lend, $gene_rend ) : ( $gene_rend, $gene_lend );
 if (@model_coords) {
  ## bound model by protein coding span
  my $model_lend = shift @model_coords;
  my $model_rend = pop @model_coords;
  ( $model_end5, $model_end3 ) =
      ( $strand eq "+" )
    ? ( $model_lend, $model_rend )
    : ( $model_rend, $model_lend );
 }
 else {
...
sub create_all_sequence_types {
 ...
 $self->create_cDNA_sequence($big_seq_ref)
   unless ( exists( $atts{cDNA} ) && $atts{cDNA} );
 $self->create_gene_sequence( $big_seq_ref, 1 )
   if ( $atts{unspliced_transcript} );    #highlight exons by default.

 if ( $self->is_coding_gene() ) {
  $self->create_CDS_sequence($big_seq_ref)
    unless ( exists( $atts{CDS} ) && $atts{CDS} );
  $self->create_protein_sequence($big_seq_ref)
    unless ( exists( $atts{protein} ) && $atts{protein} );
 }
...
sub create_cDNA_sequence {
 ...
 unless ($seq_ref) {
  print STDERR
    "The parent sequence must be specified for the cDNA creation method\n";
  return;
 }
 ...
 foreach my $exon_obj ( sort { $a->{end5} <=> $b->{end5} } @exons ) {
  ...
  $cDNA_seq .=
    substr( $$sequence_ref, ( $coord1 - 1 ), ( $coord2 - $coord1 + 1 ) );
 }
 if ( $strand eq '-' ) {
...
sub create_CDS_sequence {
 ...
 unless ($seq_ref) {
  print STDERR
    "The parent sequence must be specified for the CDS creation method\n";
  return;
 }

 unless ( $self->is_coding_gene() ) {
  print STDERR "Warning: No coding region specified for gene: "
    . $self->toString();
  return ("");
 }
...
sub create_protein_sequence {
 ...
 unless ($cds_sequence) {
  ...
  unless ( ref($seq_ref) eq 'SCALAR' ) {
   print STDERR
     "I require an assembly sequence ref if the CDS is unavailable\n";
   return;
  }
...
sub create_gene_sequence {
 ...
 if ($highlight_exons_flag) {
  ...
  while (@exons) {
   ...
   my ( $lend, $rend ) = sort { $a <=> $b } $exon->get_coords();
   $gene_seq .=
     lc( substr( $$big_seq_ref, $prev_rend, $lend - $prev_rend - 1 ) );
   $gene_seq .= uc( substr( $$big_seq_ref, $lend - 1, $rend - $lend + 1 ) );
   $prev_rend = $rend;
  }
  ...
  $gene_seq =
    substr( $$big_seq_ref, ( $coord1 - 1 ), ( $coord2 - $coord1 + 1 ) );
 }
...
sub toString {
 ...
 foreach my $key ( keys %$self ) {
  ...
  if ( ref $value ) {
   ...
   if ( ( $value eq '0' || !defined($value) ) && $key !~ /^is_/ ) {
    next;
   }    #dont print unpopulated info.
   $output .= "\t$key:\t$value\n";
  }
 }
...
#print "confirming splice sites at "  . ($splice_1_start +1) . " and " . ($splice_2_start + 1) . "\n"if $SEE;
  ...
  my ( $acceptor, $donor ) =
      ( $strand eq '+' )
    ? ( $splice_1, $splice_2 )
    : ( &reverse_complement($splice_2), &reverse_complement($splice_1) );
  ...
  if ( $exon_type eq "internal" || $exon_type eq "terminal" ) {
   unless ($check_acceptor) {
    $error_string .=
      "\tnon-consensus $acceptor acceptor splice site at $coord2\n";
   }
  }
 }
 return ($error_string);
 ...
 foreach my $exon (@exons) {    #relying on a sorted list
  ...
  if ( my $cds = $exon->get_CDS_obj() ) {
   ...
   if ( $exon_end5 != $cds_end5 ) {
    my $adj_utr_end3_coord =
      ( $strand eq '+' ) ? ( $cds_end5 - 1 ) : ( $cds_end5 + 1 );
...
sub get_3prime_UTR_coords {
 ...
 my $seen_CDS_flag = 0;
 foreach my $exon (@exons)
 {    #relying on a reverse sorted list (3' exons should come first)
  my ( $exon_end5, $exon_end3 ) = $exon->get_coords();
  if ( my $cds = $exon->get_CDS_obj() ) {
   $seen_CDS_flag = 1;
   my ( $cds_end5, $cds_end3 ) = $cds->get_coords();
   if ( $exon_end3 != $cds_end3 ) {
    my $adj_utr_end5_coord =
      ( $strand eq '+' ) ? ( $cds_end3 + 1 ) : ( $cds_end3 - 1 );
...
sub get_3prime_UTR_sequence {
 ...
 unless (
       ( my $cDNA_utr = lc substr( $cDNA, $utr_start_pos - 1, $utr_length ) ) eq
       lc $UTR_seq )
 {
  confess
"Error, 3' UTR extracted from cDNA is different from UTR sequence extracted from genome.\n"
    . "cDNA_utr:\n$cDNA_utr\nUTR_from_genome:\n$UTR_seq\n\n";
 }
...
sub to_alignment_GFF3_format {
 ...
 foreach my $exon (@exons) {
  ...
  $gff3_alignment_text .= join( "\t",
                                $scaff, $source, "match", $lend, $rend, '.',
                                $orient, '.',
...
"gene_id \"$gene_id\"; transcript_id \"$transcript_id\"; name \"$com_name\";"
   ...
   foreach my $exon (@exons) {
    ...
    $gtf_text .= join( "\t",
                   $asmbl_id, ".", "exon", $lend, $rend, ".", $orientation, ".",
...
sub to_GTF_format {
 ...
 my $gene_obj_for_gtf =
   $gene_obj;    #if got stop codon, will need to strip it off.
 my $com_name = $gene_obj->{com_name};
 ...
 unless ($is_pseudogene) {
  ...
  if (@found_stop) {
   ...
   if ( ( length($cds_seq) - $first_phase ) % 3 == 0 )
   {    # yes, stop is in frame.
 ...
 foreach my $exon ( $gene_obj_for_gtf->get_exons() ) {
  ...
  if ($cds) {
   ...
   unless ($is_pseudogene) {
    ...
    if ($phase) {
     $phase =
       ( $phase == 1 )
       ? 2
       : 1;    # reverse it according to GFF3 vs. GTF representation.
    }
   }
   ...
   push(
         @gtf_text,
         [
           $seqname, $source, "CDS", $cds_lend, $cds_rend, "0", $orientation,
           "$phase",
           "gene_id \"$gene_id\"; transcript_id \"$transcript_id\"; $name_txt"
         ]
   );
...
sub get_start_codon_coordinates {
 ...
 foreach my $cds_coordpair (@cds_coords) {
  ...
  my $extract_len =
    ( $cds_seg_len < $start_len_want ) ? $cds_seg_len : $start_len_want;
  if ( $orient eq '+' ) {
 ...
 if ( $start_len_want > 0 ) {
  confess
    "Error, trouble extracting start codon coordinates from cds coordsets: "
    . Dumper( \@cds_coords );
 }
...
sub get_stop_codon_coords {
 ...
 foreach my $cds_coordpair (@cds_coords) {
  ...
  my $extract_len =
    ( $cds_seg_len < $stop_len_want ) ? $cds_seg_len : $stop_len_want;
  if ( $orient eq '+' ) {
 ...
 if ( $stop_len_want > 0 ) {
  confess
    "Error, trouble extracting stop codon coordinates from cds coordsets: "
    . Dumper( \@cds_coords );
 }
...
sub trim_stop_codon {
 ...
 if ( $stop_len_want > 0 ) {
  confess
    "Error, trouble extracting all stop codon coordinates from cds coordsets. "
    . $gene_obj->toString();
 }
...
sub to_GFF3_format {
 ...
 my $feat_type =
   ( $gene_obj->{gene_type} eq "protein-coding" )
   ? "gene"
   : $gene_obj->{gene_type};
...
#	$gff3_text .= ";transcript_id=gnl|".$gene_obj->{genbank_submission}."|mrna.".$tmodel_id;
    ...
    if ( $sequence_ref && $intron_start && $intron_stop ) {
     my (
          ...
          $acceptor_stop, $donor_site, $acceptor_site
     );
     if ( $strand eq '+' ) {
      ( $donor_start, $donor_stop, $acceptor_start, $acceptor_stop ) = (
                                              $intron_start,
                                              ( $intron_start + 1 ),
                                              ( $intron_stop - 1 ), $intron_stop
      );
...
"$reference_id\t$source\tfive_prime_cis_splice_site\t$donor_start\t$donor_stop\t.\t$strand\t.\tID=$intron_id_string.donor;Parent=$model_id;donor_site=$donor_site\n"
       if $donor_start;
     $gff3_text .=
"$reference_id\t$source\tintron\t$intron_start\t$intron_stop\t.\t$strand\t.\tID=$intron_id_string;Parent=$model_id\n"
       if ( $intron_start && $intron_stop );
     $gff3_text .=
"$reference_id\t$source\tthree_prime_cis_splice_site\t$acceptor_start\t$acceptor_stop\t.\t$strand\t.\tID=$intron_id_string.acceptor;Parent=$model_id;acceptor_site=$acceptor_site\n"
       if $acceptor_start;
    }
...
sub BED_line_to_gene_obj {
 ...
 print "Coding: $coding_lend-$coding_rend, Exons: " . Dumper( \@exons )
   if $DEBUG;

 my $gene_obj = new Gene_obj();
 $gene_obj->build_gene_obj_exons_n_cds_range( \@exons, $coding_lend,
                                              $coding_rend, $orient );
...
sub to_GTF2_format () {
 ...
 my $asmbl_id =
      $properties_href->{seqname}
   || $gene_obj->{asmbl_id}
   || die "Error, no asmbl_id as gene_obj att";
 ...
 if ( $gene_obj->{gene_type} eq "protein-coding" ) {
  ...
  if ( !$gene_obj->is_pseudogene() ) {
   ...
   if ( !$gene_obj->is_3prime_partial() ) {
    ...
    unless (@stop_codon_objs) {
     confess $gene_obj->toString()
       . "Error, no stop codon objs retrieved for non 3' partial gene";
    }
   }
   if ( !$gene_obj->is_5prime_partial() ) {
    ...
    unless (@start_codons) {
     confess $gene_obj->toString()
       . "Error, no start codon extracted for non 5'partial gene.";
    }
   }
  ...
  foreach my $start_codon (@start_codons) {
   my ( $start_lend, $start_rend ) =
     sort { $a <=> $b } $start_codon->get_coords();
   my $phase = &$frame_convert( $start_codon->{phase} );
   $gtf2_text .=
"$asmbl_id\t$source\tstart_codon\t$start_lend\t$start_rend\t.\t$strand\t$phase\tgene_id \"$gene_id\"; transcript_id \"$model_id\";\n";
  }

  foreach my $exon ( $gene_obj->get_exons() ) {
   my ( $exon_lend, $exon_rend ) = sort { $a <=> $b } $exon->get_coords();
   $gtf2_text .=
"$asmbl_id\t$source\texon\t$exon_lend\t$exon_rend\t.\t$strand\t.\tgene_id \"$gene_id\"; transcript_id \"$model_id\";\n";

   if ( $gene_obj->is_pseudogene() ) {
    next;
   }    # don't bother trying to report nonsensical CDSs.

   if ( my $cds_obj = $exon->get_CDS_obj() ) {
    my ( $cds_lend, $cds_rend ) = sort { $a <=> $b } $cds_obj->get_coords();
    my $phase = $cds_obj->{phase};
    unless ( defined($phase) ) {
     die "Error, no phase defined for cds($cds_lend-$cds_rend) of gene"
       . $gene_obj->toString();
    }
...
sub _extract_start_codons {
 ...
 for ( my $i = 0 ; $i <= $#exons ; $i++ ) {
  if ( my $cds = $exons[$i]->get_CDS_obj() ) {
   ...
   if ( $cds_len >= 3 ) {
    ...
    if ( $orientation eq '+' ) {
     push( @start_codons,
           CDS_exon_obj->new( $cds_end5, $cds_end5 + 2 )->set_phase(0) );
     last;
    }
    else {
     push( @start_codons,
    ...
    if ( $next_cds_len >= $missing_length ) {
     ...
     if ( $orientation eq '+' ) {
      push(
            @start_codons,
            CDS_exon_obj->new( $next_cds_end5,
                               $next_cds_end5 + $missing_length - 1 )
 ...
 unless ( $sum_len == 3 ) {
  print "Error, sum len of start codons != 3 ( = $sum_len, instead) "
    . $self->toString()
    . "starts:\n";
  my $i = 0;
...
sub _remove_stop_codons {
 ...
 my $orientation = $self->get_orientation();
 my @exons       = reverse $self->get_exons()
   ;    # examining exons in reverse order, starting from stop codon direction.
  ...
  if ( my $cds = $exons[$i]->get_CDS_obj() ) {
   ...
   if ( $cds_length > 3 ) {
    ...
    if ( $orientation eq '+' ) {
     $cds->{end3} -= 3;
     push( @stop_codons,
           CDS_exon_obj->new( $cds_end3 - 2, $cds_end3 )->set_phase(0) );
    }
    else {
     $cds->{end3} += 3;
     push( @stop_codons,
    ...
    if ( $next_cds_length <= $missing_stop_length ) {
     ...
     if ( $missing_stop_length > 0 ) {
      ...
      if ( $missing_stop_length != 1 ) {
       die
"Error, too much of the stop codon is left (missing_length = $missing_stop_length).  Should only be 1 ";
      }
      ...
      if ( $cds_length > 1 ) {
       if ( $orientation eq '+' ) {
        $next_cds_obj->{end3} -= 1;
        push( @stop_codons,
              CDS_exon_obj->new( $cds_end3, $cds_end3 )->set_phase(0) );
       }
       else {
        $next_cds_obj->{end3} += 1;
        push( @stop_codons,
     ...
     if ( $orientation eq '+' ) {
      ...
      push(
            @stop_codons,
            CDS_exon_obj->new(
                               $cds_end3 - $missing_stop_length + 1, $cds_end3
              )->set_phase(0)
      );
     }
     else {
 ...
 if ( $sum_len != 3 ) {
  print "Error, stop codons sum length != 3 ( = $sum_len, instead) "
    . $self->toString();
  my $i = 0;
  foreach my $stop_codon (@stop_codons) {
...
sub DESTROY {
 ...
 warn "DESTROYING gene_obj: "
   . $self->{TU_feat_name} . ","
   . $self->{Model_feat_name} . "\n"
   if $main::DEBUG;
...
sub validate_start_codon {
 ...
 my $cds_sequence = $self->get_CDS_sequence()
   or confess
"Error, cannot get CDS sequence.  It must be built prior to calling this method";
 ## currently, only trust Met start codons.
 my $start_codon = uc substr( $cds_sequence, 0, 3 );
 if ( $start_codon ne "ATG" ) {
  die $self->toString()
    . "Error, start codon is not M (codon $start_codon instead)!";
...
"Error, cannot get CDS sequence.  It must be built prior to calling this method";
 ...
 unless ($found_stop_codon_flag) {
  die $self->toString()
    . "Error, stop codon $curr_stop_codon is not an acceptable stop codon: [@stop_codons]\n";
 }
...
sub new {
 shift;
 my $self = {
              end5         => 0,        # stores end5 of mRNA exon
...
sub merge_exon {
 ...
 ## merge the exons.
 my @coords =
   sort { $a <=> $b } ( $self->get_coords(), $other_exon->get_coords() );
 my $lend = shift @coords;
 my $rend = pop @coords;

 my ( $new_end5, $new_end3 ) =
   ( $self->get_orientation() eq '+' ) ? ( $lend, $rend ) : ( $rend, $lend );
...
sub new {
 shift;
 my $self = {
              end5      => 0,        #stores end5 of cds exon
...
sub merge_CDS {
 ...
 my @coords =
   sort { $a <=> $b } ( $self->get_coords(), $other_cds->get_coords() );
 my $lend = shift @coords;
 my $rend = pop @coords;
 ...
 my ( $end5, $end3 ) =
   ( $orientation eq '+' ) ? ( $lend, $rend ) : ( $rend, $lend );
