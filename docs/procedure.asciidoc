...
+
[source,bash]
$JAMG_PATH/3rd_party/bin/maskFastaFromBed -soft -fi $GENOME_PATH -fo $GENOME_PATH.softmasked -bed $GENOME_PATH.out.gff # this last file is the output from RepeatMasker
...
find $GENOME_NAME.softmasked_dir1 -type f -exec sh -c 'echo "snap $GENOME_NAME.hmm $1 -lcmask -quiet > $1.snap 2>/dev/null " '  find-copy '{}' \; > snap.commands
ParaFly -c snap.commands -CPU $LOCAL_CPUS -v -shuffle
cat $GENOME_NAME.softmasked_dir1/*snap > snap.out 
$JAMG_PATH/3rd_party/evidencemodeler/OtherGeneFinderTrainingGuide/SNAP/SNAP_output_to_gff3.pl snap.out $GENOME_PATH | $JAMG_PATH/3rd_party/PASA/misc_utilities/gff3_to_gtf_format.pl - $GENOME_PATH > snap.gtf
# evaluate
$JAMG_PATH/3rd_party/eval-2.2.8/evaluate_gtf.pl -g ./*golden.test.gtf  snap.gtf > snap.eval
# once satisfied
rm -rf snap.out $GENOME_NAME.softmasked_dir1
+
...
wget -c --mirror --accept=protein.faa.gz ftp://ftp.ncbi.nlm.nih.gov/refseq/release/invertebrate && gunzip -dc ftp.ncbi.nlm.nih.gov/refseq/release/invertebrate/*gz > foreign_proteins.fsa
# OR:
