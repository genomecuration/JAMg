...
# now we need to prepare some files for optimizing the extrinsic evidence scores. Specifically we need to recreate the hints for use with our 'test/train data' derived from the genbank files.
...
$JAMG_PATH/bin/align_rnaseq_gsnap.pl -fasta complete_reference_training_set.fasta -dbname reference_training -pattern1 _1_ -pattern2 _2_ -nofail
$JAMG_PATH/bin/augustus_RNAseq_hints.pl -bam *uniq_mult.bam -genome complete_reference_training_set.fasta 
$JAMG_PATH/bin/prepare_domain_exon_annotation.pl -fasta complete_reference_training_set.fasta -engine localmpi


The next step is optimizing the parameters using for the HMM buidling. You can't run these in parallel, they must run sequentially...

[source,bash]
# the following will train the coding sequence models.
...
# now also train the UTR
$JAMG_PATH/bin/optimize_augustus.pl --species $SPECIES --optimize ../final_golden_genes.gff3.nr.golden.optimization.good.gb --onlytrain ../final_golden_genes.gff3.nr.golden.train.good.gb \
 --cpus $LOCAL_CPUS --kfold $LOCAL_CPUS --onlyutr --metapars metaparameters_utr_broad.cfg | tee opt_utr.log
# now train extrinsic evidence, especially the RNA-Seq derived data. In this case, also taking UTR values into account:
$JAMG_PATH/bin/optimize_extrinsic_augustus.pl --species $SPECIES -opt ../final_golden_genes.gff3.nr.golden.optimization.good.gb --train ../final_golden_genes.gff3.nr.golden.train.good.gb \
 -cpu $LOCAL_CPUS -notrain -hint *uniq_mult.bam.rnaseq.hints -utr -extr $JAMG_PATH/configs/extrinsic_meta.cfg | tee opt_extr.log
