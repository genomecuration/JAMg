...
There are a number of ways to get these golden gene sets. First, PASA has a script that allows one to generate such models. Second, with JAMG we can use the 'prepare_golden_genes_for_predictors.pl' to acquire a set of good gene models informed by a splice aware aligner such as exonerate.
[source,bash]
...
$JAMG_PATH/bin/prepare_golden_genes_for_predictors.pl -genome $GENOME_PATH.masked -softmasked $GENOME_PATH.softmasked \
 -same_species -intron $MAX_INTRON_LENGTH -cpu $LOCAL_CPUS -norefine \
 -transdecoder_gff ./*.assemblies.fasta.transdecoder.gff3 \
 -transdecoder_peptides ./*.assemblies.fasta.transdecoder.pep \
